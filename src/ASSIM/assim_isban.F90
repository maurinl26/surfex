...
REAL(KIND=JPRB), DIMENSION (KI), INTENT(IN) ::  PLON_IN
...
!
! General snow update
IF ( CASSIM_ISBA /= 'OI   ' .AND. CASSIM_ISBA/='PF   ') THEN
  !
...
ELSEIF (CASSIM_ISBA == 'PF   ') THEN ! B. Cluzet
  CALL ASSIM_NATURE_ISBA_PF(IM%O, IM%S, HPROGRAM, KI, HTEST, PLON_IN, PLAT_IN)
ELSE
...
!to be improved later - needed for surfex course
 CALL AVERAGE_DIAG_MISC_ISBA_n(IM%ID%DM, IM%ID%NDM, IM%O, IM%NK, IM%NP, IM%NPE)
 !
