...
!     #########
      SUBROUTINE DEFAULT_ASSIM(OASSIM,HASSIM,HASSIM_ISBA,KPRINTLEV,         &
                               ...
                               OENKF,ODENKF,OCROCO,                         &
                               PERROBS_FACTOR_M, HPF_CROCUS, KEFF_PF,       &
                               PDLOC_PF, OLOO_PF, HTEST)
!     ########################################################################
!
...
LOGICAL, INTENT(OUT) :: OCROCO
LOGICAL, INTENT(OUT) :: OLOO_PF
...
HPF_CROCUS="GLOBAL"
KEFF_PF = 1
PDLOC_PF = 1E-8 ! Localisation radius. unit is in degrees. By default, take only local observation.
OLOO_PF=.FALSE. ! activate leave-one-out experiments with the PF (exclude local observations)
HOBS_M = (/'  ','  ','  ','  ','  ','  ','  ','  ','  '/)
PERROBS_M = (/1.0,0.1,0.4,0.2,0.1,10000.,10000., 10000.,10000./)!bcrj modified
PERROBS_FACTOR_M = (/1.,1.,1.,1.,1.,1.,1.,1.,1./)!bcrj modified
PQCOBS_M = (/999.,999.,999.,999.,999.,999.,999.,999.,999./)!bcrj modified
KNCO = (/0,0,0,0,0,0,0,0,0/)!bcrj modified
OOBSNAT = .FALSE.
!
KIVAR = 1
KVAR = 4
