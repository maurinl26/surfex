...
!       including PF scheme for snow 2018     B.Cluzet
!
!*       0.   DECLARATIONS
!             ------------
!
...
!
 LOGICAL                               :: LASSIM               ! Assimilation or not
                                                               !'.TRUE.'
                                                               !'.FALSE.'
 LOGICAL                               :: LCROCO        ! B. Cluzet Assimilation in Crocus
 ...
 CHARACTER(LEN=6)                      :: CPF_CROCUS           ! Type of Particle Filter algo used in Crocus.
 ...
 LOGICAL                               :: LOBSHEADER
 LOGICAL                               :: LOBSNAT
 LOGICAL                               :: LGLOBAL_PF = .TRUE.  ! Is the Particle Filter Algo Global or Local.
 LOGICAL                               :: LLOO_PF              ! activate leave-one-out experiments with the PF (exclude local observations)

 INTEGER, PARAMETER                    :: NOBSMAX = 9          ! Maximum number of observations
 ...
 INTEGER                               :: NEFF_PF              ! target effective sample size (Particle Filter)
 INTEGER                               :: NRETM,NRETS,NRETD,NCOUNTM,NRETR ! snow assimilation variable counters
 CHARACTER(LEN=30),DIMENSION(NOBSMAX)  :: COBS_M               ! Observation variable name
 ...
 CHARACTER(LEN=30),DIMENSION(:), ALLOCATABLE  :: COBS          ! Identifier for simulated observations
...
!                                                              ! covariance of model errors if B evolving (max dim)
 ...
 REAL,DIMENSION(NOBSMAX)               :: XERROBS_FACTOR_M ! magnification factor for obs errors in PF.
 ...
 REAL,DIMENSION(:,:,:,:),ALLOCATABLE   :: XF_AUX_RATIO         ! if band ratio assim, store the band ratio numerator band (def band 5)
                                                               ! for reconstruction of the reflectances
 REAL,DIMENSION(:,:,:),ALLOCATABLE     :: XI 
 INTEGER,DIMENSION(:,:,:),ALLOCATABLE     :: NPFPART            ! B. Cluzet on each grid point, SORTED vector of particles to be duplicated after SIR PF (hence with repetitions), 
 REAL,DIMENSION(:,:), ALLOCATABLE      :: XYO                  ! vector of observations
 REAL,DIMENSION(:), ALLOCATABLE      :: XYO_SCF                  ! vector of observations
 ...
 REAL,DIMENSION(:),ALLOCATABLE         :: XERROBS_FACTOR
...
INTEGER :: NIE
REAL :: XASSIM_WINH
REAL :: XDLOC_PF  ! localisation distance (degrees) for the RLOCAL particle filter
...
REAL                                  :: XTAUSCF    ! Tau parameter for SWE-SCF depletion curve 
REAL                                  :: XCEILSCF   ! Ceil parameter for SWE-SCF depletion curve [kg/m2]
LOGICAL :: LENKF
LOGICAL :: LDENKF
LOGICAL :: LENS_GEN
