...
USE MODN_IO_OFFLINE,     ONLY : NCOMPRESS_NETCDF
!
!
...
IF (PRESENT(KTYPE)) THEN
  !print*,"def_var_netcdf input  ", KFILE_ID,HNAME,KTYPE,"KDIM_ID ",KDIM_ID,"SIZE(KDIM_ID) ",SIZE(KDIM_ID)
  IRET(1) = NF90_DEF_VAR(KFILE_ID,HNAME,KTYPE,KDIM_ID,IVAR_ID)!,deflate_level=1)
  !print*,"def_var_netcdf output  ", KFILE_ID,HNAME,KTYPE,KDIM_ID,IVAR_ID,IRET(1)
ELSE
  IRET(1) = NF90_DEF_VAR(KFILE_ID,HNAME,NF90_DOUBLE,KDIM_ID,IVAR_ID)!,deflate_level=1)
  !print*,"def_var_netcdf2  ", KFILE_ID,HNAME,KTYPE,KDIM_ID,IVAR_ID,IRET(1)
ENDIF
!
IF (NCOMPRESS_NETCDF > 0 .AND. NCOMPRESS_NETCDF <= 9) THEN
  IRET(1) = NF90_DEF_VAR_DEFLATE(KFILE_ID, IVAR_ID, 0 ,1, NCOMPRESS_NETCDF)
END IF
!
IF (INDIM/=0) THEN
  IF (PRESENT(KTYPE)) THEN
