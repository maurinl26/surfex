...
! 26-09-12 : Original commit by Stephanie_FAROUX (git blame)
...
GSPEC = (HINIT=='PGD' .OR. (HINIT=='PRE' .AND. LOASIS) .OR. &
        (HINIT=='OFF' .AND. (LWRITE_TOPO .OR. LXIOS .OR. LOASIS .OR. GSHADOWS)))
!
IF ( NRANK==NPIO ) THEN
  !
...
ELSE
  U%NDIM_FULL = 0
ENDIF
! others process wait for broadcast here
IF (NPROC>1) THEN
#ifdef SFX_MPI
  IF (GSPEC) THEN
...
NSIZE_TASK(:) = 0
!
IF (NRANK==NPIO) THEN ! only when //
  DO JJ=1,U%NDIM_FULL
    NSIZE_TASK(NINDEX(JJ)) = NSIZE_TASK(NINDEX(JJ)) + 1 ! count number of points by MPI task
  ENDDO
ENDIF
!
NSIZE = 0
...
#endif
  NSIZE = MAXVAL(NSIZE_TASK) ! maximum number of points by MPI task, can be over nsize_task
  DO JJ=0,NPROC-1
    CALL WLOG_MPI('SIZE_TASK ',KLOG=JJ,KLOG2=NSIZE_TASK(JJ))
  ENDDO
ELSE
  NSIZE = NSIZE_TASK(0) ! to be deleted ?
ENDIF
!
...
!
!Routine to give the associated thread rank to each point in the domain
!
IMPLICIT NONE
!
INTEGER, INTENT(IN) :: KPROC ! number of MPI threads
INTEGER, INTENT(IN) :: KPROCMIN
INTEGER, INTENT(IN) :: KSIZE ! number of points to allocate
INTEGER, DIMENSION(:), INTENT(INOUT) :: KINDEX !array of thread rank number for each domain point
LOGICAL, INTENT(IN) :: OSHADOWS
!
INTEGER, DIMENSION(0:KPROC-1) :: ISIZE_TASK !array of number of point for each threads (index=rank of threads)
INTEGER :: JI, JJ, CPT, IPROC1
REAL(KIND=JPRB) :: ZHOOK_HANDLE
!
IF (LHOOK) CALL DR_HOOK('INIT_INDEX_MPI:SET_NB_POINTS_LIN',0,ZHOOK_HANDLE)
!
 CALL GET_SIZES_PARALLEL(DTCO, UG, U, KPROC,KSIZE,KPROCMIN,ISIZE_TASK,OSHADOWS)
! returns the breakdown of domain lines according to the number of MPI threads in ISIZE_TASK
!
IPROC1 = KPROCMIN
! should be called kprocmax because is allocated to the rank of the highest mpi thread but
! KPROCMIN is defined in upper scope as the thread rank with the minimum number of points
!
CPT   = 0  ! weird iterator used oddly below
!
! Next steps: assignment of points to processors
! fill the KINDEX table with threads ranks
! domain is filled by bands starting with the highest rank (KPROC-1) at the lower left corner of the domain
! additional points are distributed top to bottom (adding a line to 0 first)
! Exemple for Rank 0 to N-1:
!
! ^    |-------------0--------------|
! |    |-------------0--------------|
! |    |-------------1--------------|
...
!
!     0------------Xaxis-------------->
DO JJ=1,SIZE(KINDEX) ! loop on domain points
  !
  IF (KINDEX(JJ)==-1) THEN ! check if the point already assigned (assigned values = [0,N-1]) (array is initialized to -1)
    CPT = CPT + 1
    DO WHILE( CPT>ISIZE_TASK(IPROC1) ) ! check maximum number of points for current thread rank
      IF ( IPROC1.GE.KPROCMIN ) THEN ! we go from IPROCMIN to IPROC-1
        IPROC1 = IPROC1+1
        IF ( IPROC1==KPROC ) IPROC1 = KPROCMIN-1 ! once we're at IPROC-1
      ELSE                                       ! back down under IPROC1
        IPROC1 = IPROC1-1
      ENDIF
      CPT = 1
    ENDDO
    KINDEX(JJ) = IPROC1 ! array with the associated rank for each point JJ of domain
  ENDIF
  !
ENDDO
!
