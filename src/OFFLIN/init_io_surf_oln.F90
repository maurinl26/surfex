...
!
USE MODN_IO_OFFLINE,    ONLY : XTSTEP_OUTPUT, LFIX_OUTPUT
!
USE MODI_GET_LUOUT
USE MODI_READ_SURF
...
!
 CHARACTER(LEN=LEN_HREC), DIMENSION(1) :: YSELECT
REAL              :: ZDEN
...
IF (NRANK==NPIO) THEN
  ...
  IF (.NOT.LTIME_WRITTEN.AND..NOT.LDEF) THEN 
    IF (LFIX_OUTPUT) THEN
      CALL WRITE_SURF(YSELECT,HPROGRAM,'time',XTIMEC/ZDEN,IRESP,HCOMMENT=YCOMMENT)
    ELSE
