...
!!      modified 03/20 by M. Lafaysse *Meteo France*
!!      --> remove weird opening before creation or opening to do nothing just
!!          This crashes with netcdf4 format.
!!      --> remove closure of the file at this step (should stay opened for efficiency)
!!=================================================================
!
!*       0.   DECLARATIONS
!             ------------
...
!
 USE MODD_IO_SURF_NC, ONLY : CFILEOUT_NC, NID_NC,NID_NCOUT, LCREATED, CFILEOUT_NC_SAVE, LDEF
!
...
INTEGER          :: IDIM1
INTEGER          :: INPATCH, INLVLD, INLVLS, INBIOMASS, &
...
INTEGER          :: IRET, IFULL, ISNAP, INLATI, ISNOW_LAYER
  ...
  IF (.NOT.GEXIST .OR. .NOT.LCREATED .OR. CFILEOUT_NC/=CFILEOUT_NC_SAVE) THEN
    ...
    IF (CSOFTWARE=='PGD') THEN
      IF (LSPLIT_PATCH) THEN
        CALL OL_DEFINE_DIM(UG, U%NSIZE_FULL, 'NOTIME ', ILUOUT, IFULL,               &
                           IM%NPE%AL(1)%TSNOW, IDIM1, YUNIT1, YUNIT2, ZX, ZY, IDIMS, &
                           IDDIM, YNAME_DIM,.FALSE.,.FALSE.)
      ELSE
        CALL OL_DEFINE_DIM(UG, U%NSIZE_FULL, 'NOTIME ', ILUOUT, IFULL,               &
                           IM%NPE%AL(1)%TSNOW, IDIM1, YUNIT1, YUNIT2, ZX, ZY, IDIMS, &
                           IDDIM, YNAME_DIM,.FALSE.,.FALSE., KNPATCH= INPATCH)
      ENDIF
    ELSE
      IF (LSPLIT_PATCH) THEN
        CALL OL_DEFINE_DIM(UG, U%NSIZE_FULL, 'NOTIME ', ILUOUT, IFULL,               &
      ...
      ENDIF
    ENDIF
    CALL CREATE_FILE(CFILEOUT_NC,IDIMS,YNAME_DIM,NID_NCOUT,IDDIM)
    NID_NC = NID_NCOUT
    ...
    LCREATED = .TRUE.
    IF (CSOFTWARE=='PGD') THEN
      IRET = NF90_DEF_DIM(NID_NC,"N_month",12,IDIMID)
      IRET = NF90_DEF_DIM(NID_NC,"N_day_of_week",7,IDIMID)
      IRET = NF90_DEF_DIM(NID_NC,"N_hour",24,IDIMID)
      IRET = NF90_DEF_DIM(NID_NC,"Nb_of_input_data",NVEGTYPE*IM%DTV%NTIME,IDIMID)
    ENDIF
    IF (CSOFTWARE=='PGD' .OR. ( CSOFTWARE=='OFFLINE' .AND. LRESTART ) ) THEN
      ...
      IF (GDATA_BLDTYPE .OR. GDATA_IND_BLD_AGE .OR. GDATA_COL_BLD_AGE .OR. GDATA_USETYPE) THEN
        ...
        !
        IRET = NF90_DEF_DIM(NID_NC,"Bld_dimensions  ",14,IDIMID)
        IRET = NF90_DEF_DIM(NID_NC,"Bld_parameters  ",ITOT,IDIMID)
        IRET = NF90_DEF_DIM(NID_NC,"Bld_positions  ",35,IDIMID)
      ENDIF
    ...
    ENDIF
  ! Comment M Lafaysse : remove the closure of this file here before any writing (not efficient).
