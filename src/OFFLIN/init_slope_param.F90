...
IF (NPROC>NIY) THEN
  PRINT*,"WHEN SHADOWS ARE ACTIVATED, THE NUMBER OF MPI THREADS CAN NOT BE &
          GREATER THAN THE NUMBER OF LINES IN THE SIMULATION GRID"
  CALL ABOR1_SFX("WHEN SHADOWS ARE ACTIVATED, THE NUMBER OF MPI THREADS CAN &
               NOT BE GREATER THAN THE NUMBER OF LINES IN THE SIMULATION GRID")
ENDIF
!
IF (NPROC>1) THEN
