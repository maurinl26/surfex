...
SELECT CASE (HUNITS(1:4))
  ...
  CASE DEFAULT
    ...
    PRINT*,TRIM(HUNITS)
    CALL ABOR1_SFX("Error units time")
END SELECT
...
IF (KNHOURS>=0) THEN
  ...
  ELSE
    ...
    ! when time+knhours is positive, MOD or MODULO are equivalents.
    TPTIME%TIME=MODULO(INT(TPTIME%TIME)+KNHOURS,24)+TPTIME%TIME-INT(TPTIME%TIME) ! new hour (+minutes)
    !Increase days
    DO
  ...
  ENDIF
ELSE
  ! Decrease hour (add negative value KNHOURS)
  IF ((INT(TPTIME%TIME)+KNHOURS)>=0) THEN
    TPTIME%TIME=TPTIME%TIME+KNHOURS
  ELSE
    IREMAININGDAYS=(INT(TPTIME%TIME)-KNHOURS)/24+1
    ! when time+knhours is negative, modulo must be used:
    ! MODULO(A, P) = A - (FLOOR(A/P) * P) while MOD(A, P) = A - (INT(A/P) * P)
    ! MODULO(0-18, 24) = 6 while MOD(0-18,24)=-18
    TPTIME%TIME=MODULO(INT(TPTIME%TIME)+KNHOURS,24)+TPTIME%TIME-INT(TPTIME%TIME) ! new hour
    ! decrease days
    DO
