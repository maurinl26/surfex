...
END SUBROUTINE READ_SURFX2_NC
      SUBROUTINE READ_SURFX3_NC(HREC,PFIELD,KRESP,HCOMMENT,HDIR)
 CHARACTER(LEN=*),        INTENT(IN)  :: HREC     ! name of the article to be read
REAL, DIMENSION(:,:,:),     INTENT(OUT) :: PFIELD   ! array containing the data field
INTEGER,                  INTENT(OUT) :: KRESP    ! KRESP  : return-code if a problem appears
 CHARACTER(LEN=100),       INTENT(OUT) :: HCOMMENT ! comment
 CHARACTER(LEN=1),         INTENT(IN)  :: HDIR     ! type of field :
END SUBROUTINE READ_SURFX3_NC
...
!
USE MODD_IO_SURF_NC, ONLY : NID_NC,CFILE_NC
!
...
IF (NID_NC.NE.0) THEN
  ...
  IF (IRET(1).NE.NF90_NOERR) CALL ERROR_READ_SURF_NC(HREC,3) 
  IRET(1)=NF90_INQUIRE_VARIABLE(NID_NC,IVAR_ID,XTYPE=ITYPE,NDIMS=INDIMS)
  !  
  ! 2. Get variable
  !----------------------------
  ...
  ELSEIF (ITYPE==NF90_FLOAT) THEN
    ...
    PFIELD = ZFIELD
  ELSE
     CALL ERROR_READ_SURF_NC(HREC,10)
  ENDIF
  !
  IRET(3) = NF90_GET_ATT(NID_NC,IVAR_ID,"comment",HCOMMENT)
  !
ELSE
  CALL ERROR_READ_SURF_NC(CFILE_NC,2)
ENDIF
!
! 3. Check for errors
!--------------------
...
IF ((PFIELD==XUNDEF).OR.IRET(2).NE.NF90_NOERR) THEN 
  CALL ERROR_READ_SURF_NC(HREC,1)
ENDIF
!
...
!
USE MODD_IO_SURF_NC, ONLY: LMASK,NMASK,NID_NC,CFILE_NC
!
USE MODD_SURF_PAR,   ONLY: XUNDEF
!
...
IF (NRANK==NPIO) THEN
  ...
  IF (NID_NC.NE.0) THEN
    ...
    IRET(1)=NF90_INQ_VARID   (NID_NC,HREC,IVAR_ID)
    IF (IRET(1).NE.NF90_NOERR) CALL ERROR_READ_SURF_NC(HREC,3) 
    IRET(2)=NF90_INQUIRE_VARIABLE(NID_NC,IVAR_ID,XTYPE=ITYPE,NDIMS=INDIMS)
    !
    IF ( INDIMS>0 ) THEN
      !
    ...
    ISTART(:) = 1
    IF (ITYPE==NF90_DOUBLE) THEN
      IRET(2)=NF90_GET_VAR(NID_NC,IVAR_ID,ZWORK,ISTART,ICOUNT)
    ELSEIF (ITYPE==NF90_FLOAT) THEN
...
IF (NRANK==NPIO) THEN
  ...
  IF (NID_NC.NE.0) THEN
    ...
    IRET(2)=NF90_INQUIRE_VARIABLE(NID_NC,IVAR_ID,XTYPE=ITYPE,NDIMS=INDIMS)
    !
    IRET(3)=NF90_INQUIRE_VARIABLE(NID_NC,IVAR_ID,DIMIDS=IDIMIDS(1:INDIMS))
    IDIMLEN(:) = 1.
    DO JDIM=1,INDIMS
      JRET=NF90_INQUIRE_DIMENSION(NID_NC,IDIMIDS(JDIM),LEN=IDIMLEN(JDIM))
    ENDDO
...
!     #############################################################
!
!!****  *READX3* - routine to fill a real3D array for the externalised surface 
!
! B. Cluzet 2018
USE MODD_SURFEX_MPI, ONLY: NRANK, NPROC, NCOMM, NPIO, XTIME_NPIO_READ, XTIME_COMM_READ, NREQ
...
INTEGER            :: IL1, IL2, IL3
INTEGER            :: IVAR_ID,JRET,JDIM,INDIMS,ITYPE, INFOMPI
INTEGER,DIMENSION(5) :: IDIMIDS,IDIMLEN
INTEGER, DIMENSION(4) :: ISTART, ICOUNT ! BC or 3 ?
...
IF (NRANK==NPIO) THEN
  ...
  IF (NID_NC.NE.0) THEN
    ...
    IF (HDIR/='-') THEN
      IF (TRIM(YNAME).NE.'Number_of_points') THEN
        ALLOCATE(ZWORK3(IDIMLEN(1)*IDIMLEN(2),IDIMLEN(3), IDIMLEN(4)))
      ELSE
    ...
    ICOUNT(:) = IDIMLEN(1:4)    
    IF (ITYPE==NF90_DOUBLE) THEN
      IRET(2)=NF90_GET_VAR(NID_NC,IVAR_ID,ZWORK3,ISTART,ICOUNT)
    ELSEIF (ITYPE==NF90_FLOAT) THEN
      ALLOCATE(ZTAB_3D4(SIZE(ZWORK3,1),SIZE(ZWORK3,2), SIZE(ZWORK3,3)))
      IRET(2)=NF90_GET_VAR(NID_NC,IVAR_ID,ZTAB_3D4,ISTART,ICOUNT)
      ZWORK3(:,:,:) = ZTAB_3D4(:,:,:)
      DEALLOCATE(ZTAB_3D4)
    ELSE
       CALL ERROR_READ_SURF_NC(HREC,10)
  ...
  !
ELSEIF (HDIR/='-') THEN
  ALLOCATE(ZWORK3(1,SIZE(PFIELD,2), SIZE(PFIELD,3)))
ENDIF
!
#ifdef SFX_MPI
XTIME_NPIO_READ = XTIME_NPIO_READ + (MPI_WTIME() - XTIME0)
...
IF (HDIR=='E') THEN
  IF ( NRANK==NPIO ) THEN
    CALL PACK_SAME_RANK(NMASK,ZWORK3(:,:,:),PFIELD(:,:,:))
  ENDIF
ELSEIF (HDIR=='A') THEN  ! no distribution on other tasks
  IF ( NRANK==NPIO ) THEN
#ifdef SFX_MPI
    XTIME0 = MPI_WTIME()
#endif
    PFIELD(:,:,:) = ZWORK3(:,1:SIZE(PFIELD,2), 1:SIZE(PFIELD,3))
#ifdef SFX_MPI
    XTIME_COMM_READ = XTIME_COMM_READ + (MPI_WTIME() - XTIME0)
#endif
  ENDIF
ELSEIF (HDIR=='-') THEN ! distribution of the total field on other tasks
  ...
  IF (NPROC>1) THEN
    XTIME0 = MPI_WTIME()
    CALL MPI_BCAST(ZWORK3,SIZE(ZWORK3)*KIND(ZWORK3)/4,MPI_REAL,NPIO,NCOMM,INFOMPI)   
    XTIME_COMM_READ = XTIME_COMM_READ + (MPI_WTIME() - XTIME0)
  ENDIF
#endif
  PFIELD(:,:,:) = ZWORK3(:,1:SIZE(PFIELD,2), 1:SIZE(PFIELD,3))
ELSE
  IF (LMASK) THEN
    CALL READ_AND_SEND_MPI(ZWORK3(:,1:SIZE(PFIELD,2),:),PFIELD,NMASK)
  ELSE 
    CALL READ_AND_SEND_MPI(ZWORK3(:,1:SIZE(PFIELD,2),:),PFIELD)
  END IF
  !IF (NRANK==NPIO) THEN
  !  CALL MPI_WAITALL(NPROC-1,NREQ,ISTATUS,INFOMPI)
  !ENDIF    
...
DEALLOCATE(ZWORK3)
!
IF (LHOOK) CALL DR_HOOK('MODE_READ_SURF_NC:READ_SURFX3_NC',1,ZHOOK_HANDLE)
!
END SUBROUTINE READ_SURFX3_NC
...
IF ((KFIELD==NUNDEF).OR.IRET(2).NE.NF90_NOERR) THEN 
  KFIELD=NUNDEF
  CALL ERROR_READ_SURF_NC(HREC,1)
ENDIF
...
IF (NRANK==NPIO) THEN
  ...
  IF (NID_NC.NE.0) THEN
    ...
    IF (ITYPE==NF90_INT) THEN
      IRET(2)=NF90_GET_VAR(NID_NC,IVAR_ID,IWORK)
    ELSE
      CALL ERROR_READ_SURF_NC(HREC,10)
    ENDIF
    !  
...
IF (IRET(2).NE.NF90_NOERR) THEN 
  CALL ERROR_READ_SURF_NC(HREC,KRESP)
ENDIF
!
...
IF (NRANK==NPIO) THEN
  ...
  IF (NID_NC.NE.0) THEN
    ...
    IRET(1)=NF90_INQ_VARID   (NID_NC,HREC,IVAR_ID)
    IF (IRET(1).NE.NF90_NOERR) CALL ERROR_READ_SURF_NC(HREC,3) 
    IRET(2)=NF90_INQUIRE_VARIABLE(NID_NC,IVAR_ID,NDIMS=INDIMS)
    IRET(3)=NF90_INQUIRE_VARIABLE(NID_NC,IVAR_ID,DIMIDS=IDIMIDS)
    DO JDIM=1,INDIMS
      JRET=NF90_INQUIRE_DIMENSION(NID_NC,IDIMIDS(JDIM),LEN=IDIMLEN(JDIM))
    ENDDO
    IF ( INDIMS>0 ) THEN
    ...
    !----------------------------
    ISTART(1) = 1
    IRET(2)=NF90_GET_VAR(NID_NC,IVAR_ID,YTAB_1D,ISTART,ICOUNT)
    !
...
! 0. find filename
  ...
  IF (NID_NC.NE.0) THEN
    ...
    IF (JRET.NE.NF90_NOERR) CALL ERROR_READ_SURF_NC(YRECFM,3) 
    !
    ! 2. Get variable
    !----------------------------
    IF (JWRK == 1) THEN 
...
!--------------------
DO JRET=1,4
  IF (IRET(JRET).NE.NF90_NOERR) THEN 
    KRESP=1
  ENDIF
ENDDO
