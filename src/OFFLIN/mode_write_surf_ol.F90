...
ELSE
  ...
  ELSE
    ...
    INDIMS = 0
    IDIMIDS(:) = 0
    ! IDIMLEN = dimensions of variable in output file. In the case of non-IO processors,
    ! the associated variable (ZTAB3D) is never used but must be allocated to a minimal value for the call to GATHER_AND_WRITE_MPI
    IDIMLEN(:) = 1
...
IMPLICIT NONE
!
INTEGER, INTENT(IN) :: KDIM1 ! Dimensions of variable to write in output file (full domain) only used by IO processor
INTEGER, INTENT(IN) :: KDIM2
INTEGER, INTENT(IN) :: KDIM3
INTEGER, INTENT(IN) :: KNDIMS
!
REAL, DIMENSION(KDIM1,KDIM2,KDIM3) :: ZTAB3D    ! variable to write in output file (full domain) only used by IO processor
REAL, DIMENSION(KDIM1,SIZE(PFIELD,2),SIZE(PFIELD,3)) :: ZWORK_IGN ! same as ZTAB3D but with 1D spatial dimension
INTEGER :: INPOINTS1D ! 1D full spatial size only required for 2D spatial outputs (IGN), should be 1 for non-IO threads
INTEGER,         DIMENSION(KNDIMS) :: ISTART
INTEGER,         DIMENSION(KNDIMS) :: ISTRIDE
INTEGER,         DIMENSION(KNDIMS) :: ICOUNT
REAL(KIND=JPRB) :: ZHOOK_HANDLE
...
ZTAB3D(:,:,:) = XUNDEF
!
! PFIELD is the variable computed by each MPI thread (first dimension = spatial dimension of this thread).
! ZTAB3D is the variable to write in the output file (first dimension = spatial dimension over the whole domain),
! only available for the IO thread after GATHER_AND_WRITE_MPI
!
IF(.NOT.ALLOCATED(NMASK_IGN))THEN
  IF (LMASK) THEN
    CALL GATHER_AND_WRITE_MPI(PFIELD,ZTAB3D(:,1:KDIM2,1:KDIM3),NMASK)
  ELSE
    CALL GATHER_AND_WRITE_MPI(PFIELD,ZTAB3D(:,1:KDIM2,1:KDIM3)) 
  ENDIF
ELSE
  IF (NRANK==NPIO) THEN
    INPOINTS1D = SIZE(NMASK_IGN)
  ELSE
    INPOINTS1D = 1
  ENDIF  
  !ign grid 
  IF (LMASK) THEN
    CALL GATHER_AND_WRITE_MPI(PFIELD,ZWORK_IGN(1:INPOINTS1D,:,:),NMASK)
  ELSE
    CALL GATHER_AND_WRITE_MPI(PFIELD,ZWORK_IGN(1:INPOINTS1D,:,:))
  ENDIF
  IF (NRANK==NPIO) THEN
    CALL UNPACK_SAME_RANK(NMASK_IGN,ZWORK_IGN(1:INPOINTS1D,:,:),ZTAB3D(:,1:SIZE(PFIELD,2),1:SIZE(PFIELD,3)))
  ENDIF  
ENDIF
!
IF (NRANK==NPIO) THEN
