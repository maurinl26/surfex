...
!!      M. Dumont 12/2016 spectral calculation for Crocus LSPECSNOW
...
!                                                     ! 'NETCDF', 'BINARY', or 'ASCII '
INTEGER:: NCOMPRESS_NETCDF = 1                        ! level of compression for netcdf output files
INTEGER:: NOUTPUT_NETCDF = 3                          ! type of netcdf output files:
                                                      ! 3 -> 64BIT_OFFSET
                                                      ! 4 -> IOR(NF90_NETCDF4, NF90_CLASSIC_MODEL)
!
!*    Names of files
!     --------------
!
...
!
LOGICAL     :: LFIX_OUTPUT = .FALSE.      ! B. Cluzet :  logical to force diags output each day at hours
                                          ! that are multiple from the TSTEP_OUTPUT (0:6:12:18) (for ex.)
                                          ! even for a simulation starting at an hour not multiple of TSTEP_OUTPUT
!
!
...
NAMELIST/NAM_IO_OFFLINE/CSURF_FILETYPE, CTIMESERIES_FILETYPE, CFORCING_FILETYPE, &
                        ...
                        LALLOW_ADD_DIM, LDELAYEDSTART_NC, NDATESTOP, LSPECSNOW,  &
                        LFORCIMP, NIMPUROF, LFORCATMOTARTES, LFIX_OUTPUT,        &
                        NCOMPRESS_NETCDF, NOUTPUT_NETCDF, LGRID_MODE,            &
