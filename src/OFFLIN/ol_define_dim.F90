...
SUBROUTINE OL_DEFINE_DIM (UG, KSIZE_FULL, HPROGRAM, KLUOUT, KNI, &
                          ...
                          KDDIM, HNAME_DIM, OSNOWBANDS, OSPECMOD, &
                          KNPATCH, KNSNLAYER, PLAT, PLON)
!     #######################################################
!!****  *OL_DEFINE_DIM* - 
!!
!!    PURPOSE
...
USE MODD_CONST_TARTES, ONLY : NPNBANDS_MODIS  ! B. Cluzet number of modis bands
!
USE NETCDF
!
IMPLICIT NONE
...
LOGICAL, INTENT(IN)              :: OSPECMOD  ! logical for modis spectral bands output
...
IF (OSNOWBANDS) THEN
  INDIMS=INDIMS+1
  IF (OSPECMOD)   INDIMS=INDIMS+1 ! B. Cluzet
ENDIF
!
...
IF ( OSNOWBANDS) THEN
  ...
  IF (INPATCH/=0) ID = 2
  IF (OSPECMOD) THEN 
    KDIMS     (INDIMS-ID) = NPNBANDS_MODIS
    HNAME_DIM (INDIMS-ID) = 'MODIS_bands'
    ID = 3
  ENDIF
