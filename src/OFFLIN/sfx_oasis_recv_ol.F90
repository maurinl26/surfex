...
REAL, DIMENSION(KI) :: ZSEA_FCO2     ! Sea carbon flux      (molC/m2/s)
...
IF(GRECV_LAND)THEN
  CALL PUT_SFX_LAND(IM%O, IM%S, IM%K, IM%NK, IM%NP, U, ILUOUT,LCPL_GW,LCPL_FLOOD, ZLAND_WTD(:),&
                    ZLAND_FWTD(:), ZLAND_FFLOOD(:),ZLAND_PIFLOOD(:),ZLAND_TWS(:)  )        
ENDIF
!
!-------------------------------------------------------------------------------
...
IF(GRECV_SEA)THEN
  CALL PUT_SFX_SEA(S, U, W, ILUOUT,LCPL_SEAICE,LWATER,LCPL_SEACARB,LSEAICE_2FLX, &
                   ZSEA_SST(:),ZSEA_UCU(:),ZSEA_VCU(:),ZSEA_FCO2(:),             &
                   ZSEAICE_SIT(:), ZSEAICE_CVR(:),ZSEAICE_ALB(:)                 )
ENDIF
!
