...
!USE MODD_ISBA_OPTIONS_n,    ONLY : LSNOWMAK_BOOL
!
!
  ...
  IF (TPSNOW%SCHEME/='D95' .AND. TPSNOW%SCHEME/='EBA') THEN
    ...
    IF (TPSNOW%SCHEME/='1-L') THEN
      ...
      IF(TPSNOW%SCHEME=='CRO') THEN
        !
        ALLOCATE(TPSNOW%DIAMOPT(KLU,TPSNOW%NLAYER))
        ALLOCATE(TPSNOW%SPHERI(KLU,TPSNOW%NLAYER))  
        ALLOCATE(TPSNOW%HIST (KLU,TPSNOW%NLAYER))  
        ALLOCATE(TPSNOW%MMP        (KLU))  
        ...
        TPSNOW%DIAMOPT = XUNDEF
        TPSNOW%SPHERI = XUNDEF  
        TPSNOW%HIST  = XUNDEF  
        TPSNOW%MMP  = 0.
...
IF (TPSNOW%SCHEME/='CRO') THEN
  !
  ALLOCATE(TPSNOW%DIAMOPT(0,0))
  ALLOCATE(TPSNOW%SPHERI(0,0))  
  ALLOCATE(TPSNOW%HIST (0,0))  
  ALLOCATE(TPSNOW%MMP        (0)  )  
