...
!                               A cette date, les routines AV_PGD_2D et AV_PATCH_PGD ne sont plus utilisées. (#notused?)
...
INTERFACE AV_PGD
!
      SUBROUTINE AV_PGD_2D (DTCO, &                                                      ! #notused? (Not test with irrigation)
...
!
      SUBROUTINE AV_PATCH_PGD (DTCO, &                                                                        !  #notused? (Not test with irrigation)
...
!!$OMP DO SCHEDULE(DYNAMIC,1) PRIVATE(JI,ID0)
  DO JI = 1,SIZE(PCOVER,1) 
    ...
    IF ( HATYPE=='MA1' .AND. PDATA(IMASK(ID0))==0. ) THEN
      ZWEIGHT_TMP(:)=ZWEIGHT(:)
      ZWEIGHT_TMP(ID0)=0.
...
!
IF ( (PRESENT(NPAR_VEG_IRR_USE) .AND. .NOT.PRESENT(PIRRIG)) ) &
        CALL ABOR1_SFX('AV_PATCH_PGD_1D: IF THERE IS NPAR_VEG_IRR_USE, PIRRIG IS NEEDED')
!
...
ENDIF
!
ALLOCATE(PATCH_LIST(NVEGTYPE+IVEG_ADD))
DO JVEG=1,NVEGTYPE+IVEG_ADD
  IF ( IVEG_ADD == 0 ) THEN
...
ENDIF
!
ALLOCATE(ZCOVER_WEIGHT(SIZE(PCOVER,1),SIZE(PCOVER,2),NVEGTYPE+IVEG_ADD))
!
IF (PRESENT(PDZ)) THEN
  DO JP=1,IPATCH
    ZDZ(:,:,JP)=PDZ(:,:)
...
! Check if irrigation (and ECOSG) AND not in case of patching, else we don't have to not modified !
IF ( (PRESENT(NPAR_VEG_IRR_USE) .AND. .NOT.PRESENT(PIRRIG)) ) &
        CALL ABOR1_SFX('MAJOR_PATCH_PGD_1D: IF THERE IS NPAR_VEG_IRR_USE, PIRRIG IS NEEDED')
!
IF ( NVEG_IRR /= 0 .AND. PRESENT(NPAR_VEG_IRR_USE) ) THEN
  IVEG_ADD = NVEG_IRR
ELSE
  ...
  DO JP=1,IPATCH
    ...
    DO JVEG=1,NVEGTYPE+IVEG_ADD
      ...
      IF(JP == JJ) THEN
        ...
        !
        ZLIRRIG = .FALSE.
        IF ( PRESENT(NPAR_VEG_IRR_USE) .AND. IVEG_ADD /= 0 ) ZLIRRIG = ANY(NPAR_VEG_IRR_USE(:)==JVEG)
        !
