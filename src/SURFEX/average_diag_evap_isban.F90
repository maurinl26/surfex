...
!#############################
SUBROUTINE AVERAGE_DIAG_EVAP_ISBA_n (IO, DGO, DE, DEC, NDE, NDEC, NP, NPE,&
                                     PTSTEP, PRAIN, PSNOW, PSLOPECOS      )
!#############################
!
!
...
IF (DE%LSURF_EVAP_BUDGET) THEN
  ...
  IF(DE%LWATER_BUDGET)THEN
    ...
    IF (PEK%TSNOW%SCHEME=='CRO') THEN
      DE%XRAINFALL(:) = PRAIN(:) * ZCORR_SLOPE(:)
      DE%XSNOWFALL(:) = PSNOW(:) * ZCORR_SLOPE(:)
    ELSE
      DE%XRAINFALL(:) = PRAIN(:)
      DE%XSNOWFALL(:) = PSNOW(:)
    ENDIF
...
IF (DGO%LSURF_BUDGETC) THEN
  ...
  IF (DE%LWATER_BUDGET) THEN
    ...
    !
    DEC%XRAINFALL  (:) = DEC%XRAINFALL (:) + DE%XRAINFALL(:) * PTSTEP
    DEC%XSNOWFALL  (:) = DEC%XSNOWFALL (:) + DE%XSNOWFALL(:) * PTSTEP
    !
  ENDIF
  !
  IF (DE%LENERGY_BUDGET) THEN
...
DO JP=1,INP
  ...
  DO JI=1,PK%NSIZE_P
    ...
    DEA%XGNDF(IMASK) = DEA%XGNDF(IMASK) + PK%XPATCH(JI) * DEAK%XGNDF(JI)
    !
    ! Refreezing of water in the entire snowpack
    DEA%XSNFREE_SWU(IMASK) = DEA%XSNFREE_SWU(IMASK) + PK%XPATCH(JI) * DEAK%XSNFREE_SWU(JI)
    !
    ! TO BE CHECKED, OFTEN WITH MEB
    ...
    IF ( IO%LSNOWSYTRON ) THEN
      DEA%XSYTMASS(IMASK) = DEA%XSYTMASS(IMASK) + PK%XPATCH(JI) * DEAK%XSYTMASS(JI)
    END IF
    !
    ! Snowpappus
    IF ( IO%LSNOWPAPPUS ) THEN
      DEA%XQDEP_TOT      (IMASK) = DEA%XQDEP_TOT      (IMASK) + PK%XPATCH(JI) * DEAK%XQDEP_TOT      (JI)
      DEA%XQ_OUT_SUBL    (IMASK) = DEA%XQ_OUT_SUBL    (IMASK) + PK%XPATCH(JI) * DEAK%XQ_OUT_SUBL    (JI)
      DEA%XQT_TOT        (IMASK) = DEA%XQT_TOT        (IMASK) + PK%XPATCH(JI) * DEAK%XQT_TOT        (JI)
      DEA%XSNOWDEBTC     (IMASK) = DEA%XSNOWDEBTC     (IMASK) + PK%XPATCH(JI) * DEAK%XSNOWDEBTC     (JI)
      DEA%XBLOWSNWFLUXINT(IMASK) = DEA%XBLOWSNWFLUXINT(IMASK) + PK%XPATCH(JI) * DEAK%XBLOWSNWFLUXINT(JI)
      DEA%XBLOWSNWFLUX_1M(IMASK) = DEA%XBLOWSNWFLUX_1M(IMASK) + PK%XPATCH(JI) * DEAK%XBLOWSNWFLUX_1M(JI)
    END IF
    !
  END DO
  !
...
IF (ISIZE_LMEB_PATCH>0) THEN
  ...
  DO JP=1,INP
     ...
     DO JI=1,PK%NSIZE_P
        ...
        DEA%XUNLOAD   (IMASK) = DEA%XUNLOAD   (IMASK) + PK%XPATCH(JI) * DEAK%XUNLOAD   (JI)
