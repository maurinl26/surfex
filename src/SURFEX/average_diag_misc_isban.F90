...
!     #############################
      SUBROUTINE AVERAGE_DIAG_MISC_ISBA_n (DM, NDM, IO, NK, NP, NPE, PTIMEC)
!     #############################
!
!
...
!!      B. Decharme  2023    Bug in soil turn-over diag with explicit multi-layer soil carbon
!-------------------------------------------------------------------------------
!
!*       0.     DECLARATIONS
!               ------------
...
REAL, OPTIONAL,            INTENT(IN)    :: PTIMEC  ! cumulated time since beginning
!
...
INTEGER                         :: JC    ! layer loop counter
INTEGER                         :: INI,INP,INL, INCARB
...
!
#ifdef SFX_OL
IF (PRESENT(PTIMEC))THEN
GCOMPUTEDIAG = (MOD(PTIMEC,XTSTEP_OUTPUT) == 0. &
               .AND.NPE%AL(1)%TSNOW%SCHEME=="CRO")
ELSE
GCOMPUTEDIAG = (NPE%AL(1)%TSNOW%SCHEME=="CRO")
ENDIF
#else
GCOMPUTEDIAG = (NPE%AL(1)%TSNOW%SCHEME=="CRO")
#endif
!
GPROSNOW = (DM%LPROSNOW .AND. GCOMPUTEDIAG)
...
DO JP=1,INP
   ...
   DO JI=1,PK%NSIZE_P
      ...
      IMASK = PK%NR_P(JI)
      !
      !liquid water retained on canopy
...
IF (GPROSNOW) THEN
   ...
   IF (DM%LPROBANDS) THEN
      DM%XSPEC_ALB(:,:)=0.
      DM%XDIFF_RATIO(:,:)=0.
      IF (DM%LSPECMOD) THEN ! B. Cluzet case for snow spectral bands assim
        DM%XSPECMOD(:,:) = 0.
      ENDIF
   ENDIF 
   !
   DO JP=1,INP
      ...
      DO JI=1,PK%NSIZE_P
         ...
         IMASK = PK%NR_P(JI)
         !
         DM%XSNDPT_12H     (IMASK) = DM%XSNDPT_12H     (IMASK) + PK%XPATCH(JI) * DMK%XSNDPT_12H     (JI)
   ...
   DO JP=1,INP
     ...
     DO JI=1,PK%NSIZE_P
       IMASK = PK%NR_P(JI)
       !
       DM%XJJ       (IMASK) = DM%XJJ       (IMASK) + PK%XPATCH(JI) * DMK%XJJ       (JI)
       DM%XNRANK    (IMASK) = DM%XNRANK    (IMASK) + PK%XPATCH(JI) * DMK%XNRANK    (JI)
       DM%XILOCNIY  (IMASK) = DM%XILOCNIY  (IMASK) + PK%XPATCH(JI) * DMK%XILOCNIY  (JI)
       DM%XSIZE_TASK(IMASK) = DM%XSIZE_TASK(IMASK) + PK%XPATCH(JI) * DMK%XSIZE_TASK(JI)
       !
     ENDDO
     !
   ENDDO
   ...
   IF ( IO%LSNOWMAK_BOOL ) THEN
     ...
     DO JP=1,INP
       ...
       DO JI=1,PK%NSIZE_P
         ...
         DM%XWBT(IMASK) = DM%XWBT(IMASK) + PK%XPATCH(JI) * DMK%XWBT(JI)
         !
       ENDDO
     ENDDO
   ENDIF
   IF ( IO%LSNOWPAPPUS ) THEN
     ...
     DO JP=1,INP
       ...
       DO JI=1,PK%NSIZE_P
         IMASK = PK%NR_P(JI)
         !
         !     Snow transport scheme Pappus total outputs
         DM%XQ_OUT_SALT    (IMASK) = DM%XQ_OUT_SALT    (IMASK) + PK%XPATCH(JI) * DMK%XQ_OUT_SALT    (JI)
         DM%XQ_OUT_SUSP    (IMASK) = DM%XQ_OUT_SUSP    (IMASK) + PK%XPATCH(JI) * DMK%XQ_OUT_SUSP    (JI)
         DM%XVFRIC_PAPPUS  (IMASK) = DM%XVFRIC_PAPPUS  (IMASK) + PK%XPATCH(JI) * DMK%XVFRIC_PAPPUS  (JI)
         DM%XVFRIC_T_PAPPUS(IMASK) = DM%XVFRIC_T_PAPPUS(IMASK) + PK%XPATCH(JI) * DMK%XVFRIC_T_PAPPUS(JI)
         DM%XPZ0_PAPPUS    (IMASK) = DM%XPZ0_PAPPUS    (IMASK) + PK%XPATCH(JI) * DMK%XPZ0_PAPPUS    (JI)
         DM%XVFALL_PAPPUS  (IMASK) = DM%XVFALL_PAPPUS  (IMASK) + PK%XPATCH(JI) * DMK%XVFALL_PAPPUS  (JI)
         !
       ENDDO
       !
     ENDDO
...
IF(IO%CRESPSL=='CNT')THEN
  ...
  DO JP=1,INP
     ...
     DO JI=1,PK%NSIZE_P
        IMASK = PK%NR_P(JI) 
        DO JC=1,INCARB
           ZCSOILTOT    (IMASK,JC) = ZCSOILTOT    (IMASK,JC) + PK%XPATCH(JI) * PEK%XSOILCARB(JI,JC)
           DM%XTSOILPOOL(IMASK,JC) = DM%XTSOILPOOL(IMASK,JC) + PK%XPATCH(JI) * PEK%XSOILCARB(JI,JC) * DMK%XTSOILPOOL(JI,JC)
        ENDDO
      ENDDO
  ENDDO
  !
  ...
  DO JP=1,INP
     ...
     DO JL=1,INL
        DO JI=1,PK%NSIZE_P
           ...
           IF(JL<=IDEPTH.AND.IDEPTH/=NUNDEF)THEN
             DO JC=1,INCARB
                ZCSOILTOT    (IMASK,JC) = ZCSOILTOT    (IMASK,JC)+PK%XPATCH(JI)*PEK%XSOILDIF_CARB(JI,JL,JC)
                DM%XTSOILPOOL(IMASK,JC) = DM%XTSOILPOOL(IMASK,JC)+PK%XPATCH(JI)*PEK%XSOILDIF_CARB(JI,JL,JC)*DMK%XTSOILPOOL(JI,JC)
             ENDDO
           ENDIF
        ENDDO
     ENDDO
