...
!!      Original    22/06/20
!!
!!      B. Decharme  2023    Closed-budget check in % error instead of in absolute error
!
!-------------------------------------------------------------------------------
!
!*       0.     DECLARATIONS
...
REAL, DIMENSION(SIZE(PK%XDZG,1))                                             :: ZBUD_LITTER   ! g/m2
REAL, DIMENSION(SIZE(PK%XDZG,1))                                             :: ZBUD_SOILCARB ! g/m2
!
...
!-------------------------------------------------------------------------------
!
ZBUD_LITTER  (:) = ZNEW_LITTER  (:)-ZOLD_LITTER  (:)
ZBUD_SOILCARB(:) = ZNEW_SOILCARB(:)-ZOLD_SOILCARB(:)
!
WHERE((ZOLD_SOILCARB(:)+ZOLD_LITTER(:))/=0.0)
      ZBUD(:) = 100.*(ZBUD_LITTER(:)+ZBUD_SOILCARB(:))/(ZOLD_SOILCARB(:)+ZOLD_LITTER(:))
ENDWHERE
!
DO JI=1,INI
   IF(ABS(ZBUD(JI))>ZBUDLIM)THEN
      ...
      WRITE(KLUOUT,*) 'Litter (g/m2) =',ZNEW_LITTER(JI),ZOLD_LITTER(JI),ZBUD_LITTER(JI)
      WRITE(KLUOUT,*) 'Soil   (g/m2) =',ZNEW_SOILCARB(JI),ZOLD_SOILCARB(JI),ZBUD_SOILCARB(JI)
      WRITE(KLUOUT,*) 'Bilan  (%)    =',ZBUD(JI)
      WRITE(KLUOUT,*) 'Bilan  (g/m2) =',ZBUD_LITTER(JI)+ZBUD_SOILCARB(JI)
      WRITE(KLUOUT,*) '!!!!!!!!!!!!!!!!!!!!'
