...
!              -------------------------------
!
TYPE(ISBA_S_t)           :: SOLD   ! Local pointer for landuse init
TYPE(ISBA_NP_t)          :: NPGLO  ! Local pointer for landuse init
TYPE(ISBA_NPE_t)         :: NPEGLO ! Local pointer for landuse init
!
...
IF ( U%LECOSG .AND. ( LIRRIGMODE .OR. LAGRIP ) ) THEN
  ...
  IF ( NVEG_IRR /= SIZE(DTI%NPAR_VEG_IRR_USE)) &
    CALL ABOR1_SFX('COMPUTE_ISBA_PARAMETER: WHEN LIRRIGMODE or LAGRIP+LECOSG, NVEG_IRR AND NPAR_VEG_IRR_USE HAVE TO BE EQUAL')
  !
...
!-------------------------------------------------------------------------------
!
!    PART 8: Reading of prognostic and semi-prognostic fields
!    --------------------------------------------------------
!
!
...
!
!
!* Allocate work array for land use case
!
IF(IO%LLULCC.AND.HINIT=='ALL')THEN
  CALL ALLOC_ISBA_LANDUSE(U, IO, DTI, S, SOLD, NPGLO, NPEGLO, HPROGRAM, KI)
ENDIF
!
!
!* Read ISBA fields
!
CALL READ_ISBA_n(DTCO, U, IO, S, K, NK, NP, NPE, NAG, NPGLO, NPEGLO, HPROGRAM, HINIT)
!
!
...
!
!
!* Land use land cover fields
!
IF(IO%LLULCC)THEN
  CALL INIT_ISBA_LANDUSE(IG, IO, S, K, NK, NP, NPE, SOLD, NPGLO, NPEGLO, HPROGRAM, KI)
ENDIF
!
!
!*  Canopy air fields:
