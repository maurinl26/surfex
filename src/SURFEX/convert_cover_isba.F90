...
!     #########
      SUBROUTINE CONVERT_COVER_ISBA (DTCO, HALBEDO, &
                                        HISBA,OTR_ML,KDECADE,PCOVER,OCOVER,HPHOTO, &
                                         HSFTYPE,OECOSG,NPAR_VEG_IRR_USE,    &
                                         ...
                                         TPSEED, TPREAP, PWATSUP,            & ! #notused --> never CALL
...
USE MODD_DATA_COVER,     ONLY : XDATA_LAI, XDATA_H_TREE,                  &
                                  ...
                                  TDATA_SEED, TDATA_REAP,                 & ! #notused --> never CALL
                                  XDATA_IRRIGTYPE, XDATA_IRRIGFRAC,       &
...
!
TYPE(DATE_TIME), DIMENSION(:,:),   OPTIONAL, INTENT(OUT)   :: TPSEED  ! #notused --> never CALL
TYPE(DATE_TIME), DIMENSION(:,:),   OPTIONAL, INTENT(OUT)   :: TPREAP  ! #notused --> never CALL
!
REAL,            DIMENSION(:,:),   OPTIONAL, INTENT(OUT)   :: PWATSUP ! #notused --> never CALL
REAL,            DIMENSION(:,:),   OPTIONAL, INTENT(OUT)   :: PHTRUNK
REAL,            DIMENSION(:,:),   OPTIONAL, INTENT(OUT)   :: PWCROWN
!
...
REAL, DIMENSION(:,:,:), ALLOCATABLE               :: ZDATA_VEG, ZDATA_LAI,        &
                                                     ZDATA_LAI_ALL_YEARS
...
IF (PRESENT(PLAI)) THEN
  CALL AV_PGD(YDTCO, PLAI, PCOVER, XDATA_LAI(:,KDECADE,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
ENDIF
!
!* 1/Rsmin
!
IF (PRESENT(PRSMIN)) THEN
  IF (SIZE(PRSMIN)>0) CALL AV_PGD(YDTCO, PRSMIN, PCOVER, XDATA_RSMIN, YLAI, 'INV', OCOVER, & 
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
END IF
!
IF (PRESENT(PH_TREE)) CALL AV_PGD(YDTCO, PH_TREE, PCOVER, XDATA_H_TREE(:,:), YTREE, 'ARI', OCOVER, &
                                       NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
!
DO JVEG=1,NVEGTYPE
...
!
IF (PRESENT(PGAMMA)) CALL AV_PGD(YDTCO, PGAMMA, PCOVER, XDATA_GAMMA(:,:), YVEG, 'ARI', OCOVER, & 
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
!
IF (PRESENT(PWRMAX_CF)) CALL AV_PGD(YDTCO, PWRMAX_CF, PCOVER, XDATA_WRMAX_CF(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
!
IF (PRESENT(PRGL)) CALL AV_PGD(YDTCO, PRGL, PCOVER, XDATA_RGL(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)        
IF (PRESENT(PCV)) CALL AV_PGD(YDTCO, PCV, PCOVER, XDATA_CV(:,:), YVEG, 'INV', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
!
IF (PRESENT(PH_VEG)) THEN
  CALL AV_PGD(YDTCO, PH_VEG, PCOVER,XDATA_H_VEG(:,KDECADE,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC) 
ENDIF
!
!---------------------------------------------------------------------------------
!
! urban trees characteristics
!
IF (PRESENT(PHTRUNK)) CALL AV_PGD(YDTCO, PHTRUNK ,PCOVER ,XDATA_HTRUNK_HVEG (:,:) ,YTREE,'ARI',OCOVER)  
!
IF (PRESENT(PWCROWN)) CALL AV_PGD(YDTCO, PWCROWN ,PCOVER ,XDATA_WCROWN_HVEG (:,:) ,YTREE,'ARI',OCOVER)  
!
!---------------------------------------------------------------------------------
!
!* soil layers
...
IF (PRESENT(PALBNIR_VEG)) THEN
  IF (HALBEDO=='CM13') THEN
    CALL AV_PGD(YDTCO, PALBVIS_VEG, PCOVER, XDATA_ALB_VEG_NIR(:,KDECADE,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)      
  ELSE   
    CALL AV_PGD(YDTCO, PALBNIR_VEG, PCOVER, XDATA_ALBNIR_VEG(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
  ENDIF
ENDIF
!
IF (PRESENT(PALBVIS_VEG)) THEN
  IF (HALBEDO=='CM13') THEN
    CALL AV_PGD(YDTCO, PALBVIS_VEG, PCOVER, XDATA_ALB_VEG_VIS(:,KDECADE,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)      
  ELSE     
    CALL AV_PGD(YDTCO, PALBVIS_VEG, PCOVER, XDATA_ALBVIS_VEG(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
  ENDIF
ENDIF
!
IF (PRESENT(PALBUV_VEG)) THEN
  ...
  ELSE
    CALL AV_PGD(YDTCO, PALBUV_VEG, PCOVER, XDATA_ALBUV_VEG(:,:), YVEG,'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
  ENDIF
ENDIF
!  
...
IF (HPHOTO /='NON' )  THEN
   ...
   IF (PRESENT(PGMES)) THEN
     IF (SIZE(PGMES)>0) CALL AV_PGD(YDTCO, PGMES, PCOVER, XDATA_GMES_ST(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC) 
   END IF

   IF (PRESENT(PBSLAI)) THEN
     IF (SIZE(PBSLAI)>0) CALL AV_PGD(YDTCO, PBSLAI, PCOVER, XDATA_BSLAI_ST(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
   END IF

   IF (PRESENT(PSEFOLD)) THEN
    IF (SIZE(PSEFOLD)>0) CALL AV_PGD(YDTCO, PSEFOLD, PCOVER, XDATA_SEFOLD_ST(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
   END IF

   IF (PRESENT(PGC)) THEN
     IF (SIZE(PGC)>0) CALL AV_PGD(YDTCO, PGC, PCOVER, XDATA_GC_ST(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
   END IF

   IF (PRESENT(PDMAX)) THEN
     IF (SIZE(PDMAX)>0) CALL AV_PGD(YDTCO, PDMAX, PCOVER, XDATA_DMAX_ST(:,:), YTREE, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC) 
   END IF

ELSE
   IF (PRESENT(PGMES)) THEN
     IF (SIZE(PGMES)>0) CALL AV_PGD(YDTCO, PGMES, PCOVER, XDATA_GMES(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
   END IF
   IF (PRESENT(PBSLAI)) THEN
     IF (SIZE(PBSLAI)>0) CALL AV_PGD(YDTCO, PBSLAI ,PCOVER, XDATA_BSLAI(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
   END IF
   IF (PRESENT(PSEFOLD)) THEN
     IF (SIZE(PSEFOLD)>0) CALL AV_PGD(YDTCO, PSEFOLD, PCOVER, XDATA_SEFOLD(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
   END IF
   IF (PRESENT(PGC)) THEN
     IF (SIZE(PGC)>0) CALL AV_PGD(YDTCO, PGC, PCOVER, XDATA_GC(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
   END IF
   IF (PRESENT(PDMAX)) THEN
     IF (SIZE(PDMAX)>0) CALL AV_PGD(YDTCO, PDMAX, PCOVER, XDATA_DMAX(:,:), YTREE, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
   END IF
...
IF (PRESENT(PRE25)) THEN
  IF (SIZE(PRE25)>0) CALL AV_PGD(YDTCO, PRE25, PCOVER, XDATA_RE25(:,:), YNAT, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
END IF

IF (PRESENT(PLAIMIN)) THEN
  IF (SIZE(PLAIMIN)>0) CALL AV_PGD(YDTCO, PLAIMIN, PCOVER, XDATA_LAIMIN(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
END IF
IF (PRESENT(PCE_NITRO)) THEN
  IF (SIZE(PCE_NITRO)>0) &
          CALL AV_PGD(YDTCO, PCE_NITRO, PCOVER, XDATA_CE_NITRO(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
END IF
IF (PRESENT(PCF_NITRO)) THEN
 IF (SIZE(PCF_NITRO)>0) CALL AV_PGD(YDTCO, PCF_NITRO, PCOVER, XDATA_CF_NITRO(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
END IF
IF (PRESENT(PCNA_NITRO)) THEN
  IF (SIZE(PCNA_NITRO)>0) CALL AV_PGD(YDTCO, PCNA_NITRO, PCOVER, XDATA_CNA_NITRO(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
END IF
IF (PRESENT(PF2I)) THEN
  IF (SIZE(PF2I)>0) CALL AV_PGD(YDTCO, PF2I, PCOVER, XDATA_F2I(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
END IF
!
IF (PRESENT(OSTRESS)) THEN
  IF (SIZE(OSTRESS)>0) THEN
    ...
    CALL AV_PGD(YDTCO, ZWORK, PCOVER, XDATA_STRESS(:,:), YVEG, 'ARI', OCOVER, & 
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
    WHERE (ZWORK<0.5) 
      OSTRESS = .FALSE.
    ELSEWHERE
...
END IF
!
IF (HPHOTO == 'NIT' .OR. HPHOTO=='NCB')  THEN ! #notused --> never CALL
   !
   ! date of seeding
   ! ---------------
   !
   IF (PRESENT(TPSEED)) THEN
     IF (SIZE(TPSEED)>0) &
       CALL AV_PGD (TPSEED, PCOVER, TDATA_SEED(:,:), YVEG, 'MAJ', OCOVER, & 
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
   END IF
   !
   ! date of reaping
   ...
   IF (PRESENT(TPREAP)) THEN
     IF (SIZE(TPREAP)>0) &
       CALL AV_PGD (TPREAP, PCOVER, TDATA_REAP(:,:), YVEG, 'MAJ', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)  
   END IF
   ...
   ! ---------------------------
   !
   IF (PRESENT(PWATSUP)) THEN ! #notused --> never CALL
     IF (SIZE(PWATSUP)>0) &
       CALL AV_PGD(YDTCO, PWATSUP, PCOVER, XDATA_WATSUP(:,:), YVEG, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
   END IF
!
END IF
...
IF(HISBA/='DIF')THEN
  ...
  !
  DO JLAYER=1,KGROUND
     CALL AV_PGD(YDTCO, PDG(:,JLAYER,:), PCOVER, ZDATA_DG(:,JLAYER,:), YNAT, CDGAVG, OCOVER, & 
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
  ENDDO
  !
ELSE
!
  ...
  ENDIF
!
  CALL AV_PGD(YDTCO, ZDTOT(:,:), PCOVER, ZDATA_GROUND_DEPTH, YNAT, CDGAVG, OCOVER, & 
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
!  
...
!  
  ...
  IF (LDROOT .OR. LROOTFRAC  .OR. (CDGDIF=='ROOT')) THEN
    CALL AV_PGD(YDTCO, ZDROOT(:,:), PCOVER, ZDATA_ROOT_DEPTH, YDIF, CDGAVG, OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
...
!
  IF (LROOTFRAC) THEN
!      
    CALL AV_PGD(YDTCO, ZROOT_LIN(:,:), PCOVER, ZDATA_ROOT_LIN(:,:), YDIF, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
    IF (LROOTFRAC) THEN
      CALL AV_PGD(YDTCO, ZROOT_EXT(:,:), PCOVER, ZDATA_ROOT_EXTINCTION(:,:), YDIF, 'ARI', OCOVER, &
                                       KDECADE=KDECADE, NPAR_VEG_IRR_USE=NPAR_VEG_IRR_USE, PIRRIG=XDATA_IRRIGFRAC)
      DO JP = 1,SIZE(PDG,3)
