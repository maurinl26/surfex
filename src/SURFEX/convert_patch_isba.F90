...
USE MODD_DATA_COVER,     ONLY : XDATA_LAI, XDATA_H_TREE,                &
                                ...
                                XDATA_LAIMIN, XDATA_WRMAX_CF,           &
                                XDATA_STRESS, XDATA_F2I, XDATA_RE25,    &
...
IF (OFIX) THEN
  ...
  !IF (OURBTREE .OR. IO%CPHOTO/='NON'.OR.LTREEDRAG) THEN
    IF (GDATA .AND. ANY(DTV%LDATA_H_TREE)) THEN
      CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
                      PK%XH_TREE,DTV%XPAR_VEGTYPE2,DTV%XPAR_H_TREE,DTV%NPAR_VEG_IRR_USE,&
                      YTREE,'ARI',PK%NR_P,IO%NPATCH,KPATCH)
    ELSE
  ...
  IF (OURBTREE) THEN
    IF (GDATA .AND. ANY(DTV%LDATA_H_TREE)) THEN
      CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
                      PK%XHTRUNK_HVEG,DTV%XPAR_VEGTYPE,SPREAD(DTV%XPAR_HTRUNK_HVEG,2,1),DTV%NPAR_VEG_IRR_USE,&
                      YTREE,'ARI',PK%NR_P,IO%NPATCH,KPATCH)
      CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
                      PK%XWCROWN_HVEG,DTV%XPAR_VEGTYPE,SPREAD(DTV%XPAR_WCROWN_HVEG,2,1),DTV%NPAR_VEG_IRR_USE,&
                      YTREE,'ARI',PK%NR_P,IO%NPATCH,KPATCH)
    ELSE
      CALL AV_PGD_1P(DTCO, PK%XHTRUNK_HVEG,PCOVER,XDATA_HTRUNK_HVEG(:,:),DTV%XPAR_IRRIGFRAC,DTV%NPAR_VEG_IRR_USE,&
              YTREE,'ARI',OCOVER,PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)
      CALL AV_PGD_1P(DTCO, PK%XWCROWN_HVEG,PCOVER,XDATA_WCROWN_HVEG(:,:),DTV%XPAR_IRRIGFRAC,DTV%NPAR_VEG_IRR_USE,&
              YTREE,'ARI',OCOVER,PK%NR_P,IO%NPATCH,KPATCH,KDECADE=KDEC)
    ENDIF
  ENDIF
  !
  ...
  IF (IO%CPHOTO/='NON') THEN
    ...
    IF (SIZE(PK%XRE25)>0) THEN
      IF (GDATA .AND. ANY(DTV%LDATA_RE25)) THEN
        CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
                      PK%XRE25,DTV%XPAR_VEGTYPE2,DTV%XPAR_RE25,DTV%NPAR_VEG_IRR_USE,&
    ...
    IF (SIZE(PK%XDMAX)>0) THEN
      IF (GDATA .AND. ANY(DTV%LDATA_DMAX)) THEN
        CALL AV_PGD_PARAM(DTV%XPAR_LAI, DTV%XPAR_VEG, &
                      PK%XDMAX,DTV%XPAR_VEGTYPE2,DTV%XPAR_DMAX,DTV%NPAR_VEG_IRR_USE,&
                      YTREE,'ARI',PK%NR_P,IO%NPATCH,KPATCH)
      ELSE
  ...
  IF ( OIRRIGMODE )  THEN
    ...
    IF (SIZE(PEK%XIRRIGTYPE)>0) THEN
      ...
      ELSE
        ...
        ENDIF
      ENDIF
      WHERE ( PEK%XIRRIGTYPE == NUNDEF .OR. PEK%XIRRIGTYPE == XUNDEF ) PEK%XIRRIGTYPE = 0
    ENDIF
    !
    ! irrigation amount
    ! -----------------
