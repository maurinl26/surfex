...
!
REAL, DIMENSION(KI)  :: ZQ_BOT_FLK     ! Heat flux through the water-bottom sediment interface [W m^{-2}]
REAL, DIMENSION(KI)  :: ZQ_W_FLK       ! Heat flux through the ice-water or air-water interface [W m^{-2}]
REAL, DIMENSION(KI)  :: ZQ_SNOW_FLK    ! Heat flux through the air-snow interface [W m^{-2}]
REAL, DIMENSION(KI)  :: ZQ_ICE_FLK     ! Heat flux through the snow-ice or air-ice interface [W m^{-2}]
REAL, DIMENSION(KI)  :: ZI_ATM_FLK     ! Radiation flux at the lower boundary of the atmosphere [W m^{-2}]
REAL, DIMENSION(KI)  :: ZI_SNOW_FLK    ! Radiation flux through the air-snow interface [W m^{-2}]
REAL, DIMENSION(KI)  :: ZI_ICE_FLK     ! Radiation flux through the snow-ice or air-ice interface [W m^{-2}]
REAL, DIMENSION(KI)  :: ZI_W_FLK       ! Radiation flux through the ice-water or air-water interface [W m^{-2}]
REAL, DIMENSION(KI)  :: ZI_H_FLK       ! Radiation flux through the mixed-layer-thermocline interface [W m^{-2}]
REAL, DIMENSION(KI)  :: ZI_BOT_FLK       ! Radiation flux through the water-bottom sediment interface [W m^{-2}]
REAL, DIMENSION(KI)  :: ZI_INTM_0_H_FLK  ! Mean radiation flux over the mixed layer [W m^{-1}]
REAL, DIMENSION(KI)  :: ZI_INTM_H_D_FLK  ! Mean radiation flux over the thermocline [W m^{-1}]
REAL, DIMENSION(KI)  :: ZQ_STAR_FLK    ! A generalized heat flux scale [W m^{-2}]
!
...
ZI_H_FLK       (:) = XUNDEF
ZI_BOT_FLK     (:) = XUNDEF
ZI_INTM_0_H_FLK(:) = XUNDEF
ZI_INTM_H_D_FLK(:) = XUNDEF
ZQ_STAR_FLK    (:) = XUNDEF
!
ZDIR_ALB   (:,:) = XUNDEF
ZSCA_ALB   (:,:) = XUNDEF
...
! Heat and radiative fluxes diagnostics              
                      ZQ_BOT_FLK, ZQ_W_FLK, ZQ_SNOW_FLK, ZQ_ICE_FLK,  &
                      ZI_ATM_FLK, ZI_SNOW_FLK, ZI_ICE_FLK, ZI_W_FLK,  &
                      ZI_H_FLK, ZI_BOT_FLK, ZI_INTM_0_H_FLK,          &
                      ZI_INTM_H_D_FLK, ZQ_STAR_FLK,                   &
! Flags              
...
!-------------------------------------------------------------------------------------
!
 CALL DIAG_MISC_FLAKE_n(DMF, F, ZQ_BOT_FLK, ZQ_W_FLK, ZQ_SNOW_FLK, ZQ_ICE_FLK,    &
                        ZI_ATM_FLK, ZI_SNOW_FLK, ZI_ICE_FLK, ZI_W_FLK, ZI_H_FLK,  &
                        ZI_BOT_FLK, ZI_INTM_0_H_FLK, ZI_INTM_H_D_FLK, ZQ_STAR_FLK,&
                        PRAIN, PSNOW, PSFTQ, PTSTEP)
!
!-------------------------------------------------------------------------------
