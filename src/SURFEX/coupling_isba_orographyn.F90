...
SUBROUTINE COUPLING_ISBA_OROGRAPHY_n (DTCO, UG, U, USS, SB, NAG, CHI, NCHI, MGN, MSF,DTV, ID, NGB, GB, &
                                      ...
                                      SLT, BLOWSNW, HPROGRAM, HCOUPLING, PTSTEP, PTIMEC,      &
...
!
USE MODD_SURF_ATM, ONLY : LNOSOF, LSLOPE, LVERTSHIFT, LDIFFUSE_SVF, LLONGWAVE_SVF
...
!
IF (LSLOPE) THEN
! M. Lafaysse, August 2017, update January 2025
!
...
! It replaces the parameterization LNOSOF = FALSE which is incorrect in that case
!
! With this option, precipitation are projected on the corresponding surface.
!
! The LSLOPE parameterization must be used when the shortwave direct radiation has already
! been projected on the slope (no correction applied on direct radiation).
! If the incoming diffuse shortwave and longwave radiations are horizontal and isotropic in the forcing,
! simple corrections can be applied as a function of the sky view factor which is estimated for
! an infinite slope, in that case LDIFFUSE_SVF and LLONGWAVE_SVF should be True
!  
! IF LLONGWAVE_SVF, the longwave radiation of the complementary solid angle is computed assuming
...
!
! If the longwave and diffuse radiation in the forcing file already already account for slope effects,
! LDIFFUSE_SVF and LLONGWAVE_SVF should be set to False.
!
! Do not modify the direct radiation which must be projected in the forcing file.
   ZDIR_SW(:,:) = PDIR_SW(:,:)
!
! Compute the sky view factor
   Z3D_TOT_SURF_INV (:) =1./SQRT(1.+ISS%XSSO_SLOPE(:)**2) ! This is cosine of the slope
   ZSKYVF    (:) =  (1+Z3D_TOT_SURF_INV(:))/2  ! (1+cos(theta))/2 ,Dumont et al. 2017
!
  IF (LDIFFUSE_SVF) THEN
  ...
  ! same surface temperature as the simulation point.
    ...
    + XSTEFAN*S%XEMIS_NAT(:)*S%XTSRAD_NAT(:)**4 * (1.-ZSKYVF(:))! Part coming from the ground 
  ENDIF
!
...
!
 CALL COUPLING_ISBA_CANOPY_n(DTCO, UG, U, USS, SB, NAG, CHI, NCHI, MGN, MSF,DTV, ID, NGB, GB,   &
                             ISS, NISS, IG, NIG, IO, S, K, NK, NP, NPE, AT, NDST, DST, SLT, &
                             BLOWSNW,HPROGRAM, HCOUPLING, PTSTEP, PTIMEC,                     &
                             KYEAR, KMONTH, KDAY, PTIME, KI, KSV, KSW, PTSUN, PZENITH, &
                             PZENITH2, PAZIM, PZREF, PUREF, PZS, PU, PV, ZQA, ZTA,     &
                             ZRHOA, PSV, PCO2,PIMPWET,PIMPDRY, HSV, ZRAIN, ZSNOW, ZLW, &
                             ZDIR_SW,                                                  &
