...
!     #########
      SUBROUTINE DEFAULT_CROCUS(HSNOWDRIFT, OSNOWDRIFT_SUBLIM, OSNOW_ABS_ZENITH, HSNOWMETAMO,                &
                                HSNOWRAD, OATMORAD, OSNOWSYTRON, OSNOWPAPPUS, HSNOWPAPPUSERODEPO,            &
                                HSNOWFPAPPUS, HPAPPUSSUBLI, HSALTPAPPUS, HLIMVFALL,            &
                                OPAPPULIMTFLUX,OPAPPUDEBUG, HSNOWFALL, HSNOWCOND, HSNOWHOLD, HSNOWCOMP,      &
                                HSNOWZREF,OSNOWCOMPACT_BOOL, OSNOWMAK_BOOL, OPRODSNOWMAK, OSNOWMAK_PROP,     &
                                OSNOWTILLER, OSELF_PROD, HSNOWMOB)
!
!     ########################################################################
!
...
! Logial to activate / disactivate Pappus snow transport scheme
LOGICAL, INTENT(OUT)          :: OSNOWPAPPUS
CHARACTER(3), INTENT(INOUT)   :: HSNOWPAPPUSERODEPO, HSALTPAPPUS
CHARACTER(4), INTENT(INOUT)   :: HSNOWFPAPPUS,HPAPPUSSUBLI,HSNOWMOB,HLIMVFALL
LOGICAL, INTENT(INOUT)        :: OPAPPULIMTFLUX,OPAPPUDEBUG
!                                HSNOWPAPPUSERODEPO = option controling the way deposition rate is calculated
!                                HPAPPUSSUBLI = option selecting sublimation parametrisation in snow pappus transport module
!                                OPAPPULIMTFLUX = option to activate snow transport flux limitation
!                                OPAPPUDEBUG = option to activate debug mode and
!                                output in snowpappus
! Snow metamorphism scheme and radiative transfer scheme <bber
!                                HSALTPAPPUS = option for saltation transport in SnowPappus
!                                HSNOWFPAPPUS = option to force the parametrisation of fresh snow characteristics
!                                HLIMVFALL = option for the calculation of terminal fall speed ( limit old/new snow )
...
HSNOWCOMP   = 'B92'
HSNOWZREF   = 'CST'
!Pappus transport scheme option
HSNOWPAPPUSERODEPO = 'DIV'
HPAPPUSSUBLI = 'NONE'
HSNOWMOB = 'GM98'
HSNOWFPAPPUS = 'NONE'
HLIMVFALL = 'MIXT'
OPAPPULIMTFLUX = .FALSE.
OPAPPUDEBUG = .FALSE.
HSALTPAPPUS = 'P90'
