...
!     #########
      SUBROUTINE DEFAULT_DIAG_FLAKE(K2M,OSURF_BUDGET,O2M_MIN_ZS,ORAD_BUDGET,OCOEF,OSURF_VARS,      &
                                     ...
                                     PZWAT_PROFILE,PZSED_PROFILE,OSEDIM_PROFILE,OFLKFLUX,OFLKWATER )  
!     ########################################################################
!
...
LOGICAL,  INTENT(OUT) :: OWATER_PROFILE ! flag for inline computation of water 
                                        ...
                                        !to FLake model
LOGICAL,  INTENT(OUT) :: OSEDIM_PROFILE ! flag for inline computation of water 
                                        !temperature at given depths according
                                        !to FLake model    
LOGICAL,  INTENT(OUT) :: OFLKFLUX      ! flag for heat and radiation fluxes
LOGICAL,  INTENT(OUT) :: OFLKWATER     ! flag for effective rain P-E
...
REAL, DIMENSION(:), INTENT(OUT) :: PZWAT_PROFILE ! depths where to compute water
                                                ...
                                                ! namelist
REAL, DIMENSION(:), INTENT(OUT) :: PZSED_PROFILE ! depths where to compute water
                                                ! temperatures according FLake
                                                ! model - variable readed in the
                                                ! namelist
