...
!     #########
      SUBROUTINE DEFAULT_SURF_ATM(POUT_TSTEP, PCISMIN, PVMODMIN, OALDTHRES,             &
                                    ODRAG_COEF_ARP, OALDZ0H, ONOSOF, OSLOPE,            &
                                    ODIFFUSE_SVF, OLONGWAVE_SVF, OCPL_GCM,              &
...
LOGICAL,           INTENT(OUT) :: ODIFFUSE_SVF
LOGICAL,           INTENT(OUT) :: OLONGWAVE_SVF
...
ONOSOF = .TRUE.
OSLOPE = .FALSE.
ODIFFUSE_SVF = .TRUE. ! only active if lslope = True
OLONGWAVE_SVF = .TRUE. ! only active if lslope = True
OVERTSHIFT = .FALSE.
OVSHIFT_LW = .FALSE.
OVSHIFT_PRCP = .FALSE.
OCPL_GCM = .FALSE.
