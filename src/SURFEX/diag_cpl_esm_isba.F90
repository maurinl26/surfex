...
!
!
! * update drainage and leaching for landuse mass and carbon conservation (kg/m2/s)
!   -----------------------------------------------------------------------------
!
IF(IO%LLULCC.AND.IO%CISBA=='DIF')THEN
  S%XCPL_DRAIN(:) = S%XCPL_DRAIN (:) + PTSTEP * S%XWCONSRV(:)
ENDIF
!
IF(IO%LLULCC.AND.LCPL_RIVCARB.AND.IO%LCLEACH)THEN
  S%XCPL_DOCFLUX(:) = S%XCPL_DOCFLUX(:) + PTSTEP * S%XCCONSRV(:)
ENDIF
!
!-------------------------------------------------------------------------------
