...
DO JI=1,KSIZE
   ...
   DECK%XGNDF      (JI) = DECK%XGNDF      (JI) + DEK%XGNDF      (JI) * PTSTEP
   !
ENDDO
!
!-------------------------------------------------------------------------------------
...
IF (IO%LMEB_PATCH(KPATCH)) THEN
   ...
   DO JI=1,KSIZE
      ...
      DECK%XUNLOAD (JI) = DECK%XUNLOAD (JI) + DEK%XUNLOAD (JI) * PTSTEP
...
IF (PEK%TSNOW%SCHEME=='CRO' .AND. IO%LSNOWSYTRON) THEN
   DO JI=1,KSIZE
      DECK%XSYTMASS(JI) = DECK%XSYTMASS(JI) + DEK%XSYTMASS(JI) * PTSTEP
   ENDDO
END IF
!
! Snowpappus
IF (PEK%TSNOW%SCHEME=='CRO' .AND. IO%LSNOWPAPPUS) THEN
   DO JI=1,KSIZE
      DECK%XQDEP_TOT      (JI) = DECK%XQDEP_TOT      (JI) + DEK%XQDEP_TOT      (JI) * PTSTEP
      DECK%XQ_OUT_SUBL    (JI) = DECK%XQ_OUT_SUBL    (JI) + DEK%XQ_OUT_SUBL    (JI) * PTSTEP
      DECK%XQT_TOT        (JI) = DECK%XQT_TOT        (JI) + DEK%XQT_TOT        (JI) * PTSTEP
      DECK%XSNOWDEBTC     (JI) = DECK%XSNOWDEBTC     (JI) + DEK%XSNOWDEBTC     (JI) * PTSTEP
      DECK%XBLOWSNWFLUXINT(JI) = DECK%XBLOWSNWFLUXINT(JI) + DEK%XBLOWSNWFLUXINT(JI) * PTSTEP
      DECK%XBLOWSNWFLUX_1M(JI) = DECK%XBLOWSNWFLUX_1M(JI) + DEK%XBLOWSNWFLUX_1M(JI) * PTSTEP
   ENDDO
END IF
!
!-------------------------------------------------------------------------------------
