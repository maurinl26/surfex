...
IF (DMF%LWATER_PROFILE) THEN
   ALLOCATE (DMF%XZWPROF(COUNT(DMF%XZWAT_PROFILE/= XUNDEF)))
   ALLOCATE (DMF%XTWPROF(COUNT(DMF%XZWAT_PROFILE/= XUNDEF),KLU))
   DMF%XZWPROF=DMF%XZWAT_PROFILE(:COUNT(DMF%XZWAT_PROFILE /= XUNDEF))
 ELSE
   ALLOCATE (DMF%XZWPROF(0))
   ALLOCATE (DMF%XTWPROF(0,0)) 
 END IF
!
!* Flake temperature profile in sediment
!
IF (DMF%LSEDIM_PROFILE) THEN
   ALLOCATE (DMF%XZSPROF(COUNT(DMF%XZSED_PROFILE/= XUNDEF)))
   DMF%XZSPROF=DMF%XZSED_PROFILE(:COUNT(DMF%XZSED_PROFILE /= XUNDEF))
   ALLOCATE (DMF%XTSPROF(COUNT(DMF%XZSED_PROFILE/= XUNDEF),KLU))
 ELSE
   ALLOCATE (DMF%XZSPROF(0))
   ALLOCATE (DMF%XTSPROF(0,0))
 END IF
!
!* Flake diagnostics
!
IF (DMF%LFLKFLUX) THEN
   ALLOCATE (DMF%XQBOT_FLK(KLU))
   ALLOCATE (DMF%XQW_FLK(KLU))
   ALLOCATE (DMF%XQSNOW_FLK(KLU))
   ALLOCATE (DMF%XQICE_FLK(KLU))
   ALLOCATE (DMF%XIATM_FLK(KLU))
   ALLOCATE (DMF%XISNOW_FLK(KLU))
   ALLOCATE (DMF%XIICE_FLK(KLU))
   ALLOCATE (DMF%XIW_FLK(KLU))
   ALLOCATE (DMF%XIH_FLK(KLU))
   ALLOCATE (DMF%XIBOT_FLK(KLU))
   ALLOCATE (DMF%XIINTM0H_FLK(KLU))
   ALLOCATE (DMF%XIINTMHD_FLK(KLU))
   ALLOCATE (DMF%XQSTAR_FLK(KLU))
   ALLOCATE (DMF%XQSTO_FLK(KLU))
   !
   ALLOCATE (DMF%XQSTOC_FLK(KLU))
   ALLOCATE (DMF%XQWC_FLK(KLU))
   ALLOCATE (DMF%XIWC_FLK(KLU))
   ALLOCATE (DMF%XQBOTC_FLK(KLU))
   ALLOCATE (DMF%XIBOTC_FLK(KLU))
ELSE
   ALLOCATE (DMF%XQBOT_FLK(0))
   ALLOCATE (DMF%XQW_FLK(0))
   ALLOCATE (DMF%XQSNOW_FLK(0))
   ALLOCATE (DMF%XQICE_FLK(0))
   ALLOCATE (DMF%XIATM_FLK(0))
   ALLOCATE (DMF%XISNOW_FLK(0))
   ALLOCATE (DMF%XIICE_FLK(0))
   ALLOCATE (DMF%XIW_FLK(0))
   ALLOCATE (DMF%XIH_FLK(0))
   ALLOCATE (DMF%XIBOT_FLK(0))
   ALLOCATE (DMF%XIINTM0H_FLK(0))
   ALLOCATE (DMF%XIINTMHD_FLK(0))
   ALLOCATE (DMF%XQSTAR_FLK(0))
   ALLOCATE (DMF%XQSTO_FLK(0))
   !
   ALLOCATE (DMF%XQSTOC_FLK(0))
   ALLOCATE (DMF%XQWC_FLK(0))
   ALLOCATE (DMF%XIWC_FLK(0))
   ALLOCATE (DMF%XQBOTC_FLK(0))
   ALLOCATE (DMF%XIBOTC_FLK(0))
ENDIF
!
IF (DMF%LFLKWATER) THEN
   ALLOCATE (DMF%XPME(KLU))
   ALLOCATE (DMF%XPMEC(KLU))
   ALLOCATE (DMF%XPRE(KLU))
   ALLOCATE (DMF%XPREC(KLU))
   DMF%XPME(:)  = 0.0
   DMF%XPMEC(:) = 0.0
   DMF%XPRE(:)  = 0.0
   DMF%XPREC(:) = 0.0
ELSE
   ALLOCATE (DMF%XPME(0))
   ALLOCATE (DMF%XPMEC(0))
   ALLOCATE (DMF%XPRE(0))
   ALLOCATE (DMF%XPREC(0))
ENDIF
!
