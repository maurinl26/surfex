...
IF(GCUMUL)THEN
  ...
  IF (KLU>0) THEN
    DEC%XRAINFALL   = XUNDEF
    DEC%XSNOWFALL   = XUNDEF
  ENDIF
  !
...
!
! CROCUS: WATER BUDGET -> BETTER HERE THAN IN DIAG_MISC_ISBA_t
IF ( HSNOW_SCHEME=="CRO" .AND. IO%LSNOWSYTRON ) THEN
  ALLOCATE(DEA%XSYTMASS(KLUA))
ELSE
  ALLOCATE(DEA%XSYTMASS(0))
ENDIF
IF ( HSNOW_SCHEME=="CRO" .AND. IO%LSNOWPAPPUS ) THEN
  ALLOCATE(DEA%XQDEP_TOT      (KLUA))
  ALLOCATE(DEA%XQ_OUT_SUBL    (KLUA))
  ALLOCATE(DEA%XQT_TOT        (KLUA))
  ALLOCATE(DEA%XSNOWDEBTC     (KLUA))
  ALLOCATE(DEA%XBLOWSNWFLUX_1M(KLUA))
  ALLOCATE(DEA%XBLOWSNWFLUXINT(KLUA))
ELSE
  ALLOCATE(DEA%XQDEP_TOT      (0))
  ALLOCATE(DEA%XQ_OUT_SUBL    (0))
  ALLOCATE(DEA%XQT_TOT        (0))
  ALLOCATE(DEA%XSNOWDEBTC     (0))
  ALLOCATE(DEA%XBLOWSNWFLUX_1M(0))
  ALLOCATE(DEA%XBLOWSNWFLUXINT(0))
ENDIF
!
IF (KLUA>0) THEN
  !
  ...
  ENDIF
  IF ( HSNOW_SCHEME=="CRO" .AND. IO%LSNOWPAPPUS ) THEN
    DEA%XQDEP_TOT      (KLUA) = XUNDEF
    DEA%XQ_OUT_SUBL    (KLUA) = XUNDEF
    DEA%XQT_TOT        (KLUA) = XUNDEF
    DEA%XSNOWDEBTC     (KLUA) = XUNDEF
    DEA%XBLOWSNWFLUX_1M(KLUA) = XUNDEF
    DEA%XBLOWSNWFLUXINT(KLUA) = XUNDEF
  ENDIF
ENDIF
!
...
ENDDO
!
IF (IVERSION>=9 .AND. IBUG>=2 ) THEN
    YREC='GNDFC_'//YREC2
    CALL MAKE_CHOICE_ARRAY(HPROGRAM, KNP, GDIM, YREC, ZWORK)
    DO JP = 1,KNP
        CALL PACK_SAME_RANK(NP%AL(JP)%NR_P,ZWORK(:,JP),NDEC%AL(JP)%XGNDF(:))
    ENDDO
ENDIF
!
IF (LIRRIGMODE) THEN
