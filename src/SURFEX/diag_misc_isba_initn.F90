...
IF (HSNOW_SCHEME=="CRO" ) THEN
   CALL ALLOC_MISC_CROCUS(DM, KLU ,0 , DM%LPROSNOW, NPNBANDS_MODIS)
ELSE
   CALL ALLOC_MISC_CROCUS(DM, 0 ,0 , DM%LPROSNOW, NPNBANDS_MODIS)
ENDIF
!
!* UTCI specific
!
...
IF (HSNOW_SCHEME=="CRO" ) THEN
   DO JP=1,IO%NPATCH
      INIP = NP%AL(JP)%NSIZE_P   
      CALL ALLOC_MISC_CROCUS(NDM%AL(JP), INIP, INIP, DM%LPROSNOW, NPNBANDS_MODIS)
   ENDDO
ELSE
   DO JP=1,IO%NPATCH
      CALL ALLOC_MISC_CROCUS(NDM%AL(JP), 0, 0, DM%LPROSNOW, NPNBANDS_MODIS)
   ENDDO
ENDIF
!
!-------------------------------------------------------------------------------
...
ALLOCATE(DMA%XRGSOIL   (KLUAP))
ALLOCATE(DMA%XRGSOILI  (KLUAP))
...
!
!
SUBROUTINE ALLOC_MISC_CROCUS(DMA, KLUA, KLUAP, OPROSNOW, KNBANDS_MODIS)
!
TYPE(DIAG_MISC_ISBA_t), INTENT(INOUT) :: DMA
!
INTEGER, INTENT(IN) :: KLUA
...
INTEGER, INTENT(IN) :: KNBANDS_MODIS
!
INTEGER         :: ISNLAYER
INTEGER         :: ISW
INTEGER         :: IIMPUR
INTEGER         :: INBANDS_MODIS
!
REAL(KIND=JPRB) :: ZHOOK_HANDLE
!
...
IF ( OPROSNOW ) THEN
   ...
   ALLOCATE(DMA%XSNOWHIST      (KLUAP,ISNLAYER))
   ...
   ALLOCATE(DMA%XSPEC_ALB      (KLUAP,ISW))
   ALLOCATE(DMA%XDIFF_RATIO    (KLUAP,ISW))
   ALLOCATE(DMA%XSPEC_TOT      (KLUAP,ISW))
   ALLOCATE(DMA%XSPECMOD       (KLUAP,INBANDS_MODIS))
   ...
   IF (KLUAP>0) THEN
      ...
      DMA%XSPEC_ALB      = XUNDEF
      DMA%XDIFF_RATIO    = XUNDEF
      DMA%XSPEC_TOT      = XUNDEF
      DMA%XSPECMOD       = XUNDEF
      DMA%XIMPUR_CONC    = XUNDEF  
   ENDIF
   !
   ALLOCATE(DMA%XSNDPT_12H     (KLUA))
   ...
   !
   ! Diag ANGE MPI
   ALLOCATE(DMA%XJJ            (KLUA))
   ALLOCATE(DMA%XNRANK         (KLUA))
   ALLOCATE(DMA%XSIZE_TASK     (KLUA))
   ALLOCATE(DMA%XILOCNIY       (KLUA))

   !
   IF (KLUA>0) THEN
      DMA%XSNDPT_12H      = XUNDEF
...
ELSE
   ...
   ALLOCATE(DMA%XSPEC_TOT      (0,0))
   ALLOCATE(DMA%XSPECMOD       (0,0))
   ALLOCATE(DMA%XIMPUR_CONC    (0,0,0))
   !
   ALLOCATE(DMA%XSNDPT_12H     (0))
   ...
   !
   ALLOCATE(DMA%XJJ            (0))
   ALLOCATE(DMA%XNRANK         (0))
   ALLOCATE(DMA%XSIZE_TASK     (0))
   ALLOCATE(DMA%XILOCNIY       (0))
ENDIF
!
! * SNOWPAPPUS: MISC DIAG (others in EVAP_DIAG)
!
IF ( IO%LSNOWPAPPUS ) THEN
  !
  ALLOCATE(DMA%XQ_OUT_SALT    (KLUA))
  ALLOCATE(DMA%XQ_OUT_SUSP    (KLUA))
  ALLOCATE(DMA%XVFRIC_PAPPUS  (KLUA))
  ALLOCATE(DMA%XVFRIC_T_PAPPUS(KLUA))
  ALLOCATE(DMA%XPZ0_PAPPUS    (KLUA))
  ALLOCATE(DMA%XVFALL_PAPPUS  (KLUA))
  !
  IF (KLUA>0) THEN
    DMA%XQ_OUT_SALT    (KLUA) = XUNDEF
    DMA%XQ_OUT_SUSP    (KLUA) = XUNDEF
    DMA%XVFRIC_PAPPUS  (KLUA) = XUNDEF
    DMA%XVFRIC_T_PAPPUS(KLUA) = XUNDEF
    DMA%XPZ0_PAPPUS    (KLUA) = XUNDEF
    DMA%XVFALL_PAPPUS  (KLUA) = XUNDEF
  ENDIF
  !
ELSE
  !
  ALLOCATE(DMA%XQ_OUT_SALT    (0))
  ALLOCATE(DMA%XQ_OUT_SUSP    (0))
  ALLOCATE(DMA%XVFRIC_PAPPUS  (0))
  ALLOCATE(DMA%XVFRIC_T_PAPPUS(0))
  ALLOCATE(DMA%XPZ0_PAPPUS    (0))
  ALLOCATE(DMA%XVFALL_PAPPUS  (0))
  !
ENDIF
!
! * SNOWMAK_BOOL
