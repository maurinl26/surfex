...
!     #########
    SUBROUTINE DRAG(HISBA, HSNOW_ISBA, HCPSURF, HGNDRES,                     &
                    PTSTEP, PTG, PWG, PWGI,                                  &
                    PEXNS, PEXNA, PTA, PVMOD, PQA, PRR, PSR, PPS, PRS,       &
                    PVEG, PZ0, PZ0EFF, PZ0H, PWFC, PWSAT, PBCOEF, PMPOTSAT,  &
                    PPSNG, PPSNV,                                            &
                    ...
                    PLEGI_DELTA, PWR, PRHOA, PLVTT, PRGSOIL, PRGSOILI, PQSAT)  
!   ############################################################################
!
!!****  *DRAG*  
!!
...
!
CHARACTER(LEN=*),     INTENT(IN)  :: HGNDRES    ! option for baresoil Evap resistance
!                                               ! 'NON' = off (none)
!                                               ! 'S92' = Sellers et al. (1992)
!                                               ! 'DSL' = Dry Soil Layer: DSL (Swenson & Lawrence, 2014)
!
...
REAL, DIMENSION(:), INTENT(IN)   :: PWFC, PWSAT, PPSNG, PPSNV, PZREF, PUREF, &
                                    PBCOEF, PMPOTSAT
!                                     PBCOEF   = soil hydology pedotransfer slope parameter (-)
...
!                                     PRGSOIL   = baresoil evap resistance (s/m)
!                                     PRGSOILI  = baresoil sublim resistance (s/m)
...
!                                              ZWFC = field capacity in presence of ice
                                 ZWSAT,          &
!                                              ZWSAT = saturated liq water content in presence of ice
                                 ZVMOD,          &
...
!
!-------------------------------------------------------------------------------
!*       9.     Soil resistance (for baresoil evap) options
!               -------------------------------------------
!
! NOTE, for now assume PRGSOILI=0 as main effect of DSL expected for warm dry areas
!       and impact on soil ice not known.
!
PRGSOIL(:)     = 0.
PRGSOILI(:)    = 0.
IF (   HGNDRES=='DSL') THEN ! Dry Soil Layer (DSL) method
   PRGSOIL(:)  =  SOIL_RES_EG_DSL(PTG,PPS,PWG,PWGI,PWSAT,PBCOEF,PMPOTSAT)
ELSEIF(HGNDRES=='S92') THEN ! Sellers et al. (1992) method 
   ZWSAT(:)    = MAX(XWGMIN,PWSAT(:) - PWGI(:)) ! ice modified saturation water content (m3/m3)
   PRGSOIL(:)  = SOIL_RES_EG_S92(PWG, ZWSAT) 
ENDIF

! Incorporate soil resistance into delta factor:

PLEG_DELTA(:)  = PLEG_DELTA(:) *PRA(:)/(PRA(:) + PRGSOIL(:) )
PLEGI_DELTA(:) = PLEGI_DELTA(:)*PRA(:)/(PRA(:) + PRGSOILI(:))

!-------------------------------------------------------------------------------
!
!
!*      10.     HALSTEAD COEFFICIENT (WITH THE NEW VALUES OF PRA)
!               ----------------------------------------------------------
!
!
PHV(:)        = PDELTA(:) + (1.- PDELTA(:))*                                       &
