...
!   ############################################################################
!
SUBROUTINE DRAG_MEB(IO, PK, PEK, AT, DMK, DK, PTG, PTA, PQA, PVMOD,         &
                    PWG, PWGI, PWSAT, PWFC, PBCOEF, PMPOTSAT,               &
                    PEXNS, PEXNA, PPS,  PRR, PSR,                           &
                    ...
                    PQSATG, PQSATV, PQSATC, PQSATN, PDELTAVK, PLAI_SCALE)
!                    
!   ############################################################################
!
!!****  *DRAG_MEB*  
...
!                   09/2022 (A. Bouchet, A. Boone) MEB_TALL_TREE option added
!                   06/2025 (B. Marti, A. Boone) added DSL soil res option
!-------------------------------------------------------------------------------
!
!*       0.     DECLARATIONS
!               ------------
...
USE MODD_SNOW_PAR, ONLY : XZ0SN, X_RI_MAX
...
USE MODE_HYDRO_DIF, ONLY : SOIL_RES_EG_S92, SOIL_RES_EG_DSL
!
...
REAL, DIMENSION(:), INTENT(IN)   :: PTG, PTA, PQA, PVMOD, PWG, PWGI, PWSAT, PWFC, &
                                    PBCOEF, PMPOTSAT, PEXNS, PEXNA, PPS, PSNOWSWE
...
!                                     PBCOEF  = slope of the soil water retention curve (-)
!                                     PMPOTSAT= saturated matric potential (m)
...
!
REAL, DIMENSION(:), INTENT(IN)   :: PLAI_SCALE
!                                   PLAI_SCALE = LAI scale factor for intercepted snow (>= 1) (-)
!
...
!
REAL, DIMENSION(SIZE(PTG)) :: ZCHIL, ZLAISN, ZLAI, ZLW, ZDISPH, ZVELC, ZRICN, ZRA_C_A, &
                              ZRA_G_C, ZG_VG_C, ZRA_N_C, ZG_VN_C 
!                              ZLAISN = leaf area index for snow covered ground (m2/m2)
!                                       possibly rescaled for tall trees (m2/m2)
!                              ZLAI   = leaf area index for snow-free ground
!                                       possibly rescaled for tall trees (m2/m2)
!                              ZLW = leaf width
...
REAL, PARAMETER            :: ZLEAF_WIDTH  = 0.02   ! leaf width (m)
REAL, PARAMETER            :: ZLEAF_ANGLD  = 0.12   ! Ross-Goudriaan leaf angle distribution (-)
!
...
!
! Possible Rescaling with HVEG: for very tall trees rescale LAI effect
! (LAI_SCALE >= 1)
!
ZLAI(:) = PEK%XLAI(:) * PLAI_SCALE(:)
!
...
!  
CALL SURFACE_AIR_MEB(PZ0_MEBV, PZ0H_MEBV, PZ0G_WITHOUT_SNOW, PEK%XH_VEG, ZLAI,    &
                     PK%XVEGTYPE_PATCH, PTG, PEK%XTC, PEK%XTV, PVELC, ZLW, ZDISPH,&
                     ZRA_G_C, ZG_VG_C, ZUSTAR2G, ZCDG, ZCHG, ZRIG                 )
!
!Compute the lai of the canopy that is above snow
!
ZLAISN(:)= ZLAI(:)*(1.-PPALPHAN(:))
!
...
CALL SURFACE_AIR_MEB(PZ0_MEBN, PZ0H_MEBN, ZZ0SN, PEK%XH_VEG, ZLAISN,     &
                     PK%XVEGTYPE_PATCH, DMK%XSNOWTEMP(:,1), PEK%XTC,     &
                     PEK%XTV, PVELC, ZLW, ZDISPH, ZRA_N_C, ZG_VN_C,      &
...
CALL PREPS_FOR_MEB_DRAG(.FALSE.,IO%LFORC_MEASURE,                        &
                   ...
                   ZCHNN,ZCDNNN,ZCDNN                                    )
!
! For snow, Richardson number is a priori smaller -> two reals defined in namelist: XRIMAX and X_RI_MAX
ZRINN = MIN(ZRINN,X_RI_MAX)
!
IF (LRRGUST_ARP) THEN
...
! NOTE, currenty not used when litter option ON.

DMK%XRGSOIL(:)  = 0.
DMK%XRGSOILI(:) = 0.

IF (    IO%CMEB_GNDRES=='DSL' .AND. .NOT.IO%LMEB_LITTER) THEN
   DMK%XRGSOIL(:)  = SOIL_RES_EG_DSL(PTG,PPS,PWG,PWGI,PWSAT,PBCOEF,PMPOTSAT)
   DMK%XRGSOILI(:) = 0. ! As for dry warm conditions, not modified currently
ELSEIF (IO%CMEB_GNDRES=='S92' .AND. .NOT.IO%LMEB_LITTER) THEN
   DMK%XRGSOIL(:)  = SOIL_RES_EG_S92(PWG, ZWSAT)
   DMK%XRGSOILI(:) = 0. ! As for dry warm conditions, not modified currently
ENDIF

PLEG_DELTA (:)     = ZRA_G_C(:)/(ZRA_G_C(:) + DMK%XRGSOIL(:) )
PLEGI_DELTA(:)     = ZRA_G_C(:)/(ZRA_G_C(:) + DMK%XRGSOILI(:))

!-------------------------------------------------------------------------
!*       7.    CONDENSATION/DEW EFFECTS:
!              -------------------------
!
