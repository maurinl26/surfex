...
REAL, DIMENSION(NTEX)     :: ZZS0                      ! rugosite de la surface lisse
REAL, DIMENSION(NDP)      :: ZDP                       ! [micronm] diametre du particule
REAL :: ZDLNDP, ZRGH_Z0
...
!ZMBL_BSN_FCT(:) = PSAND(:)
! utilise dans le calcul de l'effet Owen 
ZWND_RFR(:) = PWIND10M(:)
!
...
DO I = 1, KSIZE
  ...
  ! 
  !tous les cas CLAY >= 0.28 sont traites
  IF ( PCLAY(I) >= 0.28 ) THEN
    ...
    ENDIF
  ENDIF
  ! les cas ZSILT >= 0.5 .AND. PCLAY < 0.28 sont traites
  ! les cas ZSILT < 0.5 .AND. PCLAY >= 0.20 sont traites 
  ! ne sont pas traites les cas PCLAY < 0.20 .AND. ZSILT < 0.5 => SAND >= 0.3
  IF ( ZSILT(I) >= 0.8 .AND. PCLAY(I) < 0.12 ) THEN ! Silt 
    ITEXT(I) = 12
    !ZWPRM(I) = 2.5E-2
  ELSEIF ( PCLAY(I) < 0.28 ) THEN    ! ( clay est forcement < 0.28 )
    ...
    ENDIF
  ENDIF
  ! les cas SAND >= 0.87 sont traites: silt < 0.13, clay < 0.1 => entrent dans les cas non encore traites
  ! les cas SAND >= 0.7 => CLAY < 0.15 , SILT < 0.3 sont traites ) => ""
  ! les cas SAND >=0.52 .AND. CLAY < 0.20 sont traites SILT < 0.48 => ""
  ! les cas restants considerement pclay < 0.20, sand >= 0.5 - pclay => sand >=  0.3
...
ENDDO
!
IF (LHOOK) CALL DR_HOOK('DUSTFLUX_GET_MB',1,ZHOOK_HANDLE)
