...
REAL, DIMENSION(SIZE(PTA))        :: ZDTH,ZDQ,ZDUWG
...
!
REAL, DIMENSION(SIZE(PTA))        :: ZLOGHS10,ZLOGTS10
REAL    :: ZCHIC,ZCHIK,ZEPS,ZPI,ZPIS2,ZPSIC,ZPSIK, &
             ZSQR3,ZZDQ,ZZDTH
REAL    :: ZUSR2,ZTSR2,ZQSR2,ZZ
!
...
!       2.3. Initial guess
!
ZDELTAT10NODT(:) = 1.2+6.3E-03*MAX(ZVMOD(:)-10.0,0.0)
!
IF (OPWG) THEN !initial guess for gustiness factor
   ZWG   = 0.5
   ZDUWG (:) = SQRT(ZVMOD(:)**2+ZWG**2)
ELSE
   ZDUWG (:) = ZVMOD(:)
ENDIF
!
ZDELTAU10N(:) = ZDUWG(:)
ZDELTAT10N(:) = ZDTH (:)*ZDELTAT10NODT(:)
ZDELTAQ10N(:) = ZDQ  (:)
ZDELTAQ10NODQ(:) = 1.
!
...
!
  DO JLON=1,SIZE(PTA)
    ...
    ZTSR2 = ZCHN(JLON)/ZUSR2
    ZQSR2 = ZCEN(JLON)/ZUSR2
...
!
  IF(OPWG) THEN
    DO JLON=1,SIZE(PTA)
      ...
      ZWG=ZBETAGUST*(ZBF(JLON)*ZZBL)**(1./3)
      ZDUWG(JLON) = SQRT(ZVMOD(JLON)**2+ZWG**2)
    ENDDO
  ENDIF
!
...
!
! For U
  !$OMP PARALLEL DO PRIVATE(ZLMOU,ZPSI_U,ZCHIK,ZPSIK,ZCHIC,ZPSIC)
  DO JLON=1,SIZE(PTA)
...
!
    ...
    ZDELTAU10N(JLON) = ZDUWG(JLON)-ZUSR(JLON)*(ZLOGHS10(JLON)-ZPSI_U)/XKARMAN
  ENDDO
  !$OMP END PARALLEL DO
! For T/Q
  !$OMP PARALLEL DO PRIVATE(ZLMOT,ZPSI_T,ZCHIK,ZPSIK,ZCHIC,ZPSIC)
  DO JLON=1,SIZE(PTA)
    ZLMOT = PZREF(JLON)*XG*XKARMAN*(ZTSR(JLON)/(PTA(JLON)) &
       +ZETV*ZQSR(JLON)/(1.0+ZETV*PQA(JLON)))/MAX(ZUSR(JLON),ZEPS)**2  
...
!
    ...
    ZDELTAT10N(JLON) = ZDTH (JLON)-ZTSR(JLON)*(ZLOGTS10(JLON)-ZPSI_T)/XKARMAN
    ZDELTAQ10N(JLON) = ZDQ  (JLON)-ZQSR(JLON)*(ZLOGTS10(JLON)-ZPSI_T)/XKARMAN

    ZDELTAT10NODT(JLON)=1.-ZTSRODT(JLON)*(ZLOGTS10(JLON)-ZPSI_T)/XKARMAN
    ZDELTAQ10NODQ(JLON)=1.-ZQSRODQ(JLON)*(ZLOGTS10(JLON)-ZPSI_T)/XKARMAN
  ENDDO
  !$OMP END PARALLEL DO
ENDDO
!
...
!
DO JLON=1,SIZE(PTA)
  ZZ = ZUSR(JLON)/ZDUWG(JLON)
  PCD(JLON) = ZZ**2
  PCH(JLON) = ZZ*ZTSRODT(JLON)
  PCE(JLON) = ZZ*ZQSRODQ(JLON)
ENDDO
!
...
IF( OPERTFLUX )THEN
  DO JLON=1,SIZE(PTA)
    ZZ = 1+PPERTFLUX(JLON)/2
    ZTAU(JLON) = ZTAU(JLON)*ZZ
    ZHF (JLON) = ZHF(JLON)*ZZ
    ZEF (JLON) = ZEF(JLON)*ZZ
  ENDDO
ENDIF
!
!-------------------------------------------------------------------------------
...
!
PRESA(:)=1./ MAX(PCH(:)*ZVMOD(:),XSURF_EPSILON)
!
