...
REAL,DIMENSION(:),INTENT(OUT)::PSSO_DIR ! resolved aspect
...
REAL, DIMENSION(:,:), ALLOCATABLE :: ZSSO_DIR   ! explicit aspect in a 2D array
...
REAL, DIMENSION(:), ALLOCATABLE :: ZSSO_DIR0
!
! parameters
...
REAL                   :: ZDIRX    ! slope in X and Y direction of a grid point for aspect calculus
REAL                   :: ZDIRY    ! 
!
...
!
    ...
    ZSSO_DIR(JI-JPHEXT,JJ-JPHEXT)=( 1.5*XPI - ATAN2( ZDIRY, ZDIRX + SIGN(1.E-30,ZDIRX) ) )*(180.0/XPI)
    IF (ZSSO_DIR(JI-JPHEXT,JJ-JPHEXT) > 360.0) THEN
        ZSSO_DIR(JI-JPHEXT,JJ-JPHEXT) = ZSSO_DIR(JI-JPHEXT,JJ-JPHEXT) - 360.0
    ENDIF
...
ALLOCATE(ZSSO_DIR0(KDIM_FULL))
!
DO JY=1,IY
  DO JX=1,IX
    ZSSO_SLOPE0( JX + (JY-1)*IX )=ZSSO_SLOPE(JX,JY)
    ZSSO_DIR0( JX + (JY-1)*IX )=ZSSO_DIR(JX,JY)
  END DO
END DO
!
CALL READ_AND_SEND_MPI(ZSSO_SLOPE0,PSSO_SLOPE)
CALL READ_AND_SEND_MPI(ZSSO_DIR0,PSSO_DIR)
!
DEALLOCATE(ZSSO_SLOPE0)
DEALLOCATE(ZSSO_DIR0)
!
