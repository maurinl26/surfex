...
REAL, DIMENSION(SIZE(PEK%XBIOMASS,1),SIZE(PEK%XBIOMASS,2)) :: ZTURNOVER       ! Biomass mortality (kgC/m2/day)
!
...
!
ZTURNOVER(:,:) = ZTURNOVER(:,:) * (PTSTEP_DAY*XKGTOG) ! kgC/m2/day -> gC/m2
!
...
DO JI=1,INI
   IF(ABS(ZCBUDGET(JI))>XMIN_FIRE)THEN
      ...
      WRITE(KLUOUT,*) 'Ffire = ',DMK%XFIREFRA(JI)
