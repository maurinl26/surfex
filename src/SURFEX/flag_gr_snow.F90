...
DO JIMP=1,NIMPUR
	DO JLAYER = 1,TPSNOW%NLAYER
		IF (KFLAG==1) THEN
			IF (SIZE(TPSNOW%DIAMOPT) >0) THEN
				WHERE(OMASK(:)) 
					TPSNOW%IMPUR(:,JLAYER,JIMP) = XUNDEF
				END WHERE
			END IF
		ENDIF
	ENDDO
...
!
  DO JLAYER = 1,TPSNOW%NLAYER
    ...
    IF (KFLAG==1) THEN 
      ...
      IF (SIZE(TPSNOW%DIAMOPT) >0) THEN
        WHERE(OMASK(:)) 
          TPSNOW%DIAMOPT(:,JLAYER) = XUNDEF
          TPSNOW%SPHERI(:,JLAYER) = XUNDEF
  ...
  ENDDO
  !
  IF (KFLAG==1 .AND. SIZE(TPSNOW%DIAMOPT) >0) THEN
    WHERE(OMASK(:)) TPSNOW%MMP    (:) = 0.
  ENDIF
  IF (KFLAG==1) THEN
    !
