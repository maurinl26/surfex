...
! Heat and radiative fluxes
                               PQ_BOT_FLK, PQ_W_FLK, PQ_SNOW_FLK, PQ_ICE_FLK, PI_ATM_FLK,     &
                               PI_SNOW_FLK, PI_ICE_FLK, PI_W_FLK, PI_H_FLK, PI_BOT_FLK,       &
                               PI_INTM_0_H_FLK, PI_INTM_H_D_FLK, PQ_STAR_FLK,                 &
...
REAL, DIMENSION(KI), INTENT(OUT)  ::    &
    ...
    PQ_BOT_FLK             ,  &! Heat flux through the water-bottom sediment interface [W m^{-2}]
    PQ_W_FLK               ,  &! Heat flux through the ice-water or air-water interface [W m^{-2}]
    PQ_SNOW_FLK            ,  &! Heat flux through the air-snow interface [W m^{-2}]
    PQ_ICE_FLK             ,  &! Heat flux through the snow-ice or air-ice interface [W m^{-2}]
    PI_ATM_FLK             ,  &! Radiation flux at the lower boundary of the atmosphere [W m^{-2}]
    PI_SNOW_FLK            ,  &! Radiation flux through the air-snow interface [W m^{-2}]
    PI_ICE_FLK             ,  &! Radiation flux through the snow-ice or air-ice interface [W m^{-2}]
    PI_W_FLK               ,  &! Radiation flux through the ice-water or air-water interface [W m^{-2}]
    PI_H_FLK               ,  &! Radiation flux through the mixed-layer-thermocline interface [W m^{-2}]
    PI_BOT_FLK             ,  &! Radiation flux through the water-bottom sediment interface [W m^{-2}]
    PI_INTM_0_H_FLK        ,  &! Mean radiation flux over the mixed layer [W m^{-1}]
    PI_INTM_H_D_FLK        ,  &! Mean radiation flux over the thermocline [W m^{-1}]
    PQ_STAR_FLK                ! A generalized heat flux scale [W m^{-2}]
...
REAL :: ZCTROCMAX ! maximum shape factor rate of change [s^{-1}]
REAL, DIMENSION(KI) ::    zwind             ! thresholded wind
...
!
   ...
   ZCTROCMAX=F%XCTROCMAX
   IF (F%CFLK_CTMIN.NE.'SLM10') THEN
      ZCTROCMAX=F%XCTROCMAX/REAL(INDT)
   ENDIF
...
!
   CALL flake_driver ( F%XWATER_DEPTH(JL), F%XDEPTH_BS(JL), F%XT_BS(JL), F%XCORIO(JL), &
                       opticpar_water(JL)%extincoef_optic(1),F%XCTMIN,ZCTROCMAX, &
...
!
   PQ_BOT_FLK      (JL) = Q_bot_flk
   PQ_W_FLK        (JL) = Q_w_flk 
   PQ_SNOW_FLK     (JL) = Q_snow_flk
   PQ_ICE_FLK      (JL) = Q_ice_flk
   PI_ATM_FLK      (JL) = I_atm_flk
   PI_SNOW_FLK     (JL) = I_snow_flk
   PI_ICE_FLK      (JL) = I_ice_flk
   PI_W_FLK        (JL) = I_w_flk
   PI_H_FLK        (JL) = I_h_flk
   PI_BOT_FLK      (JL) = I_bot_flk 
   PI_INTM_0_H_FLK (JL) = I_intm_0_h_flk
   PI_INTM_H_D_FLK (JL) = I_intm_h_D_flk 
   PQ_STAR_FLK     (JL) = Q_star_flk
