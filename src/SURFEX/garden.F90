...
!
 CALL ISBA(GDO, K, P, PEK, G, YAG, DK, DEK, DMK, TPTIME, S%XPOI, S%XABC,       &
           GB%XIACAN, .FALSE., PTSTEP, 0.0, HIMPLICIT_WIND, PZREF, PZREF,      &
           ZDIRCOSZW, XCVHEATF, ZSLOPEDIR, PEK%TSNOW%SPHERI(:,:),              &
           PEK%TSNOW%SPHERI(:,:), PTA, PQA, PEXNS, PRHOA, PPS, PEXNS, PRR, PSR,&
           PZENITH,PAZIM,ZP_MEB_SCA_SW, PSW, PLW, PLE_HVEG, PVMOD, ZWINDDIR,   &
...
DEK%XRESP_AUTO(:) = 0.
!
IF (GDO%CPHOTO/='NON' .AND. GDO%CRESPSL/='DEF' .AND. ANY(PEK%XLAI(:)/=XUNDEF)) THEN
