...
REAL                              :: ZDXLIM,ZAX,ZAY   ! X grid mesh size
...
!
 CALL XY_CONF_PROJ(XLAT0,XLON0,XRPK,XBETA,XLATOR,XLONOR,ZX,ZY,PLAT,ZLON)  
!
!
!
...
! optim: no OMP directive here
IF (KSSO == 0.AND.KFSSO == 0) THEN
  DO JL=1,SIZE(PLON)
...
!
    ZAX = XXLIM(JI)
    ZAX = (ZX(JL)-ZAX)/(XXLIM(JI+1)-ZAX)
    ZAY = XYLIM(JJ)
    ZAY = (ZY(JL)-ZAY)/(XYLIM(JJ+1)-ZAY)

    IF (KSSO/=0) THEN
      KISSOX(1,JL) = 1 + INT( FLOAT(KSSO) * ZAX)
      KISSOY(1,JL) = 1 + INT( FLOAT(KSSO) * ZAY)
    END IF

    IF (KFSSO/=0) THEN
      KFISSOX(1,JL) = 1 + INT( FLOAT(KFSSO) * ZAX)
      KFISSOY(1,JL) = 1 + INT( FLOAT(KFSSO) * ZAY)
    END IF
  END DO
END IF
!
