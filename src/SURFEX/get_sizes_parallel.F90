...
SUBROUTINE GET_SIZES_PARALLEL (DTCO, UG, U, KPROC,KSIZE,KPROCMIN,KSIZE_TASK,OSHADOWS)
  ...
  !
  !
  INTEGER, INTENT(IN) :: KPROC !nombre de thread mpi disponible
  INTEGER, INTENT(IN) :: KSIZE ! nombre de point a simuler
  INTEGER, INTENT(IN) :: KPROCMIN !en fait c'est le rank MPI MAX bordel !!  kproc-1 car commence a 0
  ...
  INTEGER, DIMENSION(0:KPROC-1), INTENT(OUT) :: KSIZE_TASK !array avec le nombre de point pour chaque thread mpi (/!\ commence a 0 ==> index de l'array=rank du threads)
  !
  ...
  IF (GSHADOWS) THEN
    ...
    IF (NIX==0) THEN
      ...
      CALL INIT_IO_SURF_n(DTCO, U, CSURF_FILETYPE, 'FULL  ', 'SURF  ', 'READ ')
      CALL READ_SURF(CSURF_FILETYPE, 'GRID_TYPE', UG%G%CGRID, IRESP, HDIR='A')
      CALL READ_GRIDTYPE(CSURF_FILETYPE, UG%G%CGRID, UG%NGRID_FULL_PAR, U%NDIM_FULL, .FALSE., HDIR="A")
      ALLOCATE(UG%XGRID_FULL_PAR(UG%NGRID_FULL_PAR))
      CALL READ_GRIDTYPE(CSURF_FILETYPE, UG%G%CGRID, UG%NGRID_FULL_PAR, U%NDIM_FULL, .TRUE., UG%XGRID_FULL_PAR, IRESP,HDIR="A")
    ...
    ISIZE_Y=KSIZE/NIX !pour recalculer NIY ==> ligne inutile dans le cas d'un appel depuis init_index

    ! Number of lines (y) for one thread
    INY_THREAD=ISIZE_Y/KPROC ! division du nombre de ligne par par le nombre de processeur
    INY_RESTE=ISIZE_Y-KPROC*INY_THREAD! equivalent a un modulo => calcul le reste des ligne non reparti par processeur a l'etape precedente

    KSIZE_TASK(:)=INY_THREAD*NIX ! array ou on fixe le nombre de point par thread mpi (avec comme index le rank mpi (commence a 0!!!!!))
    !mais il manque le reste
    !WRITE(*,*)SHAPE(KSIZE_TASK)
    !write(*,*)KSIZE_TASK
    !ici on fixe le reste de la facon la plus compliqué possible
    DO J=KPROCMIN+KPROC-INY_RESTE,KPROCMIN+KPROC-1 ! pourquoi si compliqué ? :( objectif ici c'est juste de repartir equitablement les ligne qui reste entre les processeur en commancant par le rank 1
      KSIZE_TASK(MOD(J,KPROC))=KSIZE_TASK(MOD(J,KPROC))+NIX ! aucun sens de faire comme ca => ajoute une ligne de reste
    END DO
#endif
  ELSE ! gshadown not true
