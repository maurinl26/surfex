...
!
 CALL ISBA(GRO, K, P, PEK, G, YAG, DK, DEK, DMK, TPTIME, S%XPOI, S%XABC,       &
           GB%XIACAN, .FALSE., PTSTEP, 0.0, HIMPLICIT_WIND, PZREF, PUREF,      &
           ZDIRCOSZW, XCVHEATF, ZSLOPEDIR,PEK%TSNOW%SPHERI(:,:),               &
           PEK%TSNOW%SPHERI(:,:), PTA, PQA, PEXNA, PRHOA, PPS, PEXNS, PRR, PSR,& 
...
DEK%XRESP_AUTO(:) = 0.
!
IF (GRO%CPHOTO/='NON' .AND. GRO%CRESPSL/='DEF' .AND. ANY(PEK%XLAI(:)/=XUNDEF)) THEN
