...
REAL      ::  ZPILATMAXVAL,ZPILATMINVAL,ZPILATMAXLOC,ZPILATMINLOC
...
IOS_SAVE = 1
IF (PRESENT(PILATARRAY)) THEN
  ZPILATMAXVAL=MAXVAL(PILATARRAY(:))
  ZPILATMINVAL=MINVAL(PILATARRAY(:))
  ZPILATMAXLOC=MAXLOC(PILATARRAY,1)
  ZPILATMINLOC=MINLOC(PILATARRAY,1)
END IF
DO JL = 1, KOLEN
  !
  ...
  IF (PRESENT(PILATARRAY)) THEN
    !
    IF (POLA(JL)>ZPILATMAXVAL) THEN
      KO(JL,3) = ZPILATMAXLOC
    ELSEIF (POLA(JL)<ZPILATMINVAL) THEN
      KO(JL,3) = ZPILATMINLOC
    ELSE 
      DO JLAT0 = 1,KINLA
