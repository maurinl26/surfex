...
!!                  12/2023  (B. Decharme) CHORT='CM6' -> Tuning for CNRM-CM/ESM to reduce the too earlier 
...
!
REAL, DIMENSION(SIZE(PVEG))    :: ZWGI_EXCESS, ZF2, ZICEMELT
!                                 ZWGI_EXCESS = Soil ice excess water content
!                                 ZF2         = Soilstress function for transpiration
!                                 ZICEMELT    = variable used to tunne soil insulation effect into ISBA CEB
!
...
!
  ZICEMELT(:) = 1.0
!
  IF(IO%CHORT=='CM6'.AND.PEK%TSNOW%SCHEME=='3-L'.AND.(.NOT.OMEB).AND.(.NOT.LSNOW_FRAC_TOT))THEN
    ZICEMELT(:) = MAX(0.01,1.0-PVEG(:)*PEK%XPSNG(:)) 
  ENDIF
!
...
!
     CALL ICE_SOILDIF(KK, PK, PEK, ZTSTEP, ZKSFC_IVEG, ZEGI, PSOILHCAPZ, &
                      ZICEMELT, ZWGI_EXCESS, ZDELHEATG_SFC, ZDELHEATG    )
!
