...
!!                  05/2024  B.Decharme: Explicit Surface / Water table depth coupling
!!                  01/2025  B.Decharme: Loop optimisation
!-------------------------------------------------------------------------------
!
!*       0.     DECLARATIONS
!               ------------
...
!
REAL, PARAMETER                     :: ZDZMIN = 1.0E-3 ! minimum distance between unsaturated layer mid-points (m)
!
!*      0.3    declarations of local variables
!
!
REAL, DIMENSION(SIZE(PK%XDZG,1))                 :: ZEXCESS, ZINFILTC, ZWGTOT, ZWTD, ZDZMIN_WTD, ZPSIWTD, ZLIMITK 
!                                                   ZEXCESS  = working variable: excess soil water which is used as a constraint
!                                                   ZWGTOT     = total soil moisture for ZINFNEG computation
!                                                   ZWTD       = water table depth positive below soil surface (m)
!                                                   ZDZMIN_WTD = adaptative minimum distance between unsaturated layer mid-point and WTD (m)
!
...
REAL, DIMENSION(SIZE(PK%XDZG,1),SIZE(PK%XDZG,2)) :: ZPSI, ZK, ZNU, ZWSAT, ZHEAD, ZVAPCOND, &
                                                    ZFRZ, ZKI, ZCAPACITY, ZINFNEG, ZKSAT
!                                                   ZPSI      = matric potential (m)
!                                                   ZK        = hydraulic conductivity (m s-1)
!                                                   ZKSAT     = hydraulic conductivity at saturation (m s-1)
...
!
REAL, DIMENSION(SIZE(PK%XDZG,1),SIZE(PK%XDZG,2)) :: ZCPLWTD, ZDZWTD, ZDG, ZKIS, ZNUS, ZHEADS, ZDZMINS
!                                                   ZCPLWTD   = fraction of unsaturated soil coupled with the WTD (-)
!                                                   ZDZWTD    = distance between unsaturated layer mid-point and WTD (m)
!                                                   ZDG       = work array to compute ZDZWTD (m)
!                                                   ZKIS      = interfacial hydraulic conductivity accounting for WTD (m s-1) at level z_j
!                                                   ZNUS      = interfacial total conductivity accounting for WTD (m s-1) at level z_j
!                                                   ZHEADS    = matric potential gradient accounting for WTD (-)
!
...
!
!
REAL                                :: ZLOG10, ZS, ZLOG, ZWDRAIN, ZF, ZFS, ZPG, ZEVAPCOR, &
                                       ZFWTD, ZWTDL, ZDMIN, ZDGNODE
!
REAL                                :: ZDKDT1, ZDKDT2, ZDHEADDT1, ZDHEADDT2, ZKWTD, ZWORK, &
                                       ZDKDT1S, ZDKDT2S, ZDHEADDT1S, ZDHEADDT2S
...
DO JL=1,INL
   DO JI=1,INI
      ...
      IF(JL<=IDEPTH)THEN
        ...
        !
        !hydraulic conductivity at saturation accounting for soil ice (m s-1)
        ZKSAT(JI,JL) = ZFRZ(JI,JL)*PK%XCONDSAT(JI,JL)
        !
      ENDIF
      !
   ENDDO
...
DO JL=1,INL
   DO JI=1,INI
      IDEPTH   = PK%NWG_LAYER(JI)
      ZPG      = PPG(JI)
      ZEVAPCOR = PEVAPCOR(JI)
      IF(JL<IDEPTH)THEN
        ZINFNEG(JI,JL) = (MIN(0.0,ZPG)-ZEVAPCOR)*PK%XDZG(JI,JL)*PEK%XWG(JI,JL)
...
DO JL=1,INL
   DO JI=1,INI
      ZWORK = ZWGTOT(JI)
      ZINFNEG(JI,JL) = ZINFNEG(JI,JL)/ZWORK
   ENDDO
ENDDO 
!
...
DO JL=1,INL
   DO JI=1,INI    
      ...
      IF(JL<=IDEPTH)THEN
        ...
        ZK(JI,JL) = ZKSAT(JI,JL)*EXP(-ZLOG)
      ENDIF
   ENDDO
ENDDO    
!
! Prepare water table depth coupling to match
! analytical solution of a saturated soil
! -------------------------------------------
!
IF(IO%LWTD)THEN
  !
  ...
  ENDWHERE
  !
  ZDZMIN_WTD(:)=MIN(ZDZMIN,0.5*PK%XDG(:,1))
  !
  DO JL=1,INL
     DO JI=1,INI
        ...
        ZFWTD  = KK%XFWTD(JI)
        ZDMIN  = ZDZMIN_WTD(JI)
        !Distance between each soil horizon and the WTD
        ZDG(JI,JL) = MIN(PK%XDG(JI,JL),ZWTDL)
        !Fraction and minimum thickness for WTD coupling
        IF(JL<IDEPTH.AND.ZWTDL<PK%XDG(JI,JL))THEN
           ZCPLWTD(JI,JL) = ZFWTD
           ZDZMINS(JI,JL) = ZDMIN*ZFWTD+PK%XDZDIF(JI,JL)*(1.0-ZFWTD)
        ENDIF
     ENDDO
  ENDDO
  !
  WHERE(ZCPLWTD(:,1)>0.0)
        ZDZWTD(:,1) = MAX(ZDZMINS(:,1),ZWTD(:)-0.5*ZDG(:,1))
  ENDWHERE
  !
  DO JL=2,INL
     DO JI=1,INI
        IDEPTH=PK%NWG_LAYER(JI)
        ZWTDL = ZWTD(JI)
        IF(ZCPLWTD(JI,JL)>0.0.AND.JL<IDEPTH)THEN     
           ZDZWTD(JI,JL) = MAX(ZDZMINS(JI,JL),ZWTDL-0.5*(ZDG(JI,JL)+PK%XDG(JI,JL-1)))
        ENDIF        
     ENDDO
  ENDDO
  !
  !Total interfacial conductivity accounting for WTD (m s-1)
  DO JL=1,INL
     DO JI=1,INI
        IDEPTH=PK%NWG_LAYER(JI)
        ZFWTD = KK%XFWTD(JI)
        IF(JL<IDEPTH)THEN
           ZKIS(JI,JL) = EXP(LOG(ZK(JI,JL))*(1.0-0.5*ZFWTD)+0.5*LOG(ZKSAT(JI,JL))*ZFWTD) 
        ENDIF
     ENDDO
  ENDDO
  !
  ...
  DO JI=1,INI
     ...
     ZFWTD   = KK%XFWTD(JI)
     ZDMIN   = ZDZMIN_WTD(JI)
     ZDGNODE = PK%XDG(JI,IDEPTH)-0.5*PK%XDZG(JI,IDEPTH)
     !
     !Fraction for WTD coupling (0.0 if WTD > 100m depth)
     IF(ZWTDL<XWTD_MAXDEPTH)THEN
        ZCPLWTD(JI,IDEPTH) = ZFWTD
     ENDIF
     !
     !drainage limitation
     ZLIMITK(JI) = MIN(1.0,MAX(0.0,ZWTDL-ZDGNODE)/PK%XDZG(JI,IDEPTH))
     !
     !minimum thickness for WTD coupling (assuming symetric condition if lower wtd)
     ZDZMINS(JI,IDEPTH) = ZDMIN*ZFWTD+(PK%XDG(JI,IDEPTH)-ZDGNODE)*(1.0-ZFWTD)
     ZDZWTD (JI,IDEPTH) = MIN(MAX(ZDZMINS(JI,IDEPTH),ZWTDL-ZDGNODE),PK%XDZG(JI,IDEPTH))
     !
     !matric potential (m):
     ZWORK   = MAX(0.0,(ZWTDL-PK%XDG(JI,IDEPTH))/(PK%XDG(JI,IDEPTH)-ZDGNODE))
     ZPSIWTD(JI) = (KK%XMPOTSAT(JI,IDEPTH)+ZWORK*ZPSI(JI,IDEPTH))/(1.0+ZWORK)
     !
     !Hydraulic conductivity from matric potential (m s-1):
     ZS    = ZPSIWTD(JI)/KK%XMPOTSAT(JI,IDEPTH)
     ZLOG  = (2.0+3.0/KK%XBCOEF(JI,IDEPTH))*LOG(ZS)
     ZKWTD = ZKSAT(JI,IDEPTH)*EXP(-ZLOG)
     ZKIS(JI,IDEPTH) = EXP(LOG(ZK(JI,IDEPTH))*(1.0-0.5*ZFWTD)+LOG(ZKWTD)*0.5*ZFWTD) 
     !
  ENDDO
  !
ENDIF
...
DO JL=1,INL
   DO JI=1,INI
      ...
      IF(JL<IDEPTH)THEN
        ...
        ZNUS(JI,JL) = ZKIS(JI,JL)
        ! 
        ZHEAD (JI,JL) = (ZPSI(JI,JL)-ZPSI       (JI,JL+1)) / PK%XDZDIF(JI,JL)
        ZHEADS(JI,JL) = (ZPSI(JI,JL)-KK%XMPOTSAT(JI,JL  )) / ZDZWTD   (JI,JL)
        !
        !Total Sub-surface soil water fluxes (m s-1): (+ up, - down) using Darcy's
        !Law with an added linear drainage term:
        ZF  = -ZNU (JI,JL)*ZHEAD (JI,JL)-ZKI(JI,JL)
        ZFS = -ZNUS(JI,JL)*ZHEADS(JI,JL)
        !
        ZWFLUX(JI,JL) = (1.0-ZCPLWTD(JI,JL)) * ZF + ZCPLWTD(JI,JL) * ZFS
        !
      ENDIF
      !
   ENDDO
...
DO JI=1,INI
   ...
   !
   ZNU (JI,IDEPTH) = ZKI (JI,IDEPTH)
   ZNUS(JI,IDEPTH) = ZKIS(JI,IDEPTH)
   !
   ZHEAD (JI,IDEPTH) = 0.0
   ZHEADS(JI,IDEPTH) = (ZPSI(JI,IDEPTH)-ZS) / ZDZWTD(JI,IDEPTH)
   !
   !Total Sub-surface soil water fluxes (m s-1): (+ up, - down) using Darcy's
   !Law with an added linear drainage term:
   ZF  = -ZKI(JI,IDEPTH)
   ZFS = -ZNUS(JI,IDEPTH)*ZHEADS(JI,IDEPTH)-ZKIS(JI,IDEPTH)*ZWORK
   !
   ZWFLUX(JI,IDEPTH) = (1.0-ZCPLWTD(JI,IDEPTH)) * ZF + ZCPLWTD(JI,IDEPTH) * ZFS       
   !
ENDDO
!
!
...
DO JL=1,INL
   ...
   DO JI=1,INI
      ...
      IF(JL<IDEPTH)THEN                
        ...
        ZDHEADDT1S = -KK%XBCOEF(JI,JL)*ZPSI(JI,JL)/(PEK%XWG(JI,JL)*ZDZWTD   (JI,JL))
        !
        ZDHEADDT2  = -KK%XBCOEF(JI,JL+1)*ZPSI(JI,JL+1)/(PEK%XWG(JI,JL+1)*PK%XDZDIF(JI,JL))  
        ZDHEADDT2S = 0.0  
        !     
        ZDKDT1  = (2.*KK%XBCOEF(JI,JL)+3.)*ZKI (JI,JL)/(2.0*PEK%XWG(JI,JL))
        ZDKDT1S = (2.*KK%XBCOEF(JI,JL)+3.)*ZKIS(JI,JL)/(2.0*PEK%XWG(JI,JL))
        !
        ZDKDT2  = (2.*KK%XBCOEF(JI,JL+1)+3.)*ZKI(JI,JL)/(2.0*PEK%XWG(JI,JL+1))
        ZDKDT2S = 0.0
        !
        !     Total Flux derrivative terms:
        ZF  = -ZDKDT1 *ZHEAD (JI,JL) - ZNU (JI,JL)*ZDHEADDT1  - ZDKDT1
        ZFS = -ZDKDT1S*ZHEADS(JI,JL) - ZNUS(JI,JL)*ZDHEADDT1S - ZDKDT1S
        ZDFLUXDT1(JI,JL) = (1.0-ZCPLWTD(JI,JL)) * ZF + ZCPLWTD(JI,JL) * ZFS
        !
        ZF  = -ZDKDT2 *ZHEAD (JI,JL) + ZNU (JI,JL)*ZDHEADDT2  - ZDKDT2
        ZFS = -ZDKDT2S*ZHEADS(JI,JL) + ZNUS(JI,JL)*ZDHEADDT2S - ZDKDT2S
        ZDFLUXDT2(JI,JL) = (1.0-ZCPLWTD(JI,JL)) * ZF + ZCPLWTD(JI,JL) * ZFS
        !
      ENDIF
      !
   ENDDO
...
DO JI=1,INI
   ...
   !
   ZDHEADDT1  = 0.0
   ZDHEADDT1S = -KK%XBCOEF(JI,IDEPTH)*ZPSI(JI,IDEPTH)/(PEK%XWG(JI,IDEPTH)*ZDZWTD(JI,IDEPTH))
   !
   ZDHEADDT2  = 0.0
   ZDHEADDT2S = 0.0
   !
   ZDKDT1  = (2.*KK%XBCOEF(JI,IDEPTH)+3.)*ZKI (JI,IDEPTH)/PEK%XWG(JI,IDEPTH)
   ZDKDT1S = (2.*KK%XBCOEF(JI,IDEPTH)+3.)*ZKIS(JI,IDEPTH)/PEK%XWG(JI,IDEPTH)
   ! 
   !Total Flux derrivative terms:
   ZF  = -ZDKDT1
   ZFS = -ZDKDT1S*ZHEADS(JI,IDEPTH) - ZNUS(JI,IDEPTH)*ZDHEADDT1S - ZDKDT1S*ZWORK
   !
   ZDFLUXDT1(JI,IDEPTH) = (1.0-ZCPLWTD(JI,IDEPTH)) * ZF + ZCPLWTD(JI,IDEPTH) * ZFS
...
DO JI=1,INI
   IDEPTH = PK%NWG_LAYER(JI)
   IF(PEK%XWG(JI,IDEPTH)<XWGMIN)THEN
