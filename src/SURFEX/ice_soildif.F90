...
!     #########
      SUBROUTINE ICE_SOILDIF(KK, PK, PEK, PTSTEP, PKSFC_IVEG, PEGI, &
                             PSOILHCAPZ, PICEMELT, PWGI_EXCESS,     &
...
!      Modified    08/2023     Decharme
!                              CHORT='CM6' -> Tuning for CNRM-CM/ESM to prevent 
...
!
REAL, DIMENSION(:), INTENT(IN)      :: PICEMELT
!                                      PICEMELT    = variable used to tunne soil insulation effect into ISBA CEB
!
...
!     *Melt* ice if energy and ice available:
      ZPHASEM(JI,JL) = ZTR(JI)*MIN(PICEMELT(JI)*ZK(JI,JL)*XCI*XRHOLI*MAX(0.0,ZDELTAT(JI,JL)),ZWGIM(JI,JL)*XLMTT*XRHOLW)
!
