...
IF ((IDC==1).AND.(U%LECOSG)) THEN
  CALL INIT_TYPES_PARAM(U)
END IF
...
IF (U%LECOSG) THEN
  ...
  WHERE(XDATA_VEGTYPE(:,8)>0.) XDATA_ROOT_DEPTH(:,8) = 8.0
  ...
  WHERE(XDATA_VEGTYPE(:,4)>0.) XDATA_GROUND_DEPTH(:,4) = 2.0
  ...
  WHERE(XDATA_VEGTYPE(:,9)>0.) XDATA_GROUND_DEPTH(:,9) = 2.0
  WHERE(XDATA_VEGTYPE(:,10)>0.) XDATA_GROUND_DEPTH(:,10) = 3.0
  WHERE(XDATA_VEGTYPE(:,11)>0.) XDATA_GROUND_DEPTH(:,11) = 2.0
  WHERE(XDATA_VEGTYPE(:,12)>0.) XDATA_GROUND_DEPTH(:,12) = 3.0
  WHERE(XDATA_VEGTYPE(:,13)>0.) XDATA_GROUND_DEPTH(:,13) = 1.0
