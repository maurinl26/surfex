...
!!    B. Decharme        2023  : coef GMES, CF, CNA set to old default value for NE and ND
!!    B. Bonan           2023  : coef DMAX in accordance with GMES
!!
!----------------------------------------------------------------------------
!
!*    0.     DECLARATION
...
IF (PRESENT(PGMES_ST)) THEN
  ...
  PGMES_ST(:,NVT_BONE)= 0.0020 ! cm6 via TRY = 0.0019
  PGMES_ST(:,NVT_TENE)= 0.0020 ! cm6 via TRY = 0.0019
  PGMES_ST(:,NVT_BOND)= 0.0020 ! cm6 via TRY = 0.0012
...
IF (PRESENT(PDMAX_ST)) THEN
  !
  PDMAX_ST(:,:) = 0.05
  PDMAX_ST(:,NVT_TEBD)= 0.128  ! 0.109 (to be in accordance with PGMES_ST)
  PDMAX_ST(:,NVT_TRBD)= 0.1435 ! 0.109 (to be in accordance with PGMES_ST)
  PDMAX_ST(:,NVT_TEBE)= 0.126  ! 0.109 (to be in accordance with PGMES_ST)
  PDMAX_ST(:,NVT_BOBD)= 0.128  ! 0.109 (to be in accordance with PGMES_ST)
  PDMAX_ST(:,NVT_SHRB)= 0.1325 ! 0.109 (to be in accordance with PGMES_ST)
  PDMAX_ST(:,NVT_BONE)= 0.124
  PDMAX_ST(:,NVT_TENE)= 0.124
  PDMAX_ST(:,NVT_BOND)= 0.124
  PDMAX_ST(:,NVT_TRBE)= 0.1435 ! 0.124 (to be in accordance with PGMES_ST) 
  IF (NVT_FLTR>0) THEN
     PDMAX_ST(:,NVT_FLTR) = 0.128 ! 0.109 (to be in accordance with PGMES_ST)
  ENDIF
...
IF (PRESENT(PCF_NITRO)) THEN
  ...
  PCF_NITRO(:,NVT_BONE)= -0.24 ! cm6 via TRY = -0.87
  PCF_NITRO(:,NVT_TENE)= -0.24 ! cm6 via TRY = -0.87
  PCF_NITRO(:,NVT_BOND)= -0.24 ! cm6 via TRY = 0.68
...
IF (PRESENT(PCNA_NITRO)) THEN
  ...
  PCNA_NITRO(:,NVT_BONE)= 2.80 ! cm6 via TRY = 1.21
  PCNA_NITRO(:,NVT_TENE)= 2.80 ! cm6 via TRY = 1.21
  PCNA_NITRO(:,NVT_BOND)= 2.80 ! cm6 via TRY = 1.94
  PCNA_NITRO(:,NVT_TRBE)= 1.70 
  !
  IF (NVT_FLTR>0) THEN
