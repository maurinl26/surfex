...
!!      P. Le Moigne 04/2013: Coupling with AGCM
...
USE MODD_FLAKE_PARAMETERS, ONLY : C_T_min, C_T_min2, dC_T_dt_max
!
USE MODN_IO_OFFLINE, ONLY : XTSTEP_SURF
!
...
ALLOCATE(FM%DMF%XZSED_PROFILE(100))
!
...
IF (LNAM_READ) THEN
 ...
 !        0.1. Hard defaults
 !      
 CALL DEFAULT_FLAKE(FM%F%XTSTEP,FM%F%XOUT_TSTEP,FM%F%LSEDIMENTS,FM%F%CFLK_SNOW,&
                    FM%F%CFLK_FLUX,FM%F%CFLK_ALB,FM%F%LSKINTEMP,FM%F%CFLK_CTMIN,&
                    FM%F%XCTROCMAX)
 ...
 CALL DEFAULT_DIAG_FLAKE(FM%DFO%N2M,FM%DFO%LSURF_BUDGET,FM%DFO%L2M_MIN_ZS,FM%DFO%LRAD_BUDGET,       &
                         ...
                         FM%DMF%XZSED_PROFILE,FM%DMF%LSEDIM_PROFILE,FM%DMF%LFLKFLUX,FM%DMF%LFLKWATER)  
 !
ENDIF
!
...
SELECT CASE (HINIT)
  ...
  CASE ('PRE')
    CALL PREP_CTRL_FLAKE(FM%DFO,ILUOUT,FM%DMF%LWATER_PROFILE,FM%DMF%LSEDIM_PROFILE,&
                         FM%DMF%LFLKFLUX,FM%DMF%LFLKWATER)
...
!
FM%F%XCTMIN = C_T_min
IF (FM%F%CFLK_CTMIN=='SLM10') THEN
   FM%F%XCTMIN = C_T_min2
ENDIF

ZTSTEP = XTSTEP_SURF
IF (FM%F%XCTROCMAX==XUNDEF) THEN
   IF (FM%F%CFLK_CTMIN=='SLM10') THEN
      FM%F%XCTROCMAX = dC_T_dt_max
   ELSE
      FM%F%XCTROCMAX = 0.01/ZTSTEP
   ENDIF
ENDIF
!-------------------------------------------------------------------------------
!
