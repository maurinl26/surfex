...
!!    -------------
!!      Original    01/2004
!!      P. Le Moigne      11/2004 : miscellaneous diagnostics
!!      P. Le Moigne      06/2006 : seeding and irrigation
!!      B. Decharme          2008 : SGH and Flooding scheme
!!      B. Decharme       01/2009 : optional deep soil temperature as in Arpege
!!      R. Hamdi          01/2009 : Cp and L
!!      B. Decharme       06/2009 : read topographic index statistics
!!      P. Le Moigne      01/2009 : Beljaars sso
!!      B. Decharme       08/2009 : Active Trip coupling variable if Earth System Model
!!      A.L. Gibelin      04/2009 : change BSLAI_NITRO initialisation
!!      A.L. Gibelin      04/2009 : modifications for CENTURY model
!!      A.L. Gibelin      06/2009 : soil carbon initialisation
!!      B. Decharme       07/2011 : read pgd+prep
!!      R. Alkama         05/2012 : new carbon spinup
!!      J.Escobar         11/2013 : add USE MODI_DEFAULT_CROCUS
!!      B. Decharme       04/2013 : new coupling variables
!!      P. Samuelsson     10/2014 : MEB
!!      P.Tulet           06/2016 : add MEGAN coupling
!!      P. Wautelet       02/2018 : bug correction: allocate some work arrays to 0,1,1 instead of 0,0,1 (crash with XLF)
...
!!      P. Wautelet       11/2019 : initialize YSNOW_SCHEME
!!      S. Donnier        02/2020 : correction for ECOCLIMAP SG (20 vegtypes possible)
!!      J. Colin          08/2016 : Snow and soil moisture nudging
!!      Séférian/Decharme 08/2016 : Change landuse implementation and add Fire and carbon leaching module
!!      R. Séférian       11/2016 : Implement carbon cycle coupling (Earth system model)
!!      B. Decharme       01/2018 : simplify carbon spinup procedure (delete LAGRI_TO_GRASS XCO2_START XCO2_END)
...
IF (LNAM_READ) THEN
 ...
 CALL DEFAULT_DIAG_ISBA(IM%ID%O%N2M, IM%ID%O%LSURF_BUDGET, IM%ID%O%L2M_MIN_ZS, IM%ID%O%LRAD_BUDGET, &
                        ...
                        IM%ID%DM%LVOLUMETRIC_SNOWLIQ, IM%ID%DM%LSPECMOD, IM%ID%O%XDIAG_TSTEP,       &
                        IM%ID%O%LLUTILES_BUDGET                                                     )
 !
 CALL DEFAULT_CROCUS(IM%O%CSNOWDRIFT, IM%O%LSNOWDRIFT_SUBLIM, IM%O%LSNOW_ABS_ZENITH,                 &
                     IM%O%CSNOWMETAMO, IM%O%CSNOWRAD, IM%O%LATMORAD, IM%O%LSNOWSYTRON,               &
                     IM%O%LSNOWPAPPUS, IM%O%CSNOWPAPPUSERODEPO, IM%O%CSNOWFPAPPUS, IM%O%CPAPPUSSUBLI,&
                     IM%O%CSALTPAPPUS, IM%O%CLIMVFALL, IM%O%LPAPPULIMTFLUX, IM%O%LPAPPUDEBUG,        &
                     IM%O%CSNOWFALL, IM%O%CSNOWCOND, IM%O%CSNOWHOLD, IM%O%CSNOWCOMP, IM%O%CSNOWZREF, &
                     IM%O%LSNOWCOMPACT_BOOL, IM%O%LSNOWMAK_BOOL, IM%O%LPRODSNOWMAK,                  &
                     IM%O%LSNOWMAK_PROP, IM%O%LSNOWTILLER, IM%O%LSELF_PROD, IM%O%CSNOWMOB            )
 !
ENDIF
!
...
ENDIF
IF(IM%O%CRESPSL/='CNT'.AND.IM%O%CRESPSL/='DIF'.AND.IM%O%LSPINUPCARBS)THEN
  CALL ABOR1_SFX('INIT_ISBAN: INCONSISTENCY BETWEEN CRESPSL AND LSPINUPCARBS (if not CNT or not DIF must be false)')
ENDIF
...
ENDIF
!
IF(IM%O%CNITRO_DILU=='ESM2'.AND.(.NOT.IM%O%LDOWNREGU))THEN
  WRITE(ILUOUT,*)'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
  WRITE(ILUOUT,*)'INIT_ISBAN: ISBACC INCONSISTENCY -> CNITRO_DILU = ESM2 BUT LDOWNREGU = F'
  WRITE(ILUOUT,*)'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
  CALL ABOR1_SFX('INIT_ISBAN: ISBACC INCONSISTENCY -> CNITRO_DILU = ESM2 BUT LDOWNREGU = F')
ENDIF
!
IF(IM%O%LLULCC_MANAGE.AND.(.NOT.IM%O%LLULCC))THEN
  WRITE(ILUOUT,*)'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
  WRITE(ILUOUT,*)'INIT_ISBAN: LAND USE INCONSISTENCY -> LLULCC_MANAGE = T BUT LLULCC = F'
  WRITE(ILUOUT,*)'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
  CALL ABOR1_SFX('INIT_ISBAN: INIT_ISBAN: LAND USE INCONSISTENCY -> LLULCC_MANAGE = T BUT LLULCC = F')
ENDIF
!
IF(IM%O%LLULCC_MANAGE.AND.IM%O%CPHOTO/='NCB')THEN
  WRITE(ILUOUT,*)'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
  WRITE(ILUOUT,*)'INIT_ISBAN: LAND USE INCONSISTENCY -> LLULCC_MANAGE= T BUT CPHOTO/=NCB'
  WRITE(ILUOUT,*)'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!'
  CALL ABOR1_SFX('INIT_ISBAN: INIT_ISBAN: LAND USE INCONSISTENCY -> LLULCC_MANAGE= T BUT LLULCC = F')
ENDIF
!
...
IF ( IM%NPE%AL(1)%TSNOW%SCHEME .EQ.'CRO' ) THEN
...
IF (.NOT.LSPLIT_PATCH) THEN
  ...
  ALLOCATE(IM%S%XMODISBANDS_WR(KI,NPNBANDS_MODIS, IM%O%NPATCH))
  ALLOCATE(IM%S%XRHO_WR(KI,ISNOW_NLAYER,IM%O%NPATCH))
  ALLOCATE(IM%S%XALB_WR(KI,IM%O%NPATCH))
  ALLOCATE(IM%S%XMMP_WR(KI,IM%O%NPATCH))
  IF (YSNOW_SCHEME=='3-L' .OR. YSNOW_SCHEME=='CRO') THEN
    ...
    IF (YSNOW_SCHEME=='CRO') THEN
      ...
      ALLOCATE(IM%S%XMEPRA_WR(KI,IM%O%NPATCH,6))
      IF ( NIMPUR > 0 ) THEN
...
ELSE
  ...
  ALLOCATE(IM%S%XMODISBANDS_WR(0,1,1))
  ALLOCATE(IM%S%XRHO_WR(0,1,1))
  ALLOCATE(IM%S%XALB_WR(0,1))
  ALLOCATE(IM%S%XMMP_WR(0,1))
  ALLOCATE(IM%S%XHEA_WR(0,1,1))
  ALLOCATE(IM%S%XAGE_WR(0,1,1))
  ALLOCATE(IM%S%XSG1_WR(0,1,1))
  ALLOCATE(IM%S%XSG2_WR(0,1,1))
