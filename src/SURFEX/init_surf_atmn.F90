...
!#############################################################
SUBROUTINE INIT_SURF_ATM_n (YSC, HPROGRAM, HINIT, KI, KSV, KSW, HSV,   &
...
!
!#rustine Temporary double call to solve ECOSG/urban vegetation problem
IF (YSC%U%LECOSG) THEN
...
!    IF (PDIR_ALB(JJ,1)>1.) THEN
    IF (PDIR_ALB(JJ,1)>1..OR.PSCA_ALB(JJ,1)>1.) THEN
     ...
     ! WRITE (*,*) 'init_surf_atmn: Warning incoherent grid point albedo or emissivity or surface temperature'
      ...
      WRITE (*,*) 'JJ',JJ,' LON',YSC%UG%G%XLON(JJ),' LAT',YSC%UG%G%XLAT(JJ)
      WRITE (*,*) 'NATURE',YSC%U%XNATURE(JJ)
