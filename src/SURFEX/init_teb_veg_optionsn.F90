...
!!      B.Marti, A.Boone
!!                   06/2025 : added DSL soil res option
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
LOGICAL                  :: GFIRE, GCLEACH, GADVECT_SOC,   &
                            GCRYOTURB, GBIOTURB, GSOILGAS, &
                            GLULCC_MANAGE
REAL                     :: ZMISSFCO2
!
...
IF (LNAM_READ) THEN
 ...
 CALL DEFAULT_ISBA(GDO%XTSTEP, GDO%XOUT_TSTEP, GDO%CRUNOFF,                            &
                   GDO%CSCOND, GDO%CZ0HEAT, GDO%CC1DRY, GDO%CSOILFRZ,                  &
                   GDO%CDIFSFCOND, GDO%CSNOWRES, GDO%CCPSURF, GDO%XCGMAX,              &
                   GDO%XCDRAG, GDO%XZ0HEAT, GDO%CKSAT, GDO%LSOC, GDO%CRAIN, GDO%CHORT, &
                   GDO%LGLACIER, GDO%LCANOPY_DRAG, GDO%LVEGUPD, GDO%LSPINUPCARBS,      &
                   GDO%XSPINMAXS, GDO%NNBYEARSPINS, GDO%CNITRO_DILU, ZCVHEATF,         &
                   GFIRE, GCLEACH, GADVECT_SOC, GCRYOTURB, GBIOTURB, ZMISSFCO2,        &
                   GDO%XCNLIM, GDO%LDOWNREGU, GLULCC_MANAGE                            )
 !
 ...
 CALL DEFAULT_CROCUS(GDO%CSNOWDRIFT, GDO%LSNOWDRIFT_SUBLIM, GDO%LSNOW_ABS_ZENITH,                &
                     ...
                     GDO%LSNOWPAPPUS, GDO%CSNOWPAPPUSERODEPO, GDO%CSNOWFPAPPUS, GDO%CPAPPUSSUBLI,&
                     GDO%CSALTPAPPUS, GDO%CLIMVFALL, GDO%LPAPPULIMTFLUX, GDO%LPAPPUDEBUG,        &
                     GDO%CSNOWFALL, GDO%CSNOWCOND, GDO%CSNOWHOLD, GDO%CSNOWCOMP, GDO%CSNOWZREF,  &
                     GDO%LSNOWCOMPACT_BOOL, GDO%LSNOWMAK_BOOL, GDO%LPRODSNOWMAK,                 &
                     GDO%LSNOWMAK_PROP, GDO%LSNOWTILLER, GDO%LSELF_PROD, GDO%CSNOWMOB            )
 !
ENDIF
...
GDO%CRUNOFF = 'WSAT'
GDO%CDIFSFCOND = 'MLCH'
GDO%LMEB_TALL_VEG = .FALSE.
GDO%LMEB_INT_PLUN = .FALSE.
GDO%LMEB_INT_ULUN = .FALSE.
GDO%LMEB_INT_USFC = .FALSE.
!-------------------------------------------------------------------------------
!
!         Initialisation for IO
!
...
!
IF (IVERSION==9 .AND. IBUGFIX>=1 .OR. IVERSION>9 )THEN
   YRECFM='GD_GNDRES'
   CALL READ_SURF(HPROGRAM,YRECFM,GDO%CGNDRES,IRESP)
ENDIF
!
...
IF (OGREENROOF) THEN
  ...
  GRO%CNITRO_DILU = GDO%CNITRO_DILU
  GRO%LDOWNREGU = GDO%LDOWNREGU
  !
  GRO%CMEB_GNDRES   = GDO%CMEB_GNDRES
  GRO%LMEB_LITTER   = GDO%LMEB_LITTER
  GRO%LMEB_PATCH(:) = GDO%LMEB_PATCH(:)
  GRO%LMEB_TALL_VEG = GDO%LMEB_TALL_VEG
  GRO%LMEB_INT_PLUN = GDO%LMEB_INT_PLUN
  GRO%LMEB_INT_ULUN = GDO%LMEB_INT_ULUN
  GRO%LMEB_INT_USFC = GDO%LMEB_INT_USFC
  !
  CALL DEFAULT_CROCUS(GRO%CSNOWDRIFT, GRO%LSNOWDRIFT_SUBLIM, GRO%LSNOW_ABS_ZENITH,                &
                      ...
                      GRO%LSNOWPAPPUS, GRO%CSNOWPAPPUSERODEPO, GRO%CSNOWFPAPPUS, GRO%CPAPPUSSUBLI,&
                      GRO%CSALTPAPPUS, GRO%CLIMVFALL, GRO%LPAPPULIMTFLUX, GRO%LPAPPUDEBUG,        &
                      GRO%CSNOWFALL, GRO%CSNOWCOND, GRO%CSNOWHOLD, GRO%CSNOWCOMP, GRO%CSNOWZREF,  &
                      GRO%LSNOWCOMPACT_BOOL, GRO%LSNOWMAK_BOOL, GRO%LPRODSNOWMAK,                 &
                      GRO%LSNOWMAK_PROP, GRO%LSNOWTILLER, GRO%LSELF_PROD, GRO%CSNOWMOB            )
ENDIF
!
!-------------------------------------------------------------------------------
!
