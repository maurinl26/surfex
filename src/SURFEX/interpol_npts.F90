...
REAL, DIMENSION(:), ALLOCATABLE       :: ZX, ZY,ZZX,ZZY
...
INTEGER, DIMENSION(:), ALLOCATABLE     :: ISIZE, ISIZE_TOT,IND
!
REAL    :: ZSUM
INTEGER :: IP, ICPT, IL1, IL2, JL, JP, JK, JKK, IDIM 
!
...
ALLOCATE(ININD(KNPTS,IP))
ININD(:,:) = 0
!distances of the points used for interpolation
ALLOCATE(ZNDIST(0:KNPTS,IP))
...
IF (INEAR_NBR==IDIM_FULL) THEN
  ...
  ALLOCATE(ZDIST (ICOUNT),ZZX(ICOUNT),ZZY(ICOUNT))
  ALLOCATE(IND(IP))
  IND(:)=0
  ...
  ! optim: preset zzx/zzy to avoid multiple indirect access in OMP loop
  IF (IOLD == 1) THEN
    ZZX(:) = PX(IINDEX(1:ICOUNT))
    ZZY(:) = PY(IINDEX(1:ICOUNT))
  ELSE
    ZZX(:) = ZX(IINDEX(1:ICOUNT))
    ZZY(:) = ZY(IINDEX(1:ICOUNT))
  END IF
  !$OMP PARALLEL DO PRIVATE(ZDIST,JP,JK,JKK) IF (IP > 1)
  DO JL=1,IP
    IF (IOLD == 1) THEN
      IF  ( IND(JL) > 0 ) THEN
        ZDIST(1:ICOUNT) = (ZZX(1:ICOUNT)-ZX(IND(JL)))**2+(ZZY(1:ICOUNT)-ZY(IND(JL)))**2
      ELSE
        ZDIST(1:ICOUNT) = (ZZX(1:ICOUNT))**2+(ZZY(1:ICOUNT))**2
      END IF
    ELSE
      ZDIST(1:ICOUNT) = (ZZX(1:ICOUNT)-PX(IND(JL)))**2+(ZZY(1:ICOUNT)-PY(IND(JL)))**2
    END IF

    DO JP = 1,ICOUNT
      IF (ZDIST(JP) > ZNDIST(INPTS,JL)) CYCLE

      ! search for index jk where distance is smaller
      DO JK = INPTS,1,-1
        IF ( ZDIST(JP) <= ZNDIST(JK-1,JL) ) CYCLE

        ! prepare for insertion
        IF ( JK<INPTS ) THEN
          DO JKK = INPTS,JK+1,-1
            ZNDIST(JKK,JL) = ZNDIST(JKK-1,JL)
            ININD(JKK,JL) = ININD(JKK-1,JL)
          ENDDO
        ENDIF

        !distances and indexes of points used to interpolate are saved
        ZNDIST(JK,JL) = ZDIST (JP)
        ININD(JK,JL) = IINDEX(JP)
        EXIT
      ENDDO
    ENDDO
  ENDDO
...
!
  ...
  DO JL=1,IL1
    ...
    DO JP = 1,ICOUNT
      !
      IF (ZDIST(JP)>ZNDIST(INPTS,ICPT)) CYCLE
      !
      DO JK = INPTS,1,-1
        !
        IF ( ZDIST(JP)>ZNDIST(JK-1,ICPT) ) THEN
          !
          IF ( JK<INPTS ) THEN
            DO JKK = INPTS,JK+1,-1
              ZNDIST(JKK,ICPT) = ZNDIST(JKK-1,ICPT)
              ININD(JKK,ICPT) = ININD(JKK-1,ICPT)
            ENDDO
          ENDIF
          !
          !distances and indexes of points used to interpolate are saved
          ZNDIST(JK,ICPT) = ZDIST (JP)
          ININD(JK,ICPT) = IINDEX(JP)
          !
          EXIT
          !
        ENDIF
...
ALLOCATE(ZFIELD(MAXVAL(ISIZE),KNPTS,IL2,0:NPROC-1))
ZFIELD(:,:,:,:) = XUNDEF
DO JP=0,NPROC-1
...
ALLOCATE(ZFIELD2(ICPT,KNPTS,IL2,0:NPROC-1))
IF (NPROC>1) THEN
  DO JP=0,NPROC-1
    IDIM = ISIZE(JP)*KNPTS*IL2
#if defined(SFX_MPI)
    CALL MPI_GATHER(ZFIELD(1:ISIZE(JP),:,:,JP),IDIM*KIND(ZFIELD)/4,MPI_REAL,&
                    ZFIELD2,IDIM*KIND(ZFIELD2)/4,MPI_REAL,JP,NCOMM,INFOMPI)
#elif defined(SFX_MNH)
    CALL MPI_GATHER(ZFIELD(1:ISIZE(JP),:,:,JP),IDIM,MNHREAL_MPI,&
                    ZFIELD2,IDIM,MNHREAL_MPI,JP,NCOMM,INFOMPI)
#endif
  ENDDO
ELSE
  ZFIELD2(:,:,:,:) = ZFIELD(:,:,:,:)
...
ALLOCATE(ZFIELD3(ICPT,KNPTS,IL2))
DO JP=0,NPROC-1
...
DO JL=1,IL1
  ...
  DO JP=1,KNPTS
    IF (ININD(JP,ICPT)/=0) THEN
      ZNVAL(:) = ZNVAL(:) + ZFIELD3(ICPT,JP,:)/ZNDIST(JP,ICPT)
      ZSUM = ZSUM + 1./ZNDIST(JP,ICPT)
    ENDIF
  ENDDO

  IF (ZSUM/=0.) ZNVAL(:) = ZNVAL(:) / ZSUM
