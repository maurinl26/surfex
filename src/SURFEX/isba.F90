...
!     #########
      SUBROUTINE ISBA(IO, KK, PK, PEK, G, AG, DK, DEK, DMK, TPTIME, PPOI, PABC, &
                      PIACAN,OMEB, PTSTEP,PTIMEC, HIMPLICIT_WIND, PZREF, PUREF, &
...
REAL, DIMENSION(SIZE(PEK%XWR))           :: ZT, ZPSN, ZPSNV
...
IF(OMEB)THEN
  ...
  CALL ISBA_MEB(IO, KK, PK, PEK, DK, DEK, DMK, G, AG,                         &
                TPTIME, OMEB, GSHADE, HIMPLICIT_WIND, PTSTEP, PTIMEC,         &
                ZSOILHCAPZ, ZSOILCONDZ, ZFROZEN1, PPS, PZENITH, PAZIM,        &
                ...
                PRESP_BIOMASS_INST, PPALPHAN, PLW_RAD, ZGRNDFLUX,             &
...
ELSE
  ...
  CALL SNOW3L_ISBA(IO, G, PK, PEK, DK, DEK, DMK, OMEB, HIMPLICIT_WIND,         &
                   TPTIME, PTSTEP, PTIMEC, PK%XVEGTYPE_PATCH, PEK%XTG, DMK%XCT,&
                   ZSOILHCAPZ, ZSOILCONDZ(:,1), PPS, PTA, PTA, PSW_RAD, PQA,   &
                   ...
                   ZEVAP3L, ZSNOWSFCH, ZRI3L, PZENITH, PAZIM, ZQS3L,           &
  ...
  ELSE IF (MAXVAL(PEK%XGMES(:)).NE.XUNDEF .OR. MINVAL(PEK%XGMES(:)).NE.XUNDEF) THEN
    ...
    ZPSNV(:) = PEK%XPSNV(:)
    !
    CALL COTWORES(PTSTEP, IO, GSHADE, PK, PEK, PPOI, PCSP, ZT,                       &
                  DMK%XF2, PSW_RAD, PQA, ZQSAT, ZPSN, ZPSNV, ZDELTA, PRHOA, PZENITH, &
