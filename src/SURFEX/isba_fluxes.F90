...
!
! latent heat of sublimation from the ground (W/m2)
!
  PLEGI(JI) = ZWORK1(JI) * PK%XLSTT(JI) * ( PHUI(JI) * ZWORK2(JI) - PQA(JI)) * PFROZEN1(JI) * PLEGI_DELTA(JI)
!
! total latent heat of evaporation from the ground (W/m2)
!
...
!
! latent heat of evaporation from the snow canopy (W/m2)
!
  PLES(JI) = PK%XLSTT(JI) * ZPSN(JI) * PEPOT(JI)
!
! latent heat of total evaporation from vegetation (W/m2)
!
  PLEV(JI) = PK%XLVTT(JI) * PEK%XVEG(JI)*(1.-ZPSNV(JI)) * DMK%XHV(JI) * PEPOT(JI)
!
! latent heat of transpiration (W/m2)
!                                            
...
!
! latent heat of direct evaporation from vegetation canopy (W/m2)
!
  PLER(JI) = PLEV(JI) - PLETR(JI)
!
! latent heat of free water (floodplains) (W/m2)
!
  PLE_FLOOD(JI)  = PK%XLVTT(JI) * (1.-KK%XFFROZEN(JI)) * PEPOT(JI) 
!
  PLEI_FLOOD(JI) = PK%XLSTT(JI) * KK%XFFROZEN(JI) * PEPOT(JI) 
!
! total latent heat of evaporation without flood (W/m2)
!
  PLE(JI) = PLEG(JI) + PLEV(JI) + PLES(JI) + PLEGI(JI)
!
! balance of energy fluxes at the land surface without flood (W/m2)
!
