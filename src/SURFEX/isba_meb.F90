...
!     #########
      SUBROUTINE ISBA_MEB(IO, KK, PK, PEK, DK, DEK, DMK, G, AG,                     &
                          TPTIME, OMEB, OSHADE, HIMPLICIT_WIND, PTSTEP, PTIMEC,     &
                          PSOILHCAPZ, PSOILCONDZ, PFROZEN1, PPS, PZENITH,           &
                          PAZIM, PSCA_SW, PSW_RAD, PVMOD, PVDIR, PRR,               &
                          ...
                          PRESP_BIOMASS_INST, PPALPHAN, PLW_RAD,                    &
                          PGRNDFLUX, PFLSN_COR, PUSTAR, PEMIST, PHU_AGG, PAC_AGG,   &
                          PDELHEATV_SFC, PTDEEP_A, PDEEP_FLUX, PRISNOW,             &
                          PSNOW_THRUFAL, PSNOW_THRUFAL_SOIL, PEVAPCOR,              &
...
!!      B. Decharme    02/2017  exact computation of saturation deficit near the leaf surface
!!      R. Decharme    07/2019  addd many diag for water and energy balance computation 
...
USE MODD_MEB_PAR,          ONLY : XSW_WGHT_VIS, XSW_WGHT_NIR, XH_VEG_INT_STD
...
REAL, DIMENSION(:,:), INTENT(INOUT) :: PIACAN        ! PAR in the canopy at different gauss levels
...
REAL, DIMENSION(:),   INTENT(OUT)   :: PSNOW_THRUFAL ! rate that liquid water leaves (explicit) snow pack: ISBA-ES or CROCUS [kg/(m2 s)]
...
REAL, DIMENSION(SIZE(PEK%TSNOW%WSNOW,1),SIZE(PEK%TSNOW%WSNOW,2)) :: ZSNOWDIAMOPT          ! snow optical diameter (Crocus)
REAL, DIMENSION(SIZE(PEK%TSNOW%WSNOW,1),SIZE(PEK%TSNOW%WSNOW,2)) :: ZSNOWSPHERI           ! snow sphericity (Crocus)
REAL, DIMENSION(SIZE(PEK%TSNOW%WSNOW,1),SIZE(PEK%TSNOW%WSNOW,2)) :: ZSNOWHIST             ! snow historical parameter (Crocus)
...
REAL, DIMENSION(SIZE(PPS))                         :: ZVEGFACT             ! Fraction of canopy vegetation possibly receiving rainfall (-)
...
REAL, DIMENSION(SIZE(PPS))                         :: ZLES3L               ! latent heat flux - sublimation of ice from the ground based snowpack (W/m2)
REAL, DIMENSION(SIZE(PPS))                         :: ZLEL3L               ! latent heat flux - evaporation of liquid water from the ground based snowpack (W/m2))
...
REAL, DIMENSION(SIZE(PPS))                         :: ZLAI_SCALE           ! LAI scaling factor for intercepted snow (-)
!
...
!
REAL, DIMENSION(SIZE(PPS))   :: ZALBV, ZALBG, ZTR, ZQ, ZT
!
...
!
INTEGER :: INJ, INL, INS, JJ, JL, JTYPE, JTYPE2
!
...
!
! M Lafaysse 09/24: ZUREF needed in PREPS_FOR_MEB_EBUD_RAD so this is computed earlier
!
ZZREF(:)       = PZREF(:)
ZUREF(:)       = PUREF(:)
...
ENDIF
!
CALL PREPS_FOR_MEB_EBUD_RAD(IO, PPS, PEK%XLAI, ZSNOWRHO, ZSNOWSWE, PEK%TSNOW%HEAT, &
                            ZSNOWLIQ, DMK%XSNOWTEMP, DMK%XSNOWDZ, ZSNOWCOND,   &
                            ZSNOWHCAP, PEK%TSNOW%EMIS, ZSIGMA_F, ZCHIP, PTSTEP,&
                            PSR, PTA, PVMOD, ZSNOWAGE, ZSNOWDIAMOPT,           &
                            ZSNOWSPHERI, ZSNOWHIST, ZPERMSNOWFRAC,             &
                            PEK%TSNOW%SCHEME, ZUREF, PZ0EFF_MEBN )
!
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
!
...
IF (PEK%TSNOW%SCHEME=="CRO") THEN
  CALL SNOWCROALB_SPECTRAL_BANDS_MEB(ZPERMSNOWFRAC, PEK%TSNOW%ALB, ZSNOWRHO,      &
                                     ZSNOWAGE,PEK%TSNOW%DIAMOPT,PEK%TSNOW%SPHERI, &
...
ELSEWHERE
  ...
  !
  !  For output diag, albedo must be undefined
  !
  DK%XALBT(:) = XUNDEF
  !
END WHERE
...
IF (IO%CPHOTO=='NON') THEN
  ...
  CALL VEG(PSW_RAD, PEK%XTC(:), PEK%XQC(:), PPS, PEK%XRGL(:), PEK%XLAI(:), &
           PEK%XRSMIN(:), PEK%XGAMMA(:), DMK%XF2, DMK%XRS)
  !
  !
ELSE IF (MAXVAL(PEK%XGMES) /= XUNDEF .OR. MINVAL(PEK%XGMES) /= XUNDEF) THEN
  ...
  ZFFV(:)  = 0.0
  !
  !  The saturation deficit near the leaf surface for meb is computed as (1-psn*palphan)*(Qsat(tv)-Qc)
  !
  ZQ(:)    = PEK%XQC(:)
  ZT(:)    = PEK%XTV(:)
  ZQSAT(:) = QSAT(ZT,PPS)
  ZWORK(:) = PEK%XPSN(:)*PPALPHAN(:)
!
  CALL COTWORES(PTSTEP, IO, OSHADE, PK, PEK, PPOI, PCSP, ZT,                               &
                DMK%XF2, PSW_RAD, ZQ, ZQSAT, ZWORK, PPALPHAN, ZDELTA, PRHOA, PZENITH,      &
...
!
ZRSN(:) = DMK%XRS(:)/( 1.0 - MIN(PPALPHAN(:), 1.0 - (DMK%XRS(:)/XRS_MAX)) ) 
!
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
!
...
!              ---------------------------------------------
!
IF (IO%LMEB_TALL_VEG) THEN
  ZLAI_SCALE(:) = MAX(1.0, PEK%XH_VEG(:)/XH_VEG_INT_STD)
ELSE
  ZLAI_SCALE(:) = 1.0 ! no scaling
ENDIF
!
CALL SNOW_LEAVES_FRAC_MEB(IO, ZLAI_SCALE, PEK%XPSN, PPALPHAN, PEK%XWRVN, PEK%XTV, ZCHIP, &
...
LOOP_TIME_SPLIT_EB: DO JDT=1,JTSPLIT_EB
  ...
  !              -----------------------------------------------
  !
   CALL DRAG_MEB(IO, PK, PEK, AT, DMK, DK,                          &
                ZTGL(:,1), ZTA_IC,  ZQA_IC, ZVMOD, ZWSFC, ZWISFC,   &
                ZWSAT(:,1), ZWFC(:,1), KK%XBCOEF(:,1),              &
                KK%XMPOTSAT(:,1), PEXNS, PEXNA, PPS, PRR, PSR,      &
                PRHOA, PZ0G_WITHOUT_SNOW, PZ0_MEBV, PZ0H_MEBV,      &
                PZ0EFF_MEBV, PZ0_MEBN, PZ0H_MEBN, PZ0EFF_MEBN,      &
                ZSNOWSWE(:,1), ZCHIP, ZTSTEP,                       &
                DMK%XRS, ZRSN, PPALPHAN,                            &
                ...
                ZQSATC, ZQSATN, ZDELTAVK, ZLAI_SCALE )
  !
...
!             ------------------------------------------
!
CALL SNOW_LOAD_MEB(IO, PK, PEK, DEK, PTSTEP, PSR, ZWRVNMAX, ZCHEATV, ZMELTVN, &
                   ZVELC, PSUBVCOR, ZCHIP, ZWRMAX)
!
...
!              ----------------------------------------------------------------
!
CALL SNOW3L_ISBA(IO, G, PK, PEK, DK, DEK, DMK, OMEB, HIMPLICIT_WIND, TPTIME,  &
                 PTSTEP, PTIMEC, PK%XVEGTYPE_PATCH,  ZTGL, ZCTSFC, ZSOILHCAPZ,&
                 ZSOILCONDZ(:,1), PPS, PTA, PEK%XTC, DEK%XSWDOWN_GN,          &
                 ...
                 PGRNDFLUX, PFLSN_COR, PEVAPCOR, DEK%XLES, DEK%XLESL,         &
                 ZEVAP3L, PSNOWSFCH, PRISNOW, PZENITH, PAZIM, PQSNOW,         &
...
ZDELHEATG_SFC_SUM(:) = ZDELHEATG_SFC_SUM(:) +   DEK%XDELHEATG_SFC(:) 
ZDELHEATG_SUM(:)     = ZDELHEATG_SUM(:)     +   DEK%XDELHEATG(:) 
!
...
DEK%XDELHEATG_SFC(:) = ZDELHEATG_SFC_SUM(:) /JTSPLIT_EB 
DEK%XDELHEATG(:)     = ZDELHEATG_SUM(:)     /JTSPLIT_EB 
!
...
USE MODD_SNOW_METAMO,    ONLY : XSNOWDZMIN
!
USE MODE_SNOWCRO,        ONLY : SNOWCROALB
...
REAL, DIMENSION(:,:), INTENT(IN)    :: PSNOWSPHERI
...
!                                                        MEB currently uses 2)
!                                                        1=VIS, 2=NIR, 3=UV
REAL:: ZDEFAULTDIAMOPT,ZDEFAULTSPHER
!
...
DO JJ = 1,SIZE(PSNOWDZ(:,:),2)
  DO JI = 1,SIZE(PSNOWDZ(:,:),1)
    ...
    ELSE
      ...
      ZSNOWRHO(JI,JJ)=400.
      ZSNOWAGE(JI,JJ)=10.
      ZSNOWDIAMOPT(JI,JJ)=ZDEFAULTDIAMOPT
      ZSNOWSPHERI(JI,JJ)=ZDEFAULTSPHER
    ENDIF
...
IF ( (HSNOWRAD=="T17")) THEN
  ...
  ZSNOWB0 = XPSNOWB0
    ...
    ZSNOWALB=PSNOWALB
    CALL SNOWCRO_TARTES(PSNOWDIAMOPT, PSNOWSPHERI, PSNOWRHO, PSNOWDZ,    &
                        ZSNOWG0, ZSNOWY0, ZSNOWW0, ZSNOWB0, PSNOWIMPUR,  &
                        PSOILALB, ZSW_RAD, PZENITH, PAZIM, PDIRCOSZW,    &
                        PSLOPE_DIR, INLVLS_USE, ZSNOWALB,ZRADSINK,       &
                        ZRADXS, .FALSE., HSNOWMETAMO, P_DIR_SW, P_SCA_SW,&
                        ZSNOWALB_SP, ZSPEC_DIR, ZSPEC_DIF, OATMORAD,     &
                        PSNOWALBVIS)
    !
    ! We diagnose NIR albedo such that total albedo is conserved
    ! (using just 2 spectral bands in MEB)
...
ELSE
    ...
    ZWORKA(:)        = PSNOWALB(:)
    !
    CALL SNOWCROALB(ZWORKA,ZSPECTRALALBEDO,PSNOWDZ(:,1),ZSNOWRHO(:,1:2),       &
...
DO JJ = 1,INLVLS
  DO JI= 1,INI
    CALL GET_DIAM(PSNOWDIAMOPT(JI,JJ),PSNOWSPHERI(JI,JJ),ZDSGRAIN(JI,JJ))
  ENDDO    ! end loop snow layers
ENDDO

!PRINT*,PSNOWDIAMOPT(:,:),PSNOWSPHERI(:,:),ZDSGRAIN(:,:)
!
