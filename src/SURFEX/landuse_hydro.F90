...
!     #########
    SUBROUTINE LANDUSE_HYDRO(IO, S, NK, NP, NPE, NPGLO, NPEGLO, KI)
!   ###############################################################
!!****  *LAND USE HYDRO*
!!
!!    PURPOSE
...
!
INTEGER  :: INL, INP, INS, IDEPTH, JGLO
INTEGER  :: JI, JL, JP
!
...
DO JP=1,INP
   ...
   DO JL=1,INL
      DO JGLO=1,KI
         IF(PEKGLO%XWG(JGLO,JL)/=XUNDEF.AND.PKGLO%XDG(JGLO,JL)/=XUNDEF) THEN
           ZWG_OLD (JGLO) = ZWG_OLD (JGLO) + PKGLO%XPATCH(JGLO)*PKGLO%XDZG(JGLO,JL)*PEKGLO%XWG (JGLO,JL)*XRHOLW
           ZWGI_OLD(JGLO) = ZWGI_OLD(JGLO) + PKGLO%XPATCH(JGLO)*PKGLO%XDZG(JGLO,JL)*PEKGLO%XWGI(JGLO,JL)*XRHOLW
         ENDIF
      ENDDO
   ENDDO
ENDDO
...
DO JP=1,INP
   ...
   DO JL=1,INS
      DO JGLO=1,KI
         ZSWE_OLD(JGLO) = ZSWE_OLD(JGLO) + PKGLO%XPATCH(JGLO)*PEKGLO%TSNOW%WSNOW(JGLO,JL)
      ENDDO
   ENDDO
ENDDO
!
DO JP=1,INP
   ...
   PEKGLO => NPEGLO%AL(JP)
   DO JGLO=1,KI
      ZWR_OLD(JGLO) = ZWR_OLD(JGLO) + PKGLO%XPATCH(JGLO)*PEKGLO%XWR(JGLO)
   ENDDO
ENDDO
!
IF(ISIZE_LMEB_PATCH>0)THEN
  DO JP=1,INP
     ...
     PEKGLO => NPEGLO%AL(JP)
     DO JGLO=1,KI
        ZWR_OLD(JGLO) = ZWR_OLD(JGLO) + PKGLO%XPATCH(JGLO)*(PEKGLO%XWRL(JGLO)+PEKGLO%XWRLI(JGLO)+PEKGLO%XWRVN(JGLO))
     ENDDO
  ENDDO
ENDIF
!
      ...
      DO JI=1,PK%NSIZE_P
         ...
         IF(PK%NWG_LAYER(JI)/=NUNDEF.AND.JL<=PK%NWG_LAYER(JI))THEN
           ...
           IF(PEK%XWG(JI,JL)==XUNDEF) THEN
             ...
             ZMATPOTN = KK%XMPOTSAT(JI,IDEPTH)*EXP(-ZLOG)
             !
   ...
   DO JL=1,INL
      DO JI=1,PK%NSIZE_P
         IF(PEK%XWG(JI,JL)/=XUNDEF.AND.PK%XDG(JI,JL)/=XUNDEF) THEN
           JGLO = PK%NR_P(JI)
           ZWG (JGLO) = ZWG (JGLO) + PK%XPATCH(JI)*PK%XDZG(JI,JL)*PEK%XWG (JI,JL)*XRHOLW
           ZWGI(JGLO) = ZWGI(JGLO) + PK%XPATCH(JI)*PK%XDZG(JI,JL)*PEK%XWGI(JI,JL)*XRHOLW
         ENDIF
      ENDDO
   ENDDO
ENDDO
...
DO JP=1,INP
   ...
   DO JL=1,INS
      DO JI=1,PK%NSIZE_P
         JGLO = PK%NR_P(JI)
         ZSWE(JGLO) = ZSWE(JGLO) + PK%XPATCH(JI)*PEK%TSNOW%WSNOW(JI,JL)
      ENDDO
   ENDDO
ENDDO
!
DO JP=1,INP
   ...
   DO JI=1,PK%NSIZE_P
      JGLO = PK%NR_P(JI)
      ZWR(JGLO) = ZWR(JGLO) + PK%XPATCH(JI)*PEK%XWR(JI)
   ENDDO
ENDDO
!
IF(ISIZE_LMEB_PATCH>0)THEN
  DO JP=1,INP
     ...
     DO JI=1,PK%NSIZE_P
        JGLO = PK%NR_P(JI)
        ZWR(JGLO) = ZWR(JGLO) + PK%XPATCH(JI)*(PEK%XWRL(JI)+PEK%XWRLI(JI)+PEK%XWRVN(JI))
     ENDDO
  ENDDO
ENDIF
!
