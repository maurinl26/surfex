...
!
  ...
  LOGICAL :: LCROCO                              ! B. Cluzet flag for snow assimilation activation
  LOGICAL :: LSPECMOD                            ! B. Cluzet flag fot snow modis spectral bands diagnostic
...
!
  ...
  REAL, POINTER, DIMENSION(:,:)   :: XSNOWHIST      ! snow historic variable (Crocus)
...
!
  REAL, POINTER, DIMENSION(:)     :: XSNDPT_12H, XSNDPT_1DY, XSNDPT_3DY, XSNDPT_5DY, XSNDPT_7DY ! fresh snow depth in 1, 3, 5, 7 days
...
!                                               ! content
  ...
  REAL, POINTER, DIMENSION(:)     :: XRGSOIL    ! barsoil evap resistance                        (s/m)
  REAL, POINTER, DIMENSION(:)     :: XRGSOILI   ! barsoil sublim resistance                      (s/m)
!!
! Crocus
!
  REAL, POINTER, DIMENSION(:,:)   :: XSPEC_ALB ! spectral snow albedo
  REAL, POINTER, DIMENSION(:,:)   :: XSPECMOD  ! spectral snow albedo in MODIS bands only (for assim)
  REAL, POINTER, DIMENSION(:,:)   :: XDIFF_RATIO ! spectral diffuse to total ratio
  REAL, POINTER, DIMENSION(:,:)   :: XSPEC_TOT ! total incident SW radiation reparted in each spectral band
  REAL, POINTER, DIMENSION(:,:,:) :: XIMPUR_CONC ! Impurity concentration in g/g
  REAL, POINTER, DIMENSION(:)     :: XQ_OUT_SALT ! transport rate of saltating snow (KG/(M.S))
  REAL, POINTER, DIMENSION(:)     :: XQ_OUT_SUSP ! transport rate in suspension (kg/m/s)
  REAL, POINTER, DIMENSION(:)     :: XVFRIC_PAPPUS ! wind friction velocity computed by Snowpappus (m/s)
  REAL, POINTER, DIMENSION(:)     :: XVFRIC_T_PAPPUS !threshold friction velocity (at ground level) for snow transport (m/s)
  REAL, POINTER, DIMENSION(:)     :: XPZ0_PAPPUS ! roughness length for momentum (m) used by Snowpappus
  REAL, POINTER, DIMENSION(:)     :: XVFALL_PAPPUS ! mass averaged terminal fall velocity of snow particles at the bottom of the suspension layer (m/s)
  REAL, POINTER, DIMENSION(:)     :: XJJ ! MPI OUTPUT, LOCAL NUMBER OF POINTS
  REAL, POINTER, DIMENSION(:)     :: XNRANK ! MPI OUTPUT, PROCESSOR RANK
  REAL, POINTER, DIMENSION(:)     :: XILOCNIY ! MPI OUTPUT, LOCAL NUMBER OF POINT LINE FOR CURRENT PROCESSOR
  REAL, POINTER, DIMENSION(:)     :: XSIZE_TASK ! MPI OUTPUT, NUMBER OF POINT OF PROCESSOR NSIZE_TASK(NRANK)
  REAL, POINTER, DIMENSION(:)     :: XWBT    ! Wet bulb temperature (°C)
!
...
IF (LHOOK) CALL DR_HOOK("MODD_DIAG_MISC_ISBA_N:DIAG_MISC_ISBA_INIT",0,ZHOOK_HANDLE)
  ...
  NULLIFY(DMI%XRS)
  NULLIFY(DMI%XRGSOIL)
  NULLIFY(DMI%XRGSOILI)
  NULLIFY(DMI%XSPEC_ALB)
  NULLIFY(DMI%XSPECMOD)
  NULLIFY(DMI%XDIFF_RATIO)
  NULLIFY(DMI%XSPEC_TOT)
  NULLIFY(DMI%XIMPUR_CONC)
  NULLIFY(DMI%XQ_OUT_SALT)
  NULLIFY(DMI%XQ_OUT_SUSP)
  NULLIFY(DMI%XVFRIC_PAPPUS)
  NULLIFY(DMI%XVFRIC_T_PAPPUS)
  NULLIFY(DMI%XPZ0_PAPPUS)
  NULLIFY(DMI%XVFALL_PAPPUS)
  NULLIFY(DMI%XJJ)
  NULLIFY(DMI%XNRANK)
  NULLIFY(DMI%XILOCNIY)
  NULLIFY(DMI%XSIZE_TASK)
  NULLIFY(DMI%XWBT)
...
DMI%LCROCO            = .FALSE. ! B.Cluzet
DMI%LSPECMOD          = .FALSE. ! B.Cluzet
