...
REAL, POINTER, DIMENSION(:,:)     :: XINCR_PF ! here because not by patch in this case
!INTEGER, POINTER, DIMENSION(:,:,:)    :: NPART  ! BC17/01 particles to replicate, eq. NPFPART  (PF), dims pts (=1 if SD), patches(=1always), nmbrs
INTEGER, POINTER, DIMENSION(:,:)    :: NPART  ! BC 17/01 for SD case ONLY -WRITSUFN2 doesn't exist yet WTF !
!
REAL, POINTER, DIMENSION(:,:) :: XWORK_WR
!
! Following variables are necessary to store snow variables across a loop on patches in write_surf_isban
! before being written in PREP files with Number_of_patches dimension
REAL, POINTER, DIMENSION(:,:,:) :: XWSN_WR
REAL, POINTER, DIMENSION(:,:,:) :: XBANDS_WR
REAL, POINTER, DIMENSION(:,:,:) :: XMODISBANDS_WR
...
REAL, POINTER, DIMENSION(:,:)   :: XMMP_WR
REAL, POINTER, DIMENSION(:,:,:,:) :: XIMP_WR
REAL, POINTER, DIMENSION(:,:,:) :: XMEPRA_WR
!
TYPE(DATE_TIME), POINTER, DIMENSION(:,:) :: TDATE_WR
!
! - Land-use Land cover change flux (ISBA-CC, LLULCC = T)
!
TYPE(DATE_TIME)                  :: TLULCC        ! Land use date counter
!
REAL, POINTER, DIMENSION(:)      :: XWCONSRV       ! Land use water conservation (kg/m2/s, updated each year)
REAL, POINTER, DIMENSION(:)      :: XCCONSRV       ! Land use carbon conservation (kgC/m2/s, updated each year)
!
REAL, POINTER, DIMENSION(:)      :: XFLUCLEARTOATM ! carbon emitted into the atmosphere by LU clearance (kgC m-2 s-1,updated each year)
REAL, POINTER, DIMENSION(:)      :: XFHARVESTTOATM ! carbon emitted into the atmosphere by harvesting (kgC m-2 s-1,updated each day)
!
...
NULLIFY(YISBA_S%XPATCH)
NULLIFY(YISBA_S%XVEGTYPE_PATCH)
!
...
NULLIFY(YISBA_S%XMEPRA_WR)
!
NULLIFY(YISBA_S%TDATE_WR)
!
NULLIFY(YISBA_S%XWCONSRV)
NULLIFY(YISBA_S%XCCONSRV)
NULLIFY(YISBA_S%XFLUCLEARTOATM)
NULLIFY(YISBA_S%XFHARVESTTOATM)
NULLIFY(YISBA_S%XFLUATM)
NULLIFY(YISBA_S%XFLURES)
NULLIFY(YISBA_S%XFLUANT)
NULLIFY(YISBA_S%XFANTATM)
NULLIFY(YISBA_S%XEXPORT_DECADAL)
NULLIFY(YISBA_S%XEXPORT_CENTURY)
NULLIFY(YISBA_S%XCSTOCK_DECADAL)
NULLIFY(YISBA_S%XCSTOCK_CENTURY)
!
