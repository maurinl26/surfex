...
MODULE MODD_TYPE_SNOW
  ...
  TYPE SURF_SNOW
    ...
    REAL, DIMENSION(:,:), POINTER :: WSNOW       ! snow (& liq. water) content (kg/m2)                            Crocus
    REAL, DIMENSION(:,:), POINTER :: HEAT        ! heat content                (J/m3)                             Crocus
    REAL, DIMENSION(:,:), POINTER :: T           ! temperature '1-L'
    REAL, DIMENSION(:,:), POINTER :: TEMP        ! temperature '3-L' (K)
    REAL, DIMENSION(:,:), POINTER :: RHO         ! density                    (KG/m3)                             Crocus
    ...
    REAL, DIMENSION(:,:), POINTER :: DIAMOPT     ! snow grain parameter 1: optical diameter                       Crocus
    REAL, DIMENSION(:,:), POINTER :: SPHERI      ! snow grain parameter 2: sphericity                             Crocus
    REAL, DIMENSION(:,:), POINTER :: HIST        ! snow historical variable (non dendritic case)                  Crocus
    REAL, DIMENSION(:),   POINTER :: MMP         ! water consumption for snowmaking                               Crocus
    REAL, DIMENSION(:,:), POINTER :: AGE         ! snow grain age   (days)(unity-source:Vionnet2012)              Crocus
    REAL, DIMENSION(:)  , POINTER :: DEP_SUP     ! snow depth in superior profile                                 Crocus
    REAL, DIMENSION(:)  , POINTER :: DEP_TOT     ! total snow depth (m)                                           Crocus
    REAL, DIMENSION(:)  , POINTER :: DEP_HUM     ! height of the uppest continuous block of humid snow in the sup Crocus
    REAL, DIMENSION(:)  , POINTER :: NAT_LEV     ! natural risk index (0-6)                                       Crocus
    REAL, DIMENSION(:)  , POINTER :: PRO_SUP_TYP ! type of superior profile (0, 4, 5, 6)                          Crocus
    REAL, DIMENSION(:)  , POINTER :: AVA_TYP     ! type of avalanche (0-6)                                        Crocus
    REAL, DIMENSION(:,:,:), POINTER :: IMPUR ! impurity content with new dim                                      Crocus
  END TYPE SURF_SNOW
  !
  TYPE NSURF_SNOW
    !
...
CONTAINS
  ...
  SUBROUTINE TYPE_SNOW_INIT(YSURF_SNOW)
    ...
    YSURF_SNOW%TS          => NULL()
    YSURF_SNOW%DIAMOPT     => NULL()
    YSURF_SNOW%SPHERI      => NULL()
