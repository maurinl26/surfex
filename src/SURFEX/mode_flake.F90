...
SUBROUTINE flake_driver ( depth_w, depth_bs, T_bs, par_Coriolis,       &
                            extincoef_water_typ, pctmin, pctrocmax,    &
...
! 1.00       2005/11/17 Dmitrii Mironov 
!  Initial release
! !V9.0.0!  !01/2025!     <P Le Moigne>
!  <Set the minimum shape factor C_T_min depending on the CFLK_CTMIN option>
!  <Add the maximum C_T rate of change>
!
! Code Description:
! Language: Fortran 90.
! Software Standards: "European Standards for Writing and
...
REAL, INTENT(IN) ::   &
    ...
    pctmin                            ,  &! Minimum Ct value
    pctrocmax                         ,  &! Maximum Ct rate of change [s^{-1}]
...
REAL :: &
    ...
    ZC_T_MIN                ! Minimum value of the shape factor
REAL(KIND=JPRB) :: ZHOOK_HANDLE
...
HTC_Water: IF(h_ice_n_flk.GE.h_Ice_min_flk) THEN    ! Ice exists
  ...
  IF(l_ice_create) THEN                  ! Ice has just been created 
    IF(h_ML_p_flk.GE.depth_w-h_ML_min_flk) THEN    ! h_ML=D when ice is created 
      ...
      C_T_n_flk = ZC_T_min                    ! Set C_T to its minimum value 
  ...
  ELSE                                   ! Ice exists and T_bot = T_r, convection due to bottom heating 
    ...
    ELSE                                 ! h_ML = 0 
      ...
      C_T_n_flk = MIN(C_T_max, MAX(C_T_n_flk, ZC_T_min)) ! Update the shape factor (thermocline)  
    END IF 
  END IF 
...
! The rate of change of C_T
  ...
  d_C_T_dt = (C_T_max-ZC_T_min)/MAX(d_C_T_dt, c_small_flk)     ! Rate-of-change of C_T 
  d_C_T_dt = min(d_C_T_dt,pctrocmax)
...
! using different formulations for convection and wind mixing
  ...
  Mixing_regime: IF(flk_str_1.LT.0.) THEN  ! Convective mixing 
    ...
    C_T_n_flk = MIN(C_T_max, MAX(C_T_n_flk, ZC_T_min))                ! Limit C_T 
