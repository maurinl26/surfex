...
!$OMP DO SCHEDULE(STATIC,ISIZE_OMP) PRIVATE(JI,ZXI,ZYI,ZATA,ZRO2,ZT2,ZWORK)
  DO JI = 1,SIZE(ZY)
...
REAL,ALLOCATABLE :: ZRO(:),ZGA(:)
REAL :: ZXPR,ZYPR,ZXY
!
INTEGER :: J, NP, ISIZE_OMP
!
REAL(KIND=JPRB) :: ZHOOK_HANDLE, ZHOOK_HANDLE_OMP
!
!
...
IF (LHOOK) CALL DR_HOOK('MODE_GRIDTYPE_CONF_PROJ:XY_CONF_PROJ_2',1,ZHOOK_HANDLE)
  ZXY = XRADIUS/ZRPK*ABS(ZCLAT0)**(1-ZRPK)

  NP = SIZE(PLAT)
  ALLOCATE(ZRO(NP),ZGA(NP))

  IF (PRPK < 0) THEN
    ZRO(:) = ZXY*((1+ZSLAT0)*ABS(COS(-ZRDSDG*PLAT))/(1+SIN(-ZRDSDG*PLAT)))**ZRPK
    ZGA(:) = (ZRPK*(PLON+NINT((PLON0-PLON)/360)*360+180-ZLON0)-ZBETA)*ZRDSDG
  ELSE
    ZRO(:) = ZXY*((1+ZSLAT0)*ABS(COS(ZRDSDG*PLAT))/(1+SIN(ZRDSDG*PLAT)))**ZRPK
    ZGA(:) = (ZRPK*(PLON+NINT((PLON0-PLON)/360)*360-ZLON0)-ZBETA)*ZRDSDG
  END IF

  DO J=1,SIZE(PLAT)
    PX(J) = ZXP+ZRO(j)*SIN(ZGA(j))
    PY(J) = ZYP-ZRO(j)*COS(ZGA(j))
  ENDDO

  IF (PRPK<0.) PY(1:SIZE(PLAT))=-PY(1:SIZE(PLAT))
  DEALLOCATE(ZRO,ZGA)
  !
!-------------------------------------------------------------------------------
!
