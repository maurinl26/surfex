...
ELSE
  CALL LFILEC (KRESP, NUNIT_FA, CPREFIX1D//TRIM(HREC), ZWORK8, 1)
  PFIELD=ZWORK8
  IF (KRESP/=0) CALL ERROR_READ_SURF_FA(HREC,KRESP)
ENDIF
!
...
ENDIF
!
! FOR READING COMPACTED FILES (NVGRIBSFX=123)
IF (NRANK==NPIO) WHERE (PFIELD(:)>XUNDEF) PFIELD=XUNDEF
!
DEALLOCATE(ZWORK)
!
...
!
  ...
  IF (KRESP/=0) CALL ERROR_READ_SURF_FA(HREC,KRESP)
ELSE
  CALL LFILEC (KRESP, NUNIT_FA, CPREFIX1D//TRIM(HREC), IWORK, 1)
  IF (KRESP/=0) CALL ERROR_READ_SURF_FA(HREC,KRESP)
  KFIELD = IWORK
  IF (HREC == 'DIM_FULL') THEN
    CALL READ_SURFC0_FA ("GRID_TYPE    ", YGRID, KRESP, YCOMMENT)
    LMNH_COMPATIBLE = (YGRID=="CARTESIAN   " .OR. YGRID=="CONF PROJ   ")
!
    IF (LMNH_COMPATIBLE) THEN
      CALL READ_SURFN0_FA ("IMAX        ", IIMAX, KRESP, YCOMMENT)
      NIU = IIMAX+2
      NIB = 2
      NIE = IIMAX+1
      CALL READ_SURFN0_FA ("JMAX        ", IJMAX, KRESP, YCOMMENT)
      NJU = IJMAX+2
      NJB = 2
      NJE = IJMAX+1
    END IF
...
!
REAL   :: XTIME0
INTEGER (KIND=8), ALLOCATABLE :: IWORK2 (:)
REAL(KIND=JPRB) :: ZHOOK_HANDLE
!
...
IF (NRANK==NPIO) THEN
  ...
  ELSE
    ...
    ALLOCATE (IWORK (KL))
    ALLOCATE (IWORK2 (KL))
    CALL LFILEC (KRESP, NUNIT_FA, CPREFIX1D//TRIM(HREC), IWORK2, KL)
    IF (KRESP/=0) CALL ERROR_READ_SURF_FA(HREC,KRESP)
    IWORK = IWORK2
  ENDIF
  !
...
IF (.NOT. LFAGMAP) THEN
  ...
  ENDIF
  !
  CALL FALIT_C(KRESP,NUNIT_FA,YNAME,NCAR_FILES,YFIELD)
  IF (KRESP/=0) CALL ERROR_READ_SURF_FA(HREC,KRESP)
ELSE
  N = NCAR_FILES
  ALLOCATE (IWORK (N))
  ...
  IF (KRESP/=0) CALL ERROR_READ_SURF_FA(HREC,KRESP)
  DO J = 1, N
    YFIELD (J:J) = CHAR (IWORK (J))
  ENDDO
ENDIF
WRITE(YSIZE_CAR,"(I3.2)") NCAR_FILES
WRITE(HFIELD,'('//TRIM(ADJUSTL(YSIZE_CAR))//'A1)') YFIELD(:)
!
YCOMMENT = YNAME
HCOMMENT = YCOMMENT
...
IF (NRANK==NPIO) THEN
  ...
  ELSE
    ALLOCATE (IWORK (KL))
    CALL LFILEC (KRESP, NUNIT_FA, CPREFIX1D//TRIM(HREC), IWORK, KL)
    IF (KRESP/=0) CALL ERROR_READ_SURF_FA(HREC,KRESP)
    OFIELD = IWORK /= 0
  ENDIF
  !
...
!
  ...
  IF (KRESP/=0) CALL ERROR_READ_SURF_FA(HREC,KRESP)
ELSE
  CALL LFILEC (KRESP, NUNIT_FA, TRIM(CPREFIX1D)//HREC, IWORK, SIZE (IWORK))
  IF (KRESP/=0) CALL ERROR_READ_SURF_FA(HREC,KRESP)
  KYEAR  = INT (IWORK (1))
  KMONTH = INT (IWORK (2))
  KDAY   = INT (IWORK (3))
  PTIME  = TRANSFER (IWORK (4), PTIME)
ENDIF
!
YCOMMENT = TRIM(HREC)
HCOMMENT = YCOMMENT
