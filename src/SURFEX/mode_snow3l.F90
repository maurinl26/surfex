...
END INTERFACE
!
INTERFACE CONVERTFROMDIAMOPT
  MODULE PROCEDURE CONVERTFROMDIAMOPT
END INTERFACE
!
INTERFACE CONVERT2DIAMOPT
  MODULE PROCEDURE CONVERT2DIAMOPT
END INTERFACE
!-------------------------------------------------------------------------------
CONTAINS
!
...
!
REAL, DIMENSION(SIZE(PSNOWDIAMOPT,1)) :: ZDIAMOPT, ZSPHERI, ZHIST
!
...
!
USE MODD_SNOW_METAMO, ONLY : XVGRAN1, XVDIAM6, XUEPSI
...
ELSE
  ...
  !  ZDIFTYPE = ABS( (PGRAIN3-PGRAIN4)*XVGRAN1 ) * .5 + &
  !             ABS( ( (PGRAIN1/XVDIAM6 - 4. + PGRAIN3) / (PGRAIN3 - 3.) - &
  !                    (PGRAIN2/XVDIAM6 - 4. + PGRAIN4) / (PGRAIN4 - 3.) ) * XVGRAN1 ) * .5

  !ELSE
  !  ZDIFTYPE = ABS( (PGRAIN3-PGRAIN4)*XVGRAN1 )      + ABS( ZCOEF3-ZCOEF4 ) * 5. * 10000.
  !ENDIF
  !
ENDIF
!
...
SUBROUTINE GET_MASS_HEAT(KJ,KNLVLS_NEW,KNLVLS_OLD,                                &
                         ...
                         PSNOWRHOO,PSNOWDZO,PSNOWDIAMOPTO,PSNOWSPHERIO,PSNOWHISTO,   &
                         PSNOWAGEO,PSNOWIMPURO,PSNOWHEATO,                        &
                         PSNOWRHON,PSNOWDZN,PSNOWDIAMOPTN,PSNOWSPHERIN,PSNOWHISTN,   &
                         PSNOWAGEN, PSNOWIMPURN,PSNOWHEATN)
!
...
REAL, DIMENSION(:), INTENT(IN) :: PSNOWRHOO, PSNOWDZO, PSNOWDIAMOPTO, PSNOWSPHERIO, &
                                  PSNOWHISTO, PSNOWAGEO, PSNOWHEATO
REAL, DIMENSION(:,:), INTENT(IN) :: PSNOWIMPURO
REAL, DIMENSION(:), INTENT(IN) :: PSNOWDZN
REAL, DIMENSION(:), INTENT(OUT) :: PSNOWRHON, PSNOWDIAMOPTN, PSNOWSPHERIN, &
...
!####################################################################
SUBROUTINE GET_DIAM(PSNOWDIAMOPT,PSNOWSPHERI,PDIAM)
!
...
IMPLICIT NONE
!
REAL, INTENT(IN) :: PSNOWDIAMOPT
REAL, INTENT(IN) :: PSNOWSPHERI
...
INTEGER                             :: INI
INTEGER                             :: INLVLS
INTEGER, DIMENSION(SIZE(PSNOWRHO,1),SIZE(PSNOWRHO,2)) :: JWORK
!
...
!####################################################################
!####################################################################
SUBROUTINE CONVERT2DIAMOPT(PSNOWGRAN1I,PSNOWGRAN2I,PDIAMOPT,PSPHERI)
!
!!    PURPOSE
!!    -------
...
!!    METHOD
!!    -------
!!    Based on C. Carmagnola 2012 and work from M. Laffaysse in in snowcro_diag.F90
!!
!!    AUTHOR
!!    ------
...
IF (PSNOWGRAN1I < -XUEPSI) THEN
    ...
    ZGRAN1 = XVDIAM6 * ( PSNOWGRAN1I/XVGRAN1 * (- PSNOWGRAN2I/XVGRAN1 +3.) + 4. - PSNOWGRAN2I/XVGRAN1)
    ZGRAN2 = PSNOWGRAN2I/XVGRAN1
ELSE
    !* Non dendritic case => sphericity, size of grains to Diam_opt, sphericity
    ZGRAN1 = PSNOWGRAN2I * PSNOWGRAN1I/XVGRAN1 + (1. -PSNOWGRAN1I/XVGRAN1)*MAX(XVDIAM1, PSNOWGRAN2I /2.)
    ZGRAN2 = PSNOWGRAN1I/XVGRAN1
ENDIF

PDIAMOPT = ZGRAN1
PSPHERI = ZGRAN2

END SUBROUTINE CONVERT2DIAMOPT
!####################################################################
!####################################################################
SUBROUTINE CONVERTFROMDIAMOPT(PDIAMOPT,PSPHERI,PSNOWGRAN1F,PSNOWGRAN2F)
!
!!    PURPOSE
!!    -------
!!    Metamorphism can be based on (diam_opt,sphericity) or (dendricity,sphericity)/(sphericity,grain size)
...
!!    METHOD
!!    -------
!!    Based on C. Carmagnola 2012 and work from M. Laffaysse in snowcro_diag.F90
!!
!!    AUTHOR
!!    ------
!!    M. Fructus  * Meteo-France *   09/2018
...
IF (PDIAMOPT<XVDIAM6*(4.-PSPHERI)-XUEPSI) THEN
    ...
    ZGRAN1 = -XVGRAN1 * MAX(0.,MIN(1.,(PDIAMOPT/XVDIAM6 -4. + PSPHERI)/(PSPHERI-3.)))
    ZGRAN2 = XVGRAN1 * PSPHERI
ELSE
    !* non dendritic snow => conversion Diam_opt, sphericity to sphericity, size of grains
    ZGRAN1 = XVGRAN1 * PSPHERI
    IF (PDIAMOPT - XVDIAM1*(1. + PSPHERI) >= 0.) THEN
        ZGRAN2 = 2. * PDIAMOPT/(1. + PSPHERI)
    ELSEIF (ABS(PSPHERI)< XUEPSI) THEN
        ZGRAN2 = 0.008 ! 2. * XVDIAM1
    ELSE
        ZGRAN2 = (1./PSPHERI)*(PDIAMOPT-XVDIAM1*(1.-PSPHERI))
    END IF
ENDIF
