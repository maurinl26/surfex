...
IF(CQSAT=='NEW')THEN
  ...
  DO JJ=1,SIZE(PT)
    IF (PT(JJ) <= XTT) THEN
      PPSAT(JJ) = EXP(XALPI - XBETAI/PT(JJ) - XGAMI*LOG(PT(JJ)))
    ELSE
...
IF(CQSAT=='NEW')THEN
  ...
  DO JJ=1,SIZE(PT)
    IF (PT(JJ) <= XTT) THEN
       PDPSAT(JJ) = XBETAI/PT(JJ)**2 - XGAMI/PT(JJ)
    ELSE
       PDPSAT(JJ) = XBETAW/PT(JJ)**2 - XGAMW/PT(JJ)
    END IF
  END DO
  !$OMP END PARALLEL DO
ELSE
...
!-------------------------------------------------------------------------------
!
IF (LHOOK) CALL DR_HOOK('MODE_THERMOS:QSATI_0D',0,ZHOOK_HANDLE)
!
!
!*       1.    COMPUTE SATURATION VAPOR PRESSURE
!              ---------------------------------
...
PQSAT = ZWORK2*ZWORK1 / (1.+(ZWORK2-1.)*ZWORK1)
!
IF (LHOOK) CALL DR_HOOK('MODE_THERMOS:QSATI_0D',1,ZHOOK_HANDLE)
!-------------------------------------------------------------------------------
!
