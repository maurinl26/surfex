...
!             ---------------------------
!
  IF (HAREA == "NAT".and.PRESENT(PVEGTYPE).AND.SIZE(NSIZE,2)>1) ALLOCATE(ZVEGTYPE(U%NSIZE_FULL))
    ...
    SELECT CASE (HAREA)
      ...
      CASE ('NAT')
        ...
        IF (PRESENT(PVEGTYPE) .AND.SIZE(NSIZE,2)>1) THEN
          ...
          IF ( JTN > 0.AND.JTN <= SIZE(PVEGTYPE,2) ) THEN
            ZVEGTYPE(:) = XUNDEF
            ZVEGTYPE(IMASK(1:SIZE(PVEGTYPE,1))) = PVEGTYPE(:,JTN)
  ...
  IF (HAREA == "NAT".AND.PRESENT(PVEGTYPE).AND.SIZE(NSIZE,2)>1) DEALLOCATE(ZVEGTYPE)
!
!-------------------------------------------------------------------------------
!
      ...
      IF ( (YFIELD(1:3)=='LAI'.OR.YFIELD(1:10)=='ALBNIR_VEG'.OR.YFIELD(1:10)=='ALBVIS_VEG') .OR. &
        ...
        IF (YFIELD(1:6)=='H_TREE') THEN
          IF ((.NOT.U%LECOSG.AND.((JT>=7 .AND. JT<=12) .OR. (JT>=18 .AND. JT<=19))).OR. &
              (     U%LECOSG.AND.(JT<=N2+3.OR.JT>=N2+13).AND.JT/=22 ) ) THEN
            ZFIELD(:,JT) = 0.
            NSIZE (:,JT) = 1.
          ENDIF
        ENDIF
...
IF (HAREA=='NAT'.AND.SIZE(ZFIELD,2)>SIZE(PFIELD,2).AND.U%LECOSG) THEN
  CALL PACK_SAME_RANK(IMASK,ZFIELD(:,N2+1:N3),PFIELD)
ELSE
