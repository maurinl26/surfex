...
!!    B.Marti, A.Boone
!!                  06/2025 : added DSL soil res option & modified PATCH test for low veg
!!
!----------------------------------------------------------------------------
!
!*    0.     DECLARATION
...
REAL, ALLOCATABLE, DIMENSION(:,:) :: ZSAND, ZCLAY
...
INTEGER                  :: ISAND_LAYER      ! number of sand layers
INTEGER                  :: ICLAY_LAYER      ! number of clay layers
INTEGER                  :: JVEGTYPE
...
CHARACTER(LEN=3)         :: YGNDRES          ! baresoil evap resistance options
...
CHARACTER(LEN=28), DIMENSION(20)  :: YSAND   ! file name for sand fraction
CHARACTER(LEN=28), DIMENSION(20)  :: YCLAY   ! file name for clay fraction
...
!             -------------------------
!
CALL READ_NAM_PGD_ISBA(HPROGRAM, IPATCH, IGROUND_LAYER, ISAND_LAYER, ICLAY_LAYER,&
                       YISBA,  YPEDOTF, YPHOTO, YGNDRES,                         &
                       GTR_ML, YALBEDO, ZRM_PATCH,                               &
...
IO%NSAND_LAYER   = ISAND_LAYER
IO%NCLAY_LAYER   = ICLAY_LAYER
IO%CISBA         = YISBA
IO%CPEDOTF       = YPEDOTF
IO%CPHOTO        = YPHOTO
IO%CGNDRES       = YGNDRES
...
GMEB_PATCH(:)    = .FALSE. ! init

IF ( GMEB ) THEN
  !
  IO%LTR_ML      = .TRUE. ! Always use this SW radiative transfer option with MEB
  !
  CALL READ_NAM_PGD_ISBA_MEB(HPROGRAM,ILUOUT,GMEB_PATCH,IO%LFORC_MEASURE,IO%LMEB_LITTER,IO%CMEB_GNDRES, &
                             IO%LMEB_TALL_VEG, IO%LMEB_INT_PLUN, IO%LMEB_INT_ULUN, IO%LMEB_INT_USFC)
  
  ! MEB can be used for all veg patches but is not allowed for non-veg patches.
  ! Note: The code block below (tests) works
  ...
  IF(IO%NPATCH >= 10)THEN
     IF(GMEB_PATCH(1) .OR. &
        GMEB_PATCH(2) .OR. &
        GMEB_PATCH(3))THEN
        CALL ABOR1_SFX('PGD_ISBA: MEB HAS BEEN ACTIVATED BY THE USER FOR A NON-VEGETATED PATCH')
     ENDIF
  ENDIF
  ...
  !
  IF (IO%LMEB_LITTER) IO%CMEB_GNDRES = 'NON'
  !
ENDIF
!
!-------------------------------------------------------------------------------
...
!
 ...
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CGNDRES',IO%CGNDRES,'NON','S92','DSL')
 CALL TEST_NAM_VAR_SURF(ILUOUT,'CMEB_GNDRES',IO%CMEB_GNDRES,'NON','S92','DSL')
 !
...
!          
    ...
    ENDIF
!    
    IF(IO%NCLAY_LAYER==NUNDEF)THEN
      IO%NCLAY_LAYER=1
    ELSE IF (IO%NCLAY_LAYER/=1 .AND. IO%NCLAY_LAYER/=IO%NGROUND_LAYER) THEN
      WRITE(ILUOUT,*) '*************************************************'
      WRITE(ILUOUT,*) '* Number of layers for clay texture not allowed *'
      WRITE(ILUOUT,*) '* NCLAY_LAYER must be 1 or ',IO%NGROUND_LAYER,' *'
      WRITE(ILUOUT,*) '*************************************************'
      CALL ABOR1_SFX('PGD_ISBA: NCLAY_LAYER must be equal to 1 or NGROUND_LAYER in NAM_ISBA')
    ENDIF
!    
    IF(IO%NSAND_LAYER==NUNDEF)THEN
      IO%NSAND_LAYER=1
    ELSE IF (IO%NSAND_LAYER/=1 .AND. IO%NSAND_LAYER/=IO%NGROUND_LAYER) THEN
      WRITE(ILUOUT,*) '*************************************************'
      WRITE(ILUOUT,*) '* Number of layers for sand texture not allowed *'
      WRITE(ILUOUT,*) '* NSAND_LAYER must be 1 or ',IO%NGROUND_LAYER,' *'
      WRITE(ILUOUT,*) '*************************************************'
      CALL ABOR1_SFX('PGD_ISBA: NSAND_LAYER must be equal to 1 or NGROUND_LAYER in NAM_ISBA')
    ENDIF
!    
...
ALLOCATE(ZSAND(ILU,IO%NSAND_LAYER))
!
DO JSOIL=1,IO%NSAND_LAYER
  CALL GET_FIELD(YSANDFILETYPE,YSAND(JSOIL),"SAND",LIMP_SAND,XUNIF_SAND,ZSAND(:,JSOIL))
END DO
!
IF (IO%NSAND_LAYER==1) THEN
! uniform vertical profile
  K%XSAND(:,1) = ZSAND(:,1)
  DO JLAYER=2,IO%NGROUND_LAYER
    K%XSAND(:,JLAYER) = ZSAND(:,1)
  END DO
ELSE
! non-uniform vertical profile
  DO JLAYER=1,IO%NGROUND_LAYER
    K%XSAND(:,JLAYER) = ZSAND(:,JLAYER)
  END DO
ENDIF
!-------------------------------------------------------------------------------
!
...
ALLOCATE(ZCLAY(ILU,IO%NCLAY_LAYER))
!
DO JSOIL=1,IO%NCLAY_LAYER
  CALL GET_FIELD(YCLAYFILETYPE,YCLAY(JSOIL),"CLAY",LIMP_CLAY,XUNIF_CLAY,ZCLAY(:,JSOIL))
END DO
!
IF (IO%NCLAY_LAYER==1) THEN
! uniform vertical profile
  K%XCLAY(:,1) = ZCLAY(:,1)
  DO JLAYER=2,IO%NGROUND_LAYER
    K%XCLAY(:,JLAYER) = ZCLAY(:,1)
  END DO
ELSE
! non-uniform vertical profile
  DO JLAYER=1,IO%NGROUND_LAYER
    K%XCLAY(:,JLAYER) = ZCLAY(:,JLAYER)
  END DO
ENDIF
!
!-------------------------------------------------------------------------------
