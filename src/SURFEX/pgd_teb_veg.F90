...
!!    B. Decharme   08/2016 : soil grdi optimization key
...
CHARACTER(LEN=28), DIMENSION(20) :: YCLAY    ! file name for sand fraction
...
GDO%NSAND_LAYER   = ISAND_LAYER
GDO%NCLAY_LAYER   = ICLAY_LAYER
GDO%CISBA         = YISBA
GDO%CPEDOTF       = YPEDOTF
GDO%CPHOTO        = YPHOTO
GDO%CGNDRES       = YGNDRES
...
!
 CALL PGD_FIELD(DTCO, UG, U, USS, &
                HPROGRAM,'sand fraction','TWN',YSAND(1),YSANDFILETYPE,XUNIF_SAND,GDK%XSAND(:,1))
ENDIF
!
...
ELSE
 CALL PGD_FIELD(DTCO, UG, U, USS, &
                HPROGRAM,'clay fraction','TWN',YCLAY(1),YCLAYFILETYPE,XUNIF_CLAY,GDK%XCLAY(:,1))
ENDIF
!
DO JLAYER=1,GDO%NGROUND_LAYER
...
!
  ...
  CALL TEST_NAM_VAR_SURF(ILUOUT,'CGNDRES',GDO%CGNDRES,'NON','S92','DSL')
  !
