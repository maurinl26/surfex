...
!!      B. Decharme 12/2023    Suppress ISIZE -> not consistant with mpi
!!------------------------------------------------------------------
!
!
...
!
 CALL PREP_HOR_FLAKE_FIELD(DTCO, UG, U, USS, GCP, F, &
...
IF (.NOT.LCLIM_LAKE) THEN
  ...
  !
  IF (.NOT.GNOVALUE) CALL PREP_HOR_FLAKE_FIELD(DTCO, UG, U, USS, GCP, F, &
  ...
  !
  ALLOCATE(F%XT_MNW(U%NSIZE_WATER))
  !
  IF (.NOT.GNOVALUE) CALL PREP_HOR_FLAKE_FIELD(DTCO, UG, U, USS, GCP, F, &
...
IF(LCLIM_LAKE.OR.GNOVALUE)THEN
  IF(LCLIM_LAKE)THEN
    ALLOCATE(F%XTS(U%NSIZE_WATER))
    F%XTS(:)=XUNDEF
  ENDIF
  ALLOCATE(F%XT_SNOW(U%NSIZE_WATER)) 
  ALLOCATE(F%XT_ICE (U%NSIZE_WATER))  
  ALLOCATE(F%XT_WML (U%NSIZE_WATER))
  ALLOCATE(F%XT_MNW (U%NSIZE_WATER)) 
  ALLOCATE(F%XT_BOT (U%NSIZE_WATER))  
  ALLOCATE(F%XT_B1  (U%NSIZE_WATER))
  ALLOCATE(F%XCT    (U%NSIZE_WATER))  
  ALLOCATE(F%XH_SNOW(U%NSIZE_WATER))  
  ALLOCATE(F%XH_ICE (U%NSIZE_WATER))
  ALLOCATE(F%XH_ML  (U%NSIZE_WATER))
  ALLOCATE(F%XH_B1  (U%NSIZE_WATER))  
  F%XT_SNOW(:)=XUNDEF
  F%XT_ICE(:)=XUNDEF
  F%XT_WML(:)=XUNDEF
  F%XT_MNW(:)=XUNDEF
...
!
F%LSBL = LWAT_SBL
IF (F%LSBL) CALL PREP_SBL(U%NSIZE_WATER, SB)
!
