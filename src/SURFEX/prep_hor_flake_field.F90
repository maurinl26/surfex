...
!     #########
SUBROUTINE PREP_HOR_FLAKE_FIELD (DTCO, UG, U, USS, GCP, F,            &
...
!!                             but not profiles
!!      B. Decharme 12/2023    Suppress KKLAT -> not consistent with mpi
!!------------------------------------------------------------------
!
...
GDEFAULT = (YFILETYPE=='      ' .OR. (HSURF(1:2)/='ZS' .AND. HSURF(1:2)/='TS' &
                .AND. U%NDIM_WATER.NE.1)) .AND. .NOT.GUNIF
