...
!     #########
SUBROUTINE PREP_HOR_ISBA_CC_FIELD (DTCO, U, GCP, KNI, IO, S, NK, NP, NPE, KPAR_VEG_IRR_USE,  &
...
!!      A. Druel      02/2019, Adapt the code and transmit KPAR_VEG_IRR_USE for irrigation
...
INTEGER,DIMENSION(:),INTENT(IN) :: KPAR_VEG_IRR_USE ! vegtype with irrigation
...
!
  IF(GPREP_AGS)THEN
    ...
    ZPATCH(:,:) = 0.
    !
    CALL GET_PREP_INTERP(INP,IO%NPATCH,S%XVEGTYPE,S%XPATCH,ZPATCH,KPAR_VEG_IRR_USE)
    !
    DO JP = 1, INP
...
IF (YDCTL%LPART5) THEN
  ...
  IF (GPREP_AGS) THEN
    ...
    IF (IO%NPATCH/=INP) THEN
      ...
      CALL PUT_ON_ALL_VEGTYPES(KNI,INL,INP,NVEGTYPE,KPAR_VEG_IRR_USE,ZFIELDOUTP,ZFIELDOUTV)
!
...
!
      DO JP = 1,IO%NPATCH
        ...
        CALL VEGTYPE_GRID_TO_PATCH_GRID(JP,IO%NPATCH,PK%XVEGTYPE_PATCH,PK%XPATCH,&
                                        PK%NR_P,ZFIELDOUTV,ZW%AL(JP)%ZOUT,KPAR_VEG_IRR_USE)
      ENDDO
      !
