...
!
 CALL READ_PREP_ISBA_CONF(HPROGRAM,HSURF,YFILE,YFILETYPE,YFILEPGD,YFILEPGDTYPE,&
                         ...
                         KPATCH=IO%NPATCH,KGROUND_LAYER=IO%NGROUND_LAYER)
!
CMASK = 'NATURE'
!
INI=SIZE(IG%XLAT)
...
IF (HSURF=='SN_VEG ') THEN
  ...
  DO JP = 1,IO%NPATCH
    ...
    IF (PEK%TSNOW%SCHEME=='CRO') THEN
      PEK%TSNOW%DIAMOPT = TNPSNOW%AL(JP)%DIAMOPT
      PEK%TSNOW%SPHERI = TNPSNOW%AL(JP)%SPHERI
...
!
  IF (GUNIF) THEN
    CALL PREP_ISBA_UNIF(DTCO,U,IO,ILUOUT,HSURF,ZFIELDIN)
  ELSE IF (YFILETYPE=='ASCLLV') THEN
    CALL PREP_ISBA_ASCLLV(DTCO, UG, U, USS, S, IO, HPROGRAM,HSURF,ILUOUT,ZFIELDIN)
  ELSE IF (YFILETYPE=='GRIB  ') THEN
    !SFX_KARPOS: build NetCDF-only (sans eccodes) → repli init uniforme au lieu du
    ! GRIB, pour un PREP file-less (champs sans XUNIF → valeur par défaut). Voir
    ! domains/drome, issue #5.
    WRITE(ILUOUT,*) 'KARPOS: GRIB demande pour ',HSURF,' -> repli init uniforme'
    CALL PREP_ISBA_UNIF(DTCO,U,IO,ILUOUT,HSURF,ZFIELDIN)
