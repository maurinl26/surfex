...
!
USE MODD_TYPE_SNOW,        ONLY : NSURF_SNOW, SURF_SNOW
...
IF (YDCTL%LPART1) THEN
  ...
  ELSE IF (HFILETYPE=='GRIB  ') THEN
    !SFX_KARPOS: build NetCDF-only (sans eccodes) → repli neige uniforme au lieu
    ! du GRIB (PREP file-less). Voir issue #5.
    WRITE(KLUOUT,*) 'KARPOS: GRIB neige demande pour ',HSNSURF,' -> repli uniforme'
    CALL PREP_SNOW_UNIF(KLUOUT,HSNSURF,ZFIELDIN, TPTIME, OSNOW_IDEAL,    &
                        PUNIF_WSNOW, PUNIF_RSNOW, PUNIF_TSNOW,           &
                        PUNIF_LWCSNOW, PUNIF_ASNOW, PUNIF_SG1SNOW,       &
                        PUNIF_SG2SNOW, PUNIF_HISTSNOW, PUNIF_AGESNOW,    &
  ...
  DO JP = 1,KPATCH
    ...
    SELECT CASE (HSNSURF(1:3))
      ...
      CASE('SG1')
        !
        IF (OSNOW_IDEAL) THEN
          SK%DIAMOPT(:,:) = ZW%AL(JP)%ZOUT(:,:)
        ELSEIF(INL==1) THEN
          DO JL = 1,ISNOW_NLAYER
             SK%DIAMOPT(:,JL) = ZW%AL(JP)%ZOUT(:,1)
          ENDDO      
        ELSE
          !* interpolation of heat on snow levels
          CALL INIT_FROM_REF_GRID(XGRID_SNOW,ZW%AL(JP)%ZOUT,ZGRID(1:KSIZE_P(JP),:,JP),SK%DIAMOPT)
        ENDIF
        !
        !* mask for areas where there is no snow
        DO JL=1,ISNOW_NLAYER
          WHERE(PDEPTH(1:KSIZE_P(JP),JL,JP)==0. .OR. PDEPTH(1:KSIZE_P(JP),JL,JP)==XUNDEF) SK%DIAMOPT(:,JL) = XUNDEF
        END DO
        !
      CASE('SG2')
        !
        IF (OSNOW_IDEAL) THEN
          SK%SPHERI(:,:) = ZW%AL(JP)%ZOUT(:,:)
        ELSEIF(SIZE(ZW%AL(JP)%ZOUT,2)==1) THEN
          DO JL = 1,ISNOW_NLAYER
            SK%SPHERI(:,JL) = ZW%AL(JP)%ZOUT(:,1)
          ENDDO      
        ELSE
          !* interpolation of heat on snow levels
          CALL INIT_FROM_REF_GRID(XGRID_SNOW,ZW%AL(JP)%ZOUT,ZGRID(1:KSIZE_P(JP),:,JP),SK%SPHERI)
        ENDIF
        !
        !* mask for areas where there is no snow
        DO JL=1,ISNOW_NLAYER
          WHERE(PDEPTH(1:KSIZE_P(JP),JL,JP)==0. .OR. PDEPTH(1:KSIZE_P(JP),JL,JP)==XUNDEF) SK%SPHERI(:,JL) = XUNDEF
        END DO
        !
      CASE('HIS')
        !
