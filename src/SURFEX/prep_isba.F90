...
!!    -------------
!!      Original    01/2004
!!      P. Le Moigne  11/2004 : AGS fields
!!      B. Decharme      2008 : Floodplains
!!      B. Decharme   01/2009 : Consistency with Arpege deep soil temperature
!!      B. Decharme   03/2009 : Consistency with Arpege permanent snow/ice treatment
!!      A.L. Gibelin  04/2009 : BIOMASS and RESP_BIOMASS arrays 
!!      A.L. Gibelin  06/2009 : Soil carbon variables for CNT option
!!      S. Riette     06/2009 : PREP_ISBA_CANOPY has no more arg.
!!      S. Riette     04/2010 : ecmwf ice content is computed during grib reading (no longer here)
...
!!      R. Séférian   01/2018 : Modify spin-up procedure for carbon cycle
...
IF(YDCTL%LPART6)THEN
  ...
  DO JP = 1,IO%NPATCH
    ...
    PEK%XRESA = 100.
    !
    ALLOCATE(PEK%XUSTAR(PK%NSIZE_P))
    PEK%XUSTAR = 0.
    !
  ...
  IF(IO%LLULCC)THEN
    ...
    IF(IO%CPHOTO=='NCB')THEN
      ...
      ! Carbon conservation
      !
      ALLOCATE(S%XCCONSRV(INI))
      S%XCCONSRV(:) = 0.0
      !
      ! Land-use Land Cover flux
      !
      ALLOCATE(S%XFLUATM(INI,IO%NPATCH))
      S%XFLUATM(:,:)=0.
      !
      ALLOCATE(S%XFLURES(INI,IO%NPATCH))
      S%XFLURES(:,:)=0.
      !
    ENDIF
    !
    ! Land-use Land Cover change managing
    !
    IF(IO%LLULCC_MANAGE) THEN
      !
      ALLOCATE(S%XFLUANT(INI,IO%NPATCH))
      S%XFLUANT(:,:)=0.
      !
      ALLOCATE(S%XFANTATM(INI,IO%NPATCH))
      S%XFANTATM(:,:)=0.
      !
      ALLOCATE(S%XEXPORT_DECADAL(INI,IO%NNDECADAL,IO%NPATCH))
      S%XEXPORT_DECADAL(:,:,:)=0.
      !
      ALLOCATE(S%XCSTOCK_DECADAL(INI,IO%NNDECADAL,IO%NPATCH))
      S%XCSTOCK_DECADAL(:,:,:)=0.
      !
      ALLOCATE(S%XEXPORT_CENTURY(INI,IO%NNCENTURY,IO%NPATCH))
      S%XEXPORT_CENTURY(:,:,:)=0.
      !
      ALLOCATE(S%XCSTOCK_CENTURY(INI,IO%NNCENTURY,IO%NPATCH))
      S%XCSTOCK_CENTURY(:,:,:)=0.
      !
    ENDIF
    !
