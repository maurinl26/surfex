...
DO JL=1,PEK%TSNOW%NLAYER/4
  WHERE(GWORK(:,JL)) 
    PEK%TSNOW%DIAMOPT(:,JL) = 0.0001* (1. + 8. * FLOAT(JL)/FLOAT(PEK%TSNOW%NLAYER)) !varie entre 0.0001 et 0.0003
    PEK%TSNOW%SPHERI(:,JL) = 0.5
    PEK%TSNOW%HIST(:,JL) = 0 
  END WHERE
END DO
DO JL=1+PEK%TSNOW%NLAYER/4,PEK%TSNOW%NLAYER
  WHERE(GWORK(:,JL))
    PEK%TSNOW%DIAMOPT(:,JL) = 0.0003
    PEK%TSNOW%SPHERI(:,JL) = 1.  
    PEK%TSNOW%HIST(:,JL) = 0 
  END WHERE
END DO
END IF
