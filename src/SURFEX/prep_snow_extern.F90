...
DO JP = 1,IPATCH
  ...
  !
      CASE ('DEP')
        IF (OSNOW_IDEAL) THEN
          IF (KLAYER /= SIZE(TZSNOW%WSNOW,DIM=2)) CALL ABOR1_SFX('if snow_ideal=.true. , &
                   KLAYER must equal to SIZE(TZSNOW%WSNOW,DIM=2) !')    
          IF (JP<=1) ALLOCATE(PFIELD(INI,KLAYER,IPATCH))  
        ...
        SELECT CASE (TZSNOW%SCHEME)
          ...
          CASE ('3-L','CRO')
            ...
            IF (TZSNOW%SCHEME=='CRO')THEN
              IF (HSURF(1:3)=='SG1') ZFIELD(:,:) = TZSNOW%DIAMOPT(:,1:TZSNOW%NLAYER)
              IF (HSURF(1:3)=='SG2') ZFIELD(:,:) = TZSNOW%SPHERI(:,1:TZSNOW%NLAYER)
