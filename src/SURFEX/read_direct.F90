...
REAL    :: ZSHIFT,ZDX,ZDY             ! shift on longitudes
...
IF (GCOMPRESS) THEN
  ...
  ALLOCATE (YVALUE16(INBCOL*INBLINE))
ELSEIF (YTYPE=='INTEGER' .AND. IBITS== 8) THEN  
...
DO JLAT = 1,360
  ...
  ZLATMAX = (JLAT-180)/2.
  IF ( .NOT. ANY(ZLAT(:)<ZLATMAX .AND. ZLAT(:)>=ZLATMAX-.5) ) CYCLE
  ICPT = ICPT + 1
  IMASK(ICPT) = JLAT
ENDDO
!
...
DO 
  ...
  !            ------------------------
  !
  ZDY = ZGLBLATMAX-ZDLAT/2.-ZLATMAX
  ILINE1=MAX(MIN(INT(ZDY/ZDLAT)+1,INBLINE),0)+1
  ILINE2=MAX(MIN(INT((ZDY+.5)/ZDLAT)+1,INBLINE),0)
  !
  !----------------------------------------------------------------------------
  !
  !*    8.     Loop on lines
  ...
  IF (GCOMPRESS) THEN
    IF (LHOOK) CALL DR_HOOK('READ_DIRECT_readc',0,ZH)
    IPOS = 0
    IF (JL==IPAS.OR.ILINE_COMPRESS<ILINE1) THEN
      IF (ILINE1>1) IPOS = SUM(ICPT0(1:ILINE1-1))
      IPOS = IPOS*2 + 1 + INBLINE*4
      ILINE_COMPRESS = ILINE1
    ENDIF

    IF (ILINE1 > ILINE2) CYCLE

    NCOL = SUM(ICPT0(ILINE1:ILINE2))
    IF (IPOS==0) THEN
      READ(IGLB) YVALUE16(1:NCOL)
    ELSE
      READ(IGLB,POS=IPOS) YVALUE16(1:NCOL)
    ENDIF
    IF (LHOOK) CALL DR_HOOK('READ_DIRECT_readc',1,ZH)
  ENDIF
  ...
  DO JLINE = ILINE1,ILINE2
    ...
    ELSE IF (YTYPE=='INTEGER' .AND. IBITS==16) THEN
      !
      IF (GCOMPRESS) THEN
        ICOL = SUM(ICPT0(ILINE1:JLINE))
        ZVALUE(1:ICPT0(JLINE))=YVALUE16(ICOL-ICPT0(JLINE)+1:ICOL)
      ELSE
        READ(IGLB,REC=IREC) YVALUE16(1:INBCOL)
        ZVALUE(:)=YVALUE16(1:INBCOL)
      ENDIF
      !
    ...
    !
    IF (GCOMPRESS) THEN
      IF (LHOOK) CALL DR_HOOK('READ_DIRECT_interc',0,ZH)
      WHERE (ZVALUE(1:ICPT0(JLINE))<0.)
        ZINTER(1:ICPT0(JLINE)) = NINT(32768*2.+ZVALUE(1:ICPT0(JLINE)))
      ELSEWHERE
        ZINTER(1:ICPT0(JLINE)) = ZVALUE(1:ICPT0(JLINE))
      ENDWHERE
      IF (LHOOK) CALL DR_HOOK('READ_DIRECT_interc',1,ZH)
    ...
    !            -------------------
    !
    IF (LHOOK) CALL DR_HOOK('READ_DIRECT_column',0,ZH)
    DO JLON=1,720
      !
      ...
      !            -----------------------------------------------------------------
      !
      ZSHIFT = 360. * NINT((ZLONMAX-.5-ZGLBLONMIN-180.*(1-XSURF_EPSILON))/360.)
      !
      ...
      !            -------------------------
      !
      ZDX = ZLONMAX-ZGLBLONMIN-ZDLON/2.
      ICOL1(1)=MAX(MIN(INT((ZDX-0.5)/ZDLON)+1,INBCOL),0)+1
      ICOL2(1)=MAX(MIN(INT(ZDX/ZDLON)+1,INBCOL),0)
      !
      ...
      !
      ICOL1(2)=1
      ICOL2(2)=MAX(MIN(INT((ZDX-360.)/ZDLON)+1,INBCOL),0)
      !
      !----------------------------------------------------------------------------
      !
      ...
      DO JLOOP=1,2
        ...
        !            ----------------------------------------------------------
        !
        CALL PT_BY_PT_TREATMENT(UG, U, USS, ILUOUT, ZLAT_WORK(1:IWORK),&
           ZLON_WORK(1:IWORK),ZVALUE_WORK(1:IWORK),&
           HSUBROUTINE, OMULTITYPE=GMULTITYPE, KFACT=IFACT)
      END DO
    END DO
    IF (LHOOK) CALL DR_HOOK('READ_DIRECT_column',1,ZH)
  END DO
END DO
!
!-------------------------------------------------------------------------------
