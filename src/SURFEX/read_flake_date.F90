...
!     #######################################################
!
!!****  *READ_FLAKE_DATE* - initializes the date TTIME of MODD_FLAKE
!!
!!    PURPOSE
!!    -------
!!
...
IF (TPTIME%TDATE%YEAR==NUNDEF.OR.TPTIME%TDATE%MONTH==NUNDEF &
      .OR.TPTIME%TDATE%DAY==NUNDEF.OR.TPTIME%TIME==XUNDEF) THEN  
  !
  CALL READ_PREP_FLAKE_CONF(HPROGRAM,'DATE   ',YFILE,YFILETYPE,YFILEPGD,YFILEPGDTYPE,&
...
!-----------------------------------------------------------------------
!
    CALL ABOR1_SFX('READ_FLAKE_DATE: DATE NOT SET')
  END IF
ENDIF
!
...
IF (KYEAR /= NUNDEF .AND. KMONTH /= NUNDEF .AND. KDAY /= NUNDEF .AND. PTIME /= XUNDEF) THEN
  IF (KYEAR /= TPTIME%TDATE%YEAR .OR. KMONTH /= TPTIME%TDATE%MONTH .OR. KDAY /= TPTIME%TDATE%DAY .AND. PTIME /= TPTIME%TIME) THEN
     WRITE(UNIT=KLUOUT, FMT=*)'WARNING in READ_FLAKE_DATE'
  ...
  ELSE
     ...
     WRITE(UNIT=KLUOUT, FMT=*)'DATE in READ_FLAKE_DATE'
      WRITE(UNIT=KLUOUT, FMT='(" YEAR=",I4," MONTH=",I4," DAY=",I4)') &
                                        KYEAR,KMONTH,KDAY  
     WRITE(UNIT=KLUOUT, FMT='(" TIME=",E13.6)') PTIME
  ENDIF
ENDIF
IF (LHOOK) CALL DR_HOOK('READ_FLAKE_DATE',1,ZHOOK_HANDLE)
!------------------------------------------------------------------------------
!
END SUBROUTINE READ_FLAKE_DATE
