...
!
 ...
 CHARACTER(LEN=1)    :: YDIR   ,YJIMP             ! type of reading
...
IF (TPSNOW%SCHEME=='1-L' .OR. TPSNOW%SCHEME=='D95' .OR. TPSNOW%SCHEME=='EBA' &
  ...
  IF (TPSNOW%SCHEME=='3-L' .OR. TPSNOW%SCHEME=='CRO') THEN
    ...
    IF (TPSNOW%SCHEME=='CRO') THEN
      ...
      ENDIF
      YFMT = YFMT//YNLAYER//')'
      CALL READ_LAYERS(GVERSION,TPSNOW%NLAYER,YDIR,HPREFIX,YFMT,"SGRAN",HSURFTYPE,TPSNOW%DIAMOPT,HREC2="1")
      CALL READ_LAYERS(GVERSION,TPSNOW%NLAYER,YDIR,HPREFIX,YFMT,"SGRAN",HSURFTYPE,TPSNOW%SPHERI,HREC2="2")
      !
      ! 
      !*        9.b    Snow impurity
      !                ------------
      DO JIMP=1, NIMPUR
        ...
        ENDIF     
        YFMT = YFMT//YNLAYER//')'
        CALL READ_LAYERS(GVERSION,TPSNOW%NLAYER,YDIR,HPREFIX,YFMT,"IMPUR",HSURFTYPE,TPSNOW%IMPUR(:,:,JIMP),HREC2=YJIMP)
      ENDDO
    !
    END IF
!
  ...
  ENDIF
  !
  !*      14. Water consumption for snowmaking 
  !          -------------------------     
  IF (IVERSION >= 9 .AND. TPSNOW%SCHEME=='CRO') THEN
	  WRITE(YFMT,'(A5,I1,A2,I1,A1)') '(A4,A',ISURFTYPE_LEN,',A',IPAT_LEN,')'
	  WRITE(YRECFM,YFMT) 'MMP_',ADJUSTL(HSURFTYPE(:LEN_TRIM(HSURFTYPE))),ADJUSTL(YPAT)
	  IF (GVERSION) YRECFM=ADJUSTL(HPREFIX//YRECFM)
	  IF (GDIM2) THEN
	    CALL READ_SURF(HPROGRAM,YRECFM,ZWORK(:,1),IRESP,HDIR=YDIR)
	    CALL PACK_SAME_RANK(KMASK_P,ZWORK(:,1),TPSNOW%MMP(:))
	  ELSE
	    CALL READ_SURF(HPROGRAM,YRECFM,ZWORK,IRESP,HDIR=YDIR)
	    CALL PACK_SAME_RANK(KMASK_P,ZWORK(:,MAX(1,KPATCH)),TPSNOW%MMP(:))
	  ENDIF   
  ENDIF
...
!$OMP DO PRIVATE(JI,JL)
  DO JI = 1,SIZE(TPSNOW%WSNOW,1)
    ...
    IF (TPSNOW%WSNOW(JI,1) == 0.0 ) THEN
      ...
      IF (TPSNOW%SCHEME=='CRO')TPSNOW%MMP(JI) = 0.
      !
      DO JL = 1,TPSNOW%NLAYER
        !
        ...
        ELSEIF (TPSNOW%SCHEME=='3-L' .OR. TPSNOW%SCHEME=='CRO') THEN
          ...
          IF (TPSNOW%SCHEME=='CRO') THEN
            ...
            TPSNOW%DIAMOPT(JI,JL) = XUNDEF
            TPSNOW%SPHERI(JI,JL) = XUNDEF
