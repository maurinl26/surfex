...
!     #########
      SUBROUTINE READ_PGD_ISBA_PAR_n (DTCO, U, GCP, DTI, KDIM, IO, HPROGRAM,KSIZE, TPDATE_BEG, TPDATE_END, HDIR)
!     ################################################
!
...
!!      A. Druel       02/2019 : Adapt the code to be compatible with irrigation and add IRRIGFRAC (&co.)
!!                               Make it compatible with oldest PGD version (v8.1 and before)
!!      B; Decharme    08/2020 : H_VEG by default
!!
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
...
INTEGER, PARAMETER, DIMENSION(1) :: NO_IRR_USE=(/0/)
!
...
ALLOCATE(DTI%LDATA_F2THRESHOLD (36*NVEGTYPE))
DTI%LDATA_VEG        (:) = .FALSE.
DTI%LDATA_LAI        (:) = .FALSE.
DTI%LDATA_Z0         (:) = .FALSE.
...
IF (IVERSION<7 .AND. .NOT.IO%LECOCLIMAP) THEN
  ...
  DTI%LDATA_IRRIGFRAC   = .FALSE.
  DTI%LDATA_WATSUP      = .FALSE. 
  DTI%LDATA_F2THRESHOLD = .FALSE.
  DTI%LDATA_IRRIGTYPE   = .FALSE. 
  DTI%LDATA_IRRIGTIME   = .FALSE.
  DTI%LDATA_IRRIGFREQ   = .FALSE. 
  !
  ...
  LMULTI_SEASON = .FALSE.
  DTI%LDATA_SEED_S2_M=.FALSE.
  DTI%LDATA_SEED_S2_D=.FALSE.
  DTI%LDATA_REAP_S2_M=.FALSE.
  DTI%LDATA_REAP_S2_D=.FALSE.
  DTI%LDATA_SEED_S3_M=.FALSE.
  DTI%LDATA_SEED_S3_D=.FALSE.
  DTI%LDATA_REAP_S3_M=.FALSE.
  DTI%LDATA_REAP_S3_D=.FALSE.
  !
ENDIF
!
...
!
  ...
  IF (IVERSION>=7) THEN
    ...
    IF (IVERSION>=9) THEN
      YRECFM='L_IRRIGTYPE'
      CALL READ_SURF(HPROGRAM,YRECFM,DTI%LDATA_IRRIGTYPE(1:IEND),IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      IF (ANY(DTI%LDATA_IRRIGTYPE)) DTI%LDATA_MIXPAR = .TRUE.
    ELSE
      ALLOCATE(GWORK(36*NVEGTYPE))
      GWORK(:) = .FALSE.
      YRECFM='L_IRRIG'
      CALL READ_SURF(HPROGRAM,YRECFM,GWORK,IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      IF (ANY(GWORK)) THEN
        DTI%LDATA_MIXPAR = .TRUE.
        WRITE(ILUOUT,*) ' '
        WRITE(ILUOUT,*) '*********************************************************'
        WRITE(ILUOUT,*) '* Your using the irrigation with the old configuration! *'
        WRITE(ILUOUT,*) '* (PGD version <9 with SURFEX version >= 9).            *'
        WRITE(ILUOUT,*) '* But the options are not totaly compatible. Please make*'
        WRITE(ILUOUT,*) '* a PGD (or work on it)...                              *'
        WRITE(ILUOUT,*) '*********************************************************'
        CALL ABOR1_SFX('READ_PGD_ISBA_PARN: AN OLD PGD (VERSION < 9) IS USED FOR NEW IRRIGATION (SURFEX >=9)')
      ELSE
        DTI%LDATA_IRRIGTYPE = .FALSE.
      ENDIF
      DEALLOCATE(GWORK)
    ENDIF
    !
    IF (IVERSION<9 .OR. ANY(DTI%LDATA_IRRIGTYPE)) THEN
      YRECFM='L_WATSUP'
    ...
    ENDIF
    !
    IF (IVERSION>=9 .AND. ANY(DTI%LDATA_IRRIGTYPE)) THEN
      IF ( U%LECOSG ) THEN
        YRECFM='L_IRRIGFRAC'
        CALL READ_SURF(HPROGRAM,YRECFM,DTI%LDATA_IRRIGFRAC(1:IEND),IRESP,HCOMMENT=YCOMMENT,HDIR='-')
        IF (ANY(DTI%LDATA_IRRIGFRAC)) DTI%LDATA_MIXPAR = .TRUE.
      ENDIF
      YRECFM='L_IRRIGFREQ'
      CALL READ_SURF(HPROGRAM,YRECFM,DTI%LDATA_IRRIGFREQ(1:IEND),IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      IF (ANY(DTI%LDATA_IRRIGFREQ)) DTI%LDATA_MIXPAR = .TRUE.    
      YRECFM='L_IRRIGTIME'
      CALL READ_SURF(HPROGRAM,YRECFM,DTI%LDATA_IRRIGTIME(1:IEND),IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      IF (ANY(DTI%LDATA_IRRIGTIME)) DTI%LDATA_MIXPAR = .TRUE.
      YRECFM='L_F2THRESHOLD'
      CALL READ_SURF(HPROGRAM,YRECFM,DTI%LDATA_F2THRESHOLD(1:IENDT),IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      IF (ANY(DTI%LDATA_F2THRESHOLD)) DTI%LDATA_MIXPAR = .TRUE.
    ENDIF
    !
  ENDIF
  !
  ...
  !
  LMULTI_SEASON = .FALSE.
  IF ( GDIM .AND. (IVERSION<9 .OR. ANY(DTI%LDATA_IRRIGTYPE) .OR. LAGRIP) ) THEN
    !
    YRECFM='L_SEED_M'
    ...
    !
    IF (ANY(DTI%LDATA_SEED_M) .AND. IVERSION>=9) THEN
      YRECFM='L_SEED_S2_M'
      CALL READ_SURF(HPROGRAM,YRECFM,DTI%LDATA_SEED_S2_M(1:IEND),IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      YRECFM='L_SEED_S2_D'
      CALL READ_SURF(HPROGRAM,YRECFM,DTI%LDATA_SEED_S2_D(1:IEND),IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      YRECFM='L_REAP_S2_M'
      CALL READ_SURF(HPROGRAM,YRECFM,DTI%LDATA_REAP_S2_M(1:IEND),IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      YRECFM='L_REAP_S2_D'
      CALL READ_SURF(HPROGRAM,YRECFM,DTI%LDATA_REAP_S2_D(1:IEND),IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      IF (ANY(DTI%LDATA_SEED_S2_M)) THEN
        LMULTI_SEASON = .TRUE.
        YRECFM='L_SEED_S3_M'
        CALL READ_SURF(HPROGRAM,YRECFM,DTI%LDATA_SEED_S3_M(1:IEND),IRESP,HCOMMENT=YCOMMENT,HDIR='-')
        YRECFM='L_SEED_S3_D'
        CALL READ_SURF(HPROGRAM,YRECFM,DTI%LDATA_SEED_S3_D(1:IEND),IRESP,HCOMMENT=YCOMMENT,HDIR='-')
        YRECFM='L_REAP_S3_M'
        CALL READ_SURF(HPROGRAM,YRECFM,DTI%LDATA_REAP_S3_M(1:IEND),IRESP,HCOMMENT=YCOMMENT,HDIR='-')
        YRECFM='L_REAP_S3_D'
        CALL READ_SURF(HPROGRAM,YRECFM,DTI%LDATA_REAP_S3_D(1:IEND),IRESP,HCOMMENT=YCOMMENT,HDIR='-')
      ELSE
        DTI%LDATA_SEED_S3_M=.FALSE.
        DTI%LDATA_SEED_S3_D=.FALSE.
        DTI%LDATA_REAP_S3_M=.FALSE.
  ...
  !
  IF (GDIM .OR. (ANY(DTI%LDATA_LAI)  .OR. ANY(DTI%LDATA_VEG)&
           .OR.  ANY(DTI%LDATA_H_VEG).OR.ANY(DTI%LDATA_Z0)  &
           .OR. ANY(DTI%LDATA_EMIS))                        ) THEN
    IF (.NOT.GDIM) THEN
      YRECFM='NDATA_TIME'
      CALL READ_SURF(HPROGRAM,YRECFM,DTI%NTIME,IRESP,HCOMMENT=YCOMMENT) 
    ENDIF      
  ...
  IF (ANY(DTI%LDATA_VEG)) THEN
    ...
    DO JT=1,ITIME
      ...
      IF (GTIME(JT)) THEN
        ...
        CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
...
!
  IF (ANY(DTI%LDATA_GROUND_DEPTH)) THEN
    ...
    ENDIF       
    CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM2, ILUOUT, KSIZE, &
...
!
  ...
  ENDIF
!
  IF (ANY(DTI%LDATA_IRRIGTYPE)) THEN
    ALLOCATE(DTI%XPAR_IRRIGTYPE     (KDIM,NVEGTYPE))
    IF (GDIM) THEN
      YRECFM='D_IRRTYP_'
    ELSE
      YRECFM='D_IRRIGTYPE'
    ENDIF
    CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
                                  IVERSION, IBUGFIX, DTI%LDATA_IRRIGTYPE,DTI%XPAR_IRRIGTYPE(:,:),IRESP,&
                                  HCOMMENT=YCOMMENT,HDIR=YDIR,LAUTO_C=.FALSE.)
  ENDIF
!
  ...
  ENDIF
!
  IF (ANY(DTI%LDATA_IRRIGFRAC) .AND. U%LECOSG ) THEN
    ALLOCATE(DTI%XPAR_IRRIGFRAC     (KDIM,NVEGTYPE))
    IF (GDIM) THEN
      YRECFM='D_IRRFRA_'
    ELSE
      YRECFM='D_IRRIGFRAC'
    ENDIF
    CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
                                  IVERSION, IBUGFIX, DTI%LDATA_IRRIGFRAC,DTI%XPAR_IRRIGFRAC(:,:),IRESP,&
                                  HCOMMENT=YCOMMENT,HDIR=YDIR,LAUTO_C=.FALSE.)
  ENDIF
!
  IF (ANY(DTI%LDATA_IRRIGFREQ)) THEN
    ALLOCATE(DTI%XPAR_IRRIGFREQ     (KDIM,NVEGTYPE))
    IF (GDIM) THEN
      YRECFM='D_IRRFRQ_'
    ELSE
      YRECFM='D_IRRIGFREQ'
    ENDIF
    CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
                                  IVERSION, IBUGFIX, DTI%LDATA_IRRIGFREQ,DTI%XPAR_IRRIGFREQ(:,:),IRESP,&
                                  HCOMMENT=YCOMMENT,HDIR=YDIR,LAUTO_C=.FALSE.)
  ENDIF
  !
  IF (ANY(DTI%LDATA_IRRIGTIME)) THEN
    ALLOCATE(DTI%XPAR_IRRIGTIME     (KDIM,NVEGTYPE))
    IF (GDIM) THEN
      YRECFM='D_IRRTIM_'
    ELSE
      YRECFM='D_IRRIGTIME'
    ENDIF
    CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
                                  IVERSION, IBUGFIX, DTI%LDATA_IRRIGTIME,DTI%XPAR_IRRIGTIME(:,:),IRESP,&
                                  HCOMMENT=YCOMMENT,HDIR=YDIR,LAUTO_C=.FALSE.)
  ENDIF
  !
  IF (ANY(DTI%LDATA_F2THRESHOLD)) THEN
    ALLOCATE(DTI%XPAR_F2THRESHOLD     (KDIM,DTI%NTIME,NVEGTYPE))
    DO JT=1,ITIME
      ID1 = (JT-1)*NVEGTYPE + 1
      ID2 = JT*NVEGTYPE    
      IF (GDIM) THEN
        WRITE(YRECFM,FMT='(A10,I2.2)') 'D_F2T_T',JT 
      ELSE
        WRITE(YRECFM,FMT='(A10,I2.2)') 'D_F2THRES_T',JT            
      ENDIF   
      IF (GTIME(JT)) THEN    
        CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
                                  IVERSION, IBUGFIX, DTI%LDATA_F2THRESHOLD(ID1:ID2),DTI%XPAR_F2THRESHOLD(:,JT,:),IRESP,&
                                  HCOMMENT=YCOMMENT,HDIR=YDIR,LAUTO_C=.FALSE.)
      ELSE
        DTI%XPAR_F2THRESHOLD(:,JT,:) = XUNDEF
      ENDIF                        
    END DO
  ENDIF
!
...
!
    IF (LMULTI_SEASON) THEN
      IF (ANY(DTI%LDATA_SEED_S2_M)) THEN
        ALLOCATE(DTI%XPAR_SEED_S2_M (KDIM,NVEGTYPE))
        YRECFM='D_SEED2M_'
        CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
                                  IVERSION, IBUGFIX, DTI%LDATA_SEED_S2_M,DTI%XPAR_SEED_S2_M(:,:),IRESP, &
                                  HCOMMENT=YCOMMENT,HDIR=YDIR,LAUTO_C=.FALSE.)
      ENDIF
      IF (ANY(DTI%LDATA_SEED_S2_D)) THEN
        ALLOCATE(DTI%XPAR_SEED_S2_D (KDIM,NVEGTYPE))
        YRECFM='D_SEED2D_'
        CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
                                  IVERSION, IBUGFIX, DTI%LDATA_SEED_S2_D,DTI%XPAR_SEED_S2_D(:,:),IRESP, &
                                  HCOMMENT=YCOMMENT,HDIR=YDIR,LAUTO_C=.FALSE.)
      ENDIF
      IF (ANY(DTI%LDATA_REAP_S2_M)) THEN
        ALLOCATE(DTI%XPAR_REAP_S2_M (KDIM,NVEGTYPE))
        YRECFM='D_REAP2M_'
        CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
                                  IVERSION, IBUGFIX, DTI%LDATA_REAP_S2_M,DTI%XPAR_REAP_S2_M(:,:),IRESP, &
                                  HCOMMENT=YCOMMENT,HDIR=YDIR,LAUTO_C=.FALSE.)
      ENDIF
      IF (ANY(DTI%LDATA_REAP_S2_D)) THEN
        ALLOCATE(DTI%XPAR_REAP_S2_D (KDIM,NVEGTYPE))
        YRECFM='D_REAP2D_'
        CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
                                  IVERSION, IBUGFIX, DTI%LDATA_REAP_S2_D,DTI%XPAR_REAP_S2_D(:,:),IRESP,&
                                  HCOMMENT=YCOMMENT,HDIR=YDIR,LAUTO_C=.FALSE.)
      ENDIF
      !
      IF (ANY(DTI%LDATA_SEED_S3_M)) THEN
        ALLOCATE(DTI%XPAR_SEED_S3_M (KDIM,NVEGTYPE))
        YRECFM='D_SEED3M_'
        CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
                                  IVERSION, IBUGFIX, DTI%LDATA_SEED_S3_M,DTI%XPAR_SEED_S3_M(:,:),IRESP, &
                                  HCOMMENT=YCOMMENT,HDIR=YDIR,LAUTO_C=.FALSE.)
      ENDIF
      IF (ANY(DTI%LDATA_SEED_S3_D)) THEN
        ALLOCATE(DTI%XPAR_SEED_S3_D (KDIM,NVEGTYPE))
        YRECFM='D_SEED3D_'
        CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
                                  IVERSION, IBUGFIX, DTI%LDATA_SEED_S3_D,DTI%XPAR_SEED_S3_D(:,:),IRESP, &
                                  HCOMMENT=YCOMMENT,HDIR=YDIR,LAUTO_C=.FALSE.)
      ENDIF
      IF (ANY(DTI%LDATA_REAP_S3_M)) THEN
        ALLOCATE(DTI%XPAR_REAP_S3_M (KDIM,NVEGTYPE))
        YRECFM='D_REAP3M_'
        CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
                                  IVERSION, IBUGFIX, DTI%LDATA_REAP_S3_M,DTI%XPAR_REAP_S3_M(:,:),IRESP, &
                                  HCOMMENT=YCOMMENT,HDIR=YDIR,LAUTO_C=.FALSE.)
      ENDIF
      IF (ANY(DTI%LDATA_REAP_S3_D)) THEN
        ALLOCATE(DTI%XPAR_REAP_S3_D (KDIM,NVEGTYPE))
        YRECFM='D_REAP3D_'
        CALL READ_SURF_ISBA_PAR_n(DTCO, U, GCP, IO%NPATCH, NO_IRR_USE, HPROGRAM, YRECFM, ILUOUT, KSIZE, &
                                  IVERSION, IBUGFIX, DTI%LDATA_REAP_S3_D,DTI%XPAR_REAP_S3_D(:,:),IRESP,&
                                  HCOMMENT=YCOMMENT,HDIR=YDIR,LAUTO_C=.FALSE.)
      ENDIF
      !
    ENDIF
