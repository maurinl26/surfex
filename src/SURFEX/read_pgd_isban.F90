...
!!      B. Decharme   07/2008 : add XWDRAIN
...
!!      J. Colin      08/2016 : Mask for snow and soil moisture nudging
!!      B. Decharme   08/2016 : change landuse implementation 
...
LOGICAL                  :: GMEB_WORK     ! MEB logical working variable
!
...
!
! Basesoil resistance scheme
!
IF (IVERSION==9 .AND. IBUGFIX>=1 .OR. IVERSION>9 ) THEN
   YRECFM='GNDRES'
   CALL READ_SURF(HPROGRAM,YRECFM,IO%CGNDRES,IRESP)  
ELSE
    IO%CGNDRES='NON'
ENDIF
!
...
ELSE
  ...
  CALL READ_NAM_PGD_ISBA(HPROGRAM, IPATCH, IGROUND_LAYER, ISAND_LAYER, ICLAY_LAYER,&
                       ...
                       XUNIF_FERT, GLULCC                                          )  
  IO%CALBEDO = YALBEDO
  YGNDRES    = 'NON' ! baresoil resis did not exist before 9.1+
  !
ENDIF
!
...
!
  IF (ISIZE_LMEB_PATCH>0)THEN
    ...
    YRECFM='MEB_GNDRES'
    CALL READ_SURF(HPROGRAM,YRECFM,GMEB_WORK,IRESP)
    IF(GMEB_WORK)THEN
       IO%CMEB_GNDRES = 'S92' ! In Version8 & 9.0, only this option existed
    ELSE
       IO%CMEB_GNDRES = 'NON'
    ENDIF
  ELSE
...
!
  IF (ISIZE_LMEB_PATCH>0)THEN
    ...
    YRECFM='MEB_GNDRES'
    IF(IBUGFIX>=1)THEN
       CALL READ_SURF(HPROGRAM,YRECFM,IO%CMEB_GNDRES,IRESP)
    ELSE
       CALL READ_SURF(HPROGRAM,YRECFM,GMEB_WORK,IRESP)
       IF(GMEB_WORK)THEN
          IO%CMEB_GNDRES = 'S92' ! In Version8 and 9.0, only this option existed
       ELSE
          IO%CMEB_GNDRES = 'NON'
       ENDIF
    ENDIF
    YRECFM='MEB_TALL_VEG'
    CALL READ_SURF(HPROGRAM,YRECFM,IO%LMEB_TALL_VEG,IRESP)
    YRECFM='MEB_INT_PLUN'
    CALL READ_SURF(HPROGRAM,YRECFM,IO%LMEB_INT_PLUN,IRESP)
    YRECFM='MEB_INT_ULUN'
    CALL READ_SURF(HPROGRAM,YRECFM,IO%LMEB_INT_ULUN,IRESP)
    YRECFM='MEB_INT_USFC'
    CALL READ_SURF(HPROGRAM,YRECFM,IO%LMEB_INT_USFC,IRESP)
  ELSE
    IO%LFORC_MEASURE =.FALSE.
    IO%LMEB_LITTER   =.FALSE. 
    IO%CMEB_GNDRES   ='NON'
...
!
!* number of clay and sand layers
!
IF (IVERSION>9 .OR. (IVERSION==9 .AND. IBUGFIX>=1)) THEN
  YRECFM='CLAY_LAYER'
  CALL READ_SURF(HPROGRAM,YRECFM,IO%NCLAY_LAYER,IRESP)
ELSE
  IO%NCLAY_LAYER=1
ENDIF
!
...
ELSE
  DO JLAYER=1,IO%NGROUND_LAYER
    WRITE(YLVL,'(I4)') JLAYER
    YRECFM='CLAY'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    CALL READ_SURF(HPROGRAM,YRECFM,K%XCLAY(:,JLAYER),IRESP)
  ENDDO    
ENDIF
!
...
IF (IVERSION>9 .OR. (IVERSION==9 .AND. IBUGFIX>=1)) THEN
  YRECFM='SAND_LAYER'
  CALL READ_SURF(HPROGRAM,YRECFM,IO%NSAND_LAYER,IRESP)
ELSE
  IO%NSAND_LAYER=1
ENDIF
!
...
ELSE
  DO JLAYER=1,IO%NGROUND_LAYER
    WRITE(YLVL,'(I4)') JLAYER
    YRECFM='SAND'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    CALL READ_SURF(HPROGRAM,YRECFM,K%XSAND(:,JLAYER),IRESP)
  ENDDO    
ENDIF
!
...
IF ( NVEG_IRR /= 0 ) THEN
  ...
  DO JLAYER=1,LEN_TRIM(YWORK)
    ...
    ENDIF
  ENDDO
  IF ( ANY(DTV%NPAR_VEG_IRR_USE(:)<1 .OR. II/=NVEG_IRR) ) &
