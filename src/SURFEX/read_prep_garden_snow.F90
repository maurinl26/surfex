...
REAL, DIMENSION(NSNOW_LAYER_MAX) :: XWSNOW_GD, XZSNOW_GD, XRSNOW_GD, XTSNOW_GD, XLWCSNOW_GD, &
                                    XSG1SNOW_GD, XSG2SNOW_GD, XHISTSNOW_GD, XMMPSNOW_GD, XAGESNOW_GD
!
REAL, DIMENSION(NSNOW_LAYER_MAX,NIMPUR_MAX) :: XIMPURSNOW_GD
!
LOGICAL           :: LFILE
!
...
IF (LNAM_READ) THEN
  ...
  IF (GFOUND) THEN
    ...
    XMMPSNOW_GD = XMMPSNOW
    XAGESNOW_GD(:) = XAGESNOW(:)
    DO JIMP=1,NIMPUR
      XIMPURSNOW_GD(:,JIMP) = XIMPURSNOW(:,JIMP)
    ENDDO
    !
  ENDIF
  !
  ...
  ALLOCATE(XIMPURSNOW_p(NSNOW_LAYER_GD,NIMPUR))
  ...
  XIMPURSNOW_p =XIMPURSNOW_GD(1:NSNOW_LAYER_GD,1:NIMPUR)
