...
!     #######################################################
!
!!****  *READ_SURF_ATM_DATE* - routine to initialise the date TPTIME
!!
!!    PURPOSE
!!    -------
!!
...
!-------------------------------------------------------------------------------
!
!* look for a date in the namelist NAM_SURF_ATM
!----------------------------------------------
!
...
!
  CALL READ_PREP_SURF_ATM_CONF(HPROGRAM,YFILE,YFILETYPE,YFILEPGD,YFILEPGDTYPE,&
...
!------------------------------------------------------------------
!
IF (LEN_TRIM(YFILE)==0 .AND. (TPTIME%TDATE%YEAR==XUNDEF.OR.TPTIME%TDATE%MONTH==XUNDEF &
                               .OR.TPTIME%TDATE%DAY==XUNDEF.OR.TPTIME%TIME==XUNDEF)) THEN  
!
...
!-----------------------------------------------------------------------
!
    CALL ABOR1_SFX('READ_SURF_ATM_DATE: DATE NOT SET')
  END IF
END IF
!
...
!
IF (KYEAR /= NUNDEF .AND. KMONTH /= NUNDEF .AND. KDAY /= NUNDEF .AND. PTIME /= XUNDEF) THEN
  IF (KYEAR /= TPTIME%TDATE%YEAR .OR. KMONTH /= TPTIME%TDATE%MONTH &
         .OR. KDAY /= TPTIME%TDATE%DAY .AND. PTIME /= TPTIME%TIME) THEN  
    WRITE(UNIT=KLUOUT, FMT=*) 'WARNING in READ_SURF_ATM_DATE'
  ...
  ELSE
    ...
    WRITE(UNIT=KLUOUT, FMT=*)'DATE in READ_SURF_ATM_DATE:'
    WRITE(UNIT=KLUOUT, FMT='(" YEAR=",I4," MONTH=",I4," DAY=",I4)') &
                                        KYEAR,KMONTH,KDAY  
    WRITE(UNIT=KLUOUT, FMT='(" TIME=",E13.6)') PTIME
  ENDIF
ENDIF
IF (LHOOK) CALL DR_HOOK('READ_SURF_ATM_DATE',1,ZHOOK_HANDLE)
!------------------------------------------------------------------------------
!
