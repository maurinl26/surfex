...
!-------------------------------------------------------------------------------
!
!* look for a date in the namelist NAM_PREP_TEB or NAM_PREP_SURF_ATM
!-------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
!
   CALL ABOR1_SFX('READ_TEB_DATE: DATE NOT SET')
  END IF
ENDIF
!
...
IF (KYEAR /= NUNDEF .AND. KMONTH /= NUNDEF .AND. KDAY /= NUNDEF .AND. PTIME /= XUNDEF) THEN
  IF (KYEAR /= TPTIME%TDATE%YEAR .OR. KMONTH /= TPTIME%TDATE%MONTH .OR. KDAY /= TPTIME%TDATE%DAY .AND. PTIME /= TPTIME%TIME) THEN
    WRITE(UNIT=KLUOUT, FMT=*)'WARNING in READ_TEB_DATE'
  ...
  ENDIF
ENDIF
IF (LHOOK) CALL DR_HOOK('READ_TEB_DATE',1,ZHOOK_HANDLE)
!------------------------------------------------------------------------------
!
