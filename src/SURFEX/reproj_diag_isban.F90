...
USE MODN_ISBA_n,           ONLY : LSNOWSYTRON,LSNOWPAPPUS
!
#ifdef SFX_OL
...
IF (OPROSNOW) THEN
  ...
  IF ( OSURF_EVAP_BUDGET ) THEN
     ...
     IF ( OMEB_PATCH ) THEN
       ...
       DEK%XUNLOAD   (:) = DEK%XUNLOAD   (:) * ZCORR_SLOPE(:)
  ...
  IF (OSURF_MISC_BUDGET) THEN
    ...
    IF ( PEK%TSNOW%SCHEME=='3-L' .OR. PEK%TSNOW%SCHEME=='CRO' ) THEN
      ...
      ENDWHERE
      !
      IF (PEK%TSNOW%SCHEME=='CRO' .AND. GCOMPUTEDIAG) THEN
        WHERE(DMK%XTWSNOW>0.)
          DMK%XSNDPT_12H     (:) = DMK%XSNDPT_12H     (:) * ZCORR_SLOPE(:)
  ...
  !
ENDIF
IF (PEK%TSNOW%SCHEME=='CRO' .AND. LSNOWSYTRON) THEN
  DEK%XSYTMASS (:)    = DEK%XSYTMASS    (:) * ZCORR_SLOPE(:)
ENDIF
IF (PEK%TSNOW%SCHEME=='CRO' .AND. LSNOWPAPPUS) THEN
  DEK%XQDEP_TOT (:)   = DEK%XQDEP_TOT   (:) * ZCORR_SLOPE(:)
  DEK%XQ_OUT_SUBL (:) = DEK%XQ_OUT_SUBL (:) * ZCORR_SLOPE(:)
  DEK%XSNOWDEBTC (:)  = DEK%XSNOWDEBTC  (:) * ZCORR_SLOPE(:)
ENDIF
!
