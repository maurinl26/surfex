...
!
REAL, DIMENSION(SIZE(PTA)) :: ZDF_ROOF,ZTS3,ZTS4 ! snow-free fraction
...
!
!
IF (SIZE(PDN_ROOF) /= SIZE(T%XT_ROOF,1)) CALL ABOR1_SFX("Error: sizes are different")
IF (LHOOK) CALL DR_HOOK('ROOF_LAYER_E_BUDGET_0',0,ZH)
DO JJ=1,SIZE(PDN_ROOF)
  ...
  ZTS3(JJ) = ZTS_ROOF(JJ)**3
  ZTS4(JJ) = ZTS_ROOF(JJ)**4
  !
  ...
  !
ENDDO
IF (LHOOK) CALL DR_HOOK('ROOF_LAYER_E_BUDGET_0',1,ZH)
!
...
!              ------------
! 
IF (LHOOK) CALL DR_HOOK('ROOF_LAYER_E_BUDGET_1',0,ZH)
DO JJ=1,SIZE(T%XT_ROOF,1)
  ...
  ZB(JJ,1) = ZB(JJ,1) + ZDF_ROOF(JJ) * (1.-T%XGREENROOF(JJ)) * (                           &
                        ...
                        + ZIMP_LW * XSTEFAN * T%XEMIS_ROOF(JJ) * 4.*ZTS3(JJ)))      &
  ...
  ZY(JJ,1) = ZY(JJ,1) + (1.-T%XGREENROOF(JJ))                                                            &
                      * (PDN_ROOF(JJ)*PGSNOW_ROOF(JJ) + ZDF_ROOF(JJ) * ( PABS_SW_ROOF(JJ)                &
                        ...
                        + ZIMP_LW * XSTEFAN * T%XEMIS_ROOF(JJ) * ZTS4(JJ) * ( 3.*ZIMPL-ZEXPL ) )) &
  ...
  !
ENDDO
IF (LHOOK) CALL DR_HOOK('ROOF_LAYER_E_BUDGET_1',1,ZH)
!
!
...
IF (LHOOK) CALL DR_HOOK('ROOF_LAYER_E_BUDGET_2',0,ZH)
DO JJ=1,SIZE(T%XT_ROOF,1)
  !
  ...
  ELSE
     !
     ZTRAD_ROOF(JJ) = ( ZTS4(JJ) + &
                     4.*ZIMPL*ZTS3(JJ) * (T%XT_ROOF(JJ,1) - ZTS_ROOF(JJ)) )**0.25
     !
     !* absorbed LW
  ...
  ENDIF
ENDDO
IF (LHOOK) CALL DR_HOOK('ROOF_LAYER_E_BUDGET_2',1,ZH)
  !
  !
...
IF (CT%LCHECK_TEB) CT%XHEAT_RR_ROOF = ZHEAT_RR_ROOF
  !
IF (LHOOK) CALL DR_HOOK('ROOF_LAYER_E_BUDGET_ZIMB',0,ZH)
DO JJ=1,SIZE(T%XT_ROOF,1)
  ...
  ENDIF
ENDDO
IF (LHOOK) CALL DR_HOOK('ROOF_LAYER_E_BUDGET_ZIMB',1,ZH)
!
