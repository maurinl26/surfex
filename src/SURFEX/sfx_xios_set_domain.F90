...
!
!
GRECT=(KDIM2/=1).OR.(HGRID=="LONLAT REG").OR.(HGRID=="CONF_PROJ").OR.(HGRID=="CARTESIAN").OR.(HGRID=="IGN       ")
!
IF (GRECT) THEN 
   !
   ...
   ELSE
      IF ((HGRID/="CONF PROJ ").OR.(HGRID=='IGN       ')) &
           PRINT*,"SFX_XIOS_SET_DOMAIN : Managing "//HGRID//" type grid is under test"
   ...
   !
   IF (PRESENT(PCORNER_LAT) .AND. (HGRID /='CARTESIAN') .AND. (HGRID /="IGN       ")) THEN 
