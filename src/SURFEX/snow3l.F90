...
SUBROUTINE SNOW3L(HSNOWRES, OMEB, HIMPLICIT_WIND,                           &
                  ...
                  PSNOWAGE, PTSTEP, PPS, PSR, PUNLOAD, PRR, PPSN3L,         &
                  PTAR,PTAC,PTG,PSW_RAD,PQA,PVMOD,PLW_RAD, PRHOA,           &
...
!
 CALL SNOW3LFALL(PTSTEP, PSR, PTAR, PVMOD, ZSNOW, PSNOWRHO, PSNOWDZ, &
...
IF (HSNOWDRIFT == 'DFLT') THEN
   CALL SNOW3LDRIFT(PTSTEP,PFORESTFRAC,PVMOD,PTAR,PQA,PPS,PRHOA,&
...
!
! Some details:
! using (implicit conduction flux between layers 1 & 2)
! G_1,2+ = Lambda*(T_1+ - T_2+)
!
! where Lambda = ZSCONDA/(dz_1 + dz_2)/2
!
! and define: dz_1*c_n = 1/CT  and T_1 = PTS
!
! so:
!
! T_1+ = (T_1*Zb + Zc + CT*Lambda*T_2+)/Za
!
! T_1+ = (T_1*Zb + Zc)/Za + (CT*Lambda/Za)*T_2+
!
! T_1+ = Term1 + Term2 * T_2+
!
! the above is implicitly solved with snow T profile outside of here
! and the "Term"s are defined as:
!
