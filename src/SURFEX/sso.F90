...
!            -----------------------------
!
INTEGER, DIMENSION(:), ALLOCATABLE :: ISSO, ISSOT, ISSO_ANIS, ISSOT_ANIS
...
ELSE
  !warning: mind the order (the same as the 'if' branch)
  CALL GATHER_AND_WRITE_MPI(U%XSEA,ZSEA)
  CALL GATHER_AND_WRITE_MPI(UG%G%XMESH_SIZE,ZMESH_SIZE) 
  CALL GATHER_AND_WRITE_MPI(XSSQO,ZSSQO)
  CALL GATHER_AND_WRITE_MPI(ISSQO,ISSQOT)
...
ALLOCATE(ISSO(NL),ISSO_ANIS(NL))
!
IF (NRANK==NPIO) THEN
  ALLOCATE(ISSOT(U%NDIM_FULL),ISSOT_ANIS(U%NDIM_FULL))
  ISSOT(:) = 0
  WHERE (GSSO(:)) ISSOT(:) = 1
  DEALLOCATE(GSSO)
  ...
  ISSOT_ANIS(:) = 0
  WHERE (GSSO_ANIS(:)) ISSOT_ANIS(:) = 1
  DEALLOCATE(GSSO_ANIS)
  CALL READ_AND_SEND_MPI(ISSOT_ANIS,ISSO_ANIS)
  DEALLOCATE(ISSOT,ISSOT_ANIS)
ELSE
  !warning: mind the order (the same as the 'if' branch)
  CALL READ_AND_SEND_MPI(ISSOT,ISSO)
  CALL READ_AND_SEND_MPI(ISSOT_ANIS,ISSO_ANIS)
...
WHERE(ISSO_ANIS(:)==1) OSSO_ANIS(:) = .TRUE.
DEALLOCATE(ISSO,ISSO_ANIS)
!
!----------------------------------------------------------------------------
!
