...
INTEGER :: ILUOUT, IS2, INFOMPI, JP, ICPT, JCOV, JI, JL, J, N2, IREQ, IDX,&
                        IDX_SAVE, JT
LOGICAL :: GMULTITYPE
...
SELECT CASE (HFILETYPE)
  ...
  CASE ('ASCLLV')
    ...
    print * , ' NRANK = ' , NRANK
    print * , ' NPIO = ' , NPIO
    print * , 'before call read_ascllv'
    print * , ' HFILENAME = ' , HFILENAME
    print * , ' HSUBROUTINE = ' , HSUBROUTINE
    IF (NRANK==NPIO) CALL READ_ASCLLV(UG, U, USS, &
                        HPROGRAM,HSUBROUTINE,HFILENAME)
    print * , 'after call read_ascllv'
...
N2 = SIZE(NSIZE_ALL,2)
ALLOCATE(NSIZE(U%NSIZE_FULL,N2))
!
...
IF (NPROC>1) THEN
  ...
  IF (HFILETYPE=='DIRECT'.OR.HFILETYPE=='DIRTYP') THEN
    ...
    !
    ALLOCATE(ISIZE (NSIZE_max,N2,NPROC))
    ALLOCATE(ISIZE0(NSIZE_max,N2))
    !
    IDX_SAVE = IDX_R
    IDX = IDX_SAVE + NRANK
    !
...
!$OMP PARALLEL PRIVATE(ZHOOK_HANDLE_OMP) 
    IF (LHOOK) CALL DR_HOOK('TREAT_FIELD_packs',0,ZHOOK_HANDLE_OMP)
!$OMP DO SCHEDULE(DYNAMIC,1) PRIVATE(J,ICPT,JI) COLLAPSE(2)
    DO JP=0,NPROC-1
      DO J=1,N2
        ICPT = 0
        DO JI = 1,SIZE(NINDEX)
          IF (NINDEX(JI)==JP) THEN
            ICPT = ICPT + 1
            ISIZE(ICPT,J,JP+1) = NSIZE_ALL(JI,J)
          ENDIF
        ENDDO
      ENDDO
    ENDDO
!$OMP END DO
    IF (LHOOK) CALL DR_HOOK('TREAT_FIELD_packs',1,ZHOOK_HANDLE_OMP)
!$OMP END PARALLEL

    IF (LHOOK) CALL DR_HOOK('TREAT_FIELD_22',0,ZHOOK_HANDLE)
    DO JP=0,NPROC-1
      IF (JP==NRANK) CYCLE
      ...
      ENDIF
#ifdef SFX_MPI        
      CALL MPI_ISEND(ISIZE(:,:,JP+1),NSIZE_max*N2*KIND(ISIZE)/4,&
                           MPI_INTEGER,JP,IDX,NCOMM,NREQ(IREQ),INFOMPI)
#endif
    ENDDO
    IF (LHOOK) CALL DR_HOOK('TREAT_FIELD_22',1,ZHOOK_HANDLE)

    IF (LHOOK) CALL DR_HOOK('TREAT_FIELD_size',0,ZHOOK_HANDLE)
    NSIZE(:,:) = ISIZE(1:NSIZE_TASK(NRANK),:,NRANK+1)
    IF (LHOOK) CALL DR_HOOK('TREAT_FIELD_size',1,ZHOOK_HANDLE)
...
#ifdef SFX_MPI        
      CALL MPI_RECV(ISIZE0,NSIZE_max*N2*KIND(ISIZE0)/4,MPI_INTEGER,&
...
#ifdef SFX_MPI    
    IF (LHOOK) CALL DR_HOOK('TREAT_FIELD_waitall',0,ZHOOK_HANDLE)
    CALL MPI_WAITALL(NPROC-1,NREQ(1:NPROC-1),ISTATUS2,INFOMPI)
    IF (LHOOK) CALL DR_HOOK('TREAT_FIELD_waitall',1,ZHOOK_HANDLE)
#endif
    !
