...
REAL, DIMENSION(PK%NSIZE_P)            :: ZFAPIR, ZFAPAR, ZFAPIR_BS, ZFAPAR_BS,ZRN_SHADE,ZRN_SUNLIT
...
IF(IO%LMEB_PATCH(KPATCH).OR.IO%LFLOOD)THEN
  ...
  IF(IO%LMEB_PATCH(KPATCH))THEN
    ...
    IF(PRESENT(PDIR_SW))THEN
      ...
      CALL PACK_SAME_RANK(PK%NR_P,PRN_SHADE(:), ZRN_SHADE(:))
      CALL PACK_SAME_RANK(PK%NR_P,PRN_SUNLIT(:), ZRN_SUNLIT(:)) 
      !
    ENDIF
    !
  ENDIF
  ...
  IF(PRESENT(PDIR_SW))THEN
    ...
    DO JSWB=1,ISWB
      ...
      CALL RADIATIVE_TRANSFERT(KK%XVEGTYPE,                                      &
              ...
              ZRN_SHADE, ZRN_SUNLIT        )
      CALL UNPACK_SAME_RANK(PK%NR_P,ZRN_SUNLIT(:),PRN_SUNLIT(:),ZPUT0)
      CALL UNPACK_SAME_RANK(PK%NR_P,ZRN_SHADE(:),PRN_SHADE(:),ZPUT0)
