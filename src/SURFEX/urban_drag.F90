...
!     #########
    SUBROUTINE URBAN_DRAG(TOP, T, B, HIMPLICIT_WIND, PTSTEP, PT_CANYON, PQ_CANYON,  &
                          ...
                          PQA_ROOF, PTS_ROOF, PTS_ROAD, PTS_WALL, PTS_GARDEN,       &
...
!                            wall/roof/window
!!         03 /17 (M. Goret) add Demuzere et al 2017 Surface Interception Distribution approach
!!         03 /17 (M. Goret) bug fix in 8.2, replace U_CANYON+W_CAN by U_LOWCAN+W_CAN
!!                           in the arg of urban_exch_coef according to E. Redon
!!
!-------------------------------------------------------------------------------
!
!*       0.     DECLARATIONS
...
USE MODD_CSTS,ONLY : XLVTT, XCPD, XG, XKARMAN
...
REAL, DIMENSION(:), INTENT(IN)    :: PQA_ROOF       ! hum. near the roof
...
!
REAL, DIMENSION(SIZE(PTA)) :: ZW_STAR !! 
REAL                       :: ZQ0              !! 
!
REAL                       :: ZUSTAR2      ! square of friction velocity (m2/s2
REAL                       :: ZVMOD        ! module of the horizontal wind at t+1
!
! for calculation of momentum fluxes
REAL                       :: ZUSTAR_ROOF  ! friction velocity for roofs
!
...
INTEGER                   ::  JLOOP, JJ            !! 
!
REAL,PARAMETER :: ZZ0_O_Z0H = 200.  ! z0/z0h ratio used in Mascart (1995) formulation.
...
!              -------------------------------
!
  ZAVDELT_ROOF = PDELT_ROOF(JJ) * PDELT_SNOW_ROOF(JJ)
!
!*      4.3    Roof specific humidity
!              ----------------------
!
  ZQ_ROOF(JJ) = PQSAT_ROOF(JJ) * ZAVDELT_ROOF
!
...
!
DO JJ=1,SIZE(PTA)
  ZLE      =(PQSAT_ROOF(JJ) - PQA_ROOF(JJ)) &
                 * PAC_ROOF(JJ) * PDELT_ROOF(JJ) * XLVTT * PRHOA(JJ)
!
  IF (ZLE>0.) THEN
     ZLE_MAX     = T%XWS_ROOF(JJ) / PTSTEP * XLVTT
     PAC_ROOF_WAT(JJ) = PAC_ROOF(JJ) * MIN ( 1. , ZLE_MAX/ZLE )
  ELSE IF (PDELT_ROOF(JJ)==0.) THEN
     PAC_ROOF_WAT(JJ)=0.
  ELSE
...
!
IF (TOP%CCH_BEM == "DOE-2") THEN
   ZCHTCN_WALL(:) = CHTC_VERT_DOE(PTS_WALL(:), PT_CANYON(:))
   ZCHTCS_WALL(:) = 0.5 * (CHTC_SMOOTH_LEE_DOE (ZCHTCN_WALL(:), PU_CANYON(:)) + &
                           CHTC_SMOOTH_WIND_DOE(ZCHTCN_WALL(:), PU_CANYON(:)) )
   PAC_WALL(:) = CHTC_ROUGH_DOE(ZCHTCN_WALL(:), ZCHTCS_WALL(:), T%XROUGH_WALL(:)) / XCPD / PRHOA(:)
ELSE
...
DO JLOOP=1,3
 !
  ...
  DO JJ=1,SIZE(PTA)
    ...
    ZQ0     = (PTS_WALL  (JJ) - PT_CANYON(JJ)) * PAC_WALL  (JJ) * T%XWALL_O_GRND(JJ)
    ...
    !
    IF (ZQ0 >= 0.) THEN
      ZW_STAR(JJ) = ( (XG * PEXNA(JJ) / PTA(JJ)) * ZQ0 * T%XBLD_HEIGHT(JJ)) ** (1/3.)
    ELSE
      ZW_STAR(JJ) = 0.
    ENDIF
!
...
DO JJ=1,SIZE(PTA)
  !
  ZLE      = ( PQSAT_ROAD(JJ) - PQ_LOWCAN(JJ) )                   &
                   *   PAC_ROAD(JJ) * PDELT_ROAD(JJ) * XLVTT * PRHOA(JJ)
  !
  IF (ZLE>0.) THEN
     ZLE_MAX     = T%XWS_ROAD(JJ) / PTSTEP * XLVTT
     PAC_ROAD_WAT(JJ) = PAC_ROAD(JJ) * MIN ( 1. , ZLE_MAX/ZLE )
  ELSE IF (PDELT_ROAD(JJ)==0.) THEN
     PAC_ROAD_WAT(JJ) = 0.
  ELSE
...
END DO
  ...
  !              ------------------------------------
  !
  ZCHTCN_WIN(:) = CHTC_VERT_DOE(B%XT_WIN1(:), PT_CANYON(:))
  !
  PAC_WIN(:) = 0.5 * (CHTC_SMOOTH_LEE_DOE(ZCHTCN_WIN(:), PU_CANYON(:)) + &
                      CHTC_SMOOTH_WIND_DOE(ZCHTCN_WIN(:), PU_CANYON(:)) ) &
                   / PRHOA(:) / XCPD
  !
  !-------------------------------------------------------------------------------
  !
  ...
  IF (TOP%LCANOPY) THEN
    !
    PDUWDU_ROAD(:) = 0. ! implicitation already taken into account in PUW_ROAD
    PDUWDU_ROOF(:) = 0.
    !
    DO JJ=1,SIZE(PTA)
    IF(HIMPLICIT_WIND=='OLD')THEN
      !   old implicitation
      ZUSTAR2 = (ZCD_ROAD(JJ)*PU_LOWCAN(JJ)*PPEW_B_COEF_LOWCAN(JJ))/              &
                    (1.0-PRHOA(JJ)*ZCD_ROAD(JJ)*PU_LOWCAN(JJ)*PPEW_A_COEF_LOWCAN(JJ))
    ELSE
      !   new implicitation
      ZUSTAR2 = (ZCD_ROAD(JJ)*PU_LOWCAN(JJ)*(2.*PPEW_B_COEF_LOWCAN(JJ)-PU_LOWCAN(JJ)))/  &
                    (1.0-2.0*PRHOA(JJ)*ZCD_ROAD(JJ)*PU_LOWCAN(JJ)*PPEW_A_COEF_LOWCAN(JJ))
      !                   
      ZVMOD = PRHOA(JJ)*PPEW_A_COEF_LOWCAN(JJ)*ZUSTAR2 + PPEW_B_COEF_LOWCAN(JJ)
      ZVMOD = MAX(ZVMOD,0.)
      !
      IF(PPEW_A_COEF_LOWCAN(JJ)/= 0.)THEN
        ZUSTAR2 = MAX( ( ZVMOD - PPEW_B_COEF_LOWCAN(JJ) ) / (PRHOA(JJ)*PPEW_A_COEF_LOWCAN(JJ)), 0.)
      ENDIF
      !              
    ENDIF
    !
    ...
    !
    !
    ZUSTAR_ROOF = PVMOD(JJ) * XKARMAN / LOG(PZREF(JJ)/ZZ0_ROOF(JJ))
    !
    PUW_ROOF(JJ)    = - ZUSTAR_ROOF**2
  ...
  ELSE
    ...
    IF(HIMPLICIT_WIND=='OLD')THEN
      !   old implicitation
      ZUSTAR2 = (PCD(JJ)*PVMOD(JJ)*PPEW_B_COEF(JJ))/            &
                    (1.0-PRHOA(JJ)*PCD(JJ)*PVMOD(JJ)*PPEW_A_COEF(JJ))
    ELSE
      !   new implicitation
      ZUSTAR2 = (PCD(JJ)*PVMOD(JJ)*(2.*PPEW_B_COEF(JJ)-PVMOD(JJ)))/ &
                    (1.0-2.0*PRHOA(JJ)*PCD(JJ)*PVMOD(JJ)*PPEW_A_COEF(JJ)) 
      !                   
      ZVMOD = PRHOA(JJ)*PPEW_A_COEF(JJ)*ZUSTAR2 + PPEW_B_COEF(JJ)
      ZVMOD = MAX(ZVMOD,0.)
      !
      IF(PPEW_A_COEF(JJ)/= 0.)THEN
        ZUSTAR2 = MAX( ( ZVMOD - PPEW_B_COEF(JJ) ) / (PRHOA(JJ)*PPEW_A_COEF(JJ)), 0.)
      ENDIF
      !                        
    ENDIF
    !
    PUSTAR_TOWN(JJ) = SQRT(ZUSTAR2)
    END DO
    !
  ENDIF
  !
