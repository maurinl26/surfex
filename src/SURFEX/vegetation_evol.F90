...
REAL, PARAMETER                   :: ZCDILU_ESM2 = 288. !Tuning for ISBA-CC
REAL, PARAMETER                   :: ZCDILU_CA08 = 350. !Original value from Yin (2002)
!
...
REAL, PARAMETER                   :: ZCALV      = 1.814   !Coef for Calvet et al. (2008), nitrogen dilu param without vegetation temperature
!
...
IF (.NOT.GIRRIG_ONLY) THEN
   ...
   !
   IF(GMASK.AND.IO%LLULCC)THEN
     IF(IO%CPHOTO=='NCB')THEN
       WHERE(GCROP(:)) 
            DEK%XFHARVEST(:) = (PK%XTURNOVER(:,1) + PK%XTURNOVER(:,2) + PK%XTURNOVER(:,3) + PK%XTURNOVER(:,5)) * XGTOKG           
            PK%XTURNOVER(:,1) = 0.0 ! reset turnover since AGB has been harvested
            PK%XTURNOVER(:,2) = 0.0 ! reset turnover since AGB has been harvested
            PK%XTURNOVER(:,3) = 0.0 ! reset turnover since AGB has been harvested
            PK%XTURNOVER(:,5) = 0.0 ! reset turnover since AGB has been harvested
       ELSEWHERE
            DEK%XFHARVEST(:) = 0.0
       ENDWHERE
     ELSE
...
ZTG_VEG (:) = 0.0
!
IF(IO%CNITRO_DILU=='CA08')THEN
  ZCDILU3=ZCDILU_CA08
ELSEIF(IO%CNITRO_DILU=='ESM2')THEN
  ZCDILU3=ZCDILU_ESM2
ELSE
  ZCDILU3=XUNDEF
ENDIF
...
IF (GMASK) THEN
  ...
  IF(IO%CNITRO_DILU/='NONE')THEN
    ...
    DO JTYPE=1,SIZE(PK%XVEGTYPE_PATCH,2)
       ...
       DO JI = 1,INI
          IF(IO%CNITRO_DILU=='CA08')THEN
            ! Version Calvet et al. (2008)
            ZPARAM_TYPE =        ZDILUDEC  * (ZDECIDUS +  ZCALV - ZCOEF * ZFERT(JI)) &
                        + (1.0 - ZDILUDEC) * (            ZCALV - ZCOEF * ZFERT(JI))
          ELSEIF(IO%CNITRO_DILU=='ESM2')THEN
            ! Version Yin (2002) but for ISBA-CC (CNRM-ESM2)
