...
!     #########
    SUBROUTINE WASP_SEAFLUX    (S, PMASK,KSIZE_WATER,KSIZE_ICE,     &
...
!!    -------
!     
!      Calculate the sea surface fluxes with modified bulk algorithm WASP  
!   
...
!              ------------------------------------
IF (LHOOK) CALL DR_HOOK('MODI_WASP_SEAFLUX:WASP_SEAFLUX',0,ZHOOK_HANDLE)
!
IR_WATER(:)=0
IR_ICE(:)=0
J1=0
...
!
!
IF (LHOOK) CALL DR_HOOK('MODI_WASP_SEAFLUX:WASP_SEAFLUX',1,ZHOOK_HANDLE)
!-------------------------------------------------------------------------------
!
CONTAINS
!
...
REAL(KIND=JPRB) :: ZHOOK_HANDLE
!
IF (LHOOK) CALL DR_HOOK('WASP_SEAFLUX:TREAT_SURF',0,ZHOOK_HANDLE)
!
DO JJ=1, SIZE(KMASK)
...
IF (YTYPE=='W') THEN
  !
  CALL WASP_FLUX(S, ZW_Z0SEA,ZW_TA,ZW_EXNA,ZW_RHOA,ZW_SST,ZW_EXNS,&
...
END DO
IF (LHOOK) CALL DR_HOOK('WASP_SEAFLUX:TREAT_SURF',1,ZHOOK_HANDLE)
!
END SUBROUTINE TREAT_SURF
! 
END SUBROUTINE WASP_SEAFLUX
