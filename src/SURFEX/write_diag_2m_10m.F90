...
!!      B. Decharme 12/2023 : ATAN2(0,0)  don't work with all compilater for XWIDD_MEAN
!!                            + used XPI instead of 3.14....
!------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
!              -------------------------------
!
REAL, DIMENSION(SIZE(D%XT2M))  :: ZWORK
!
...
IF (DFO%N2M>=1) THEN
  ...
  IF (D%NCOUNT_STEP/=0) THEN
    ...
    !
    WHERE(D%XMER10M_MEAN(:)==0.0.AND.D%XZON10M_MEAN(:)==0.0)
         ZWORK(:)=0.0
    ELSEWHERE
         ZWORK(:)=ATAN2(D%XMER10M_MEAN(:)/D%NCOUNT_STEP,D%XZON10M_MEAN(:)/D%NCOUNT_STEP)
    ENDWHERE
    !
    D%XWIDD_MEAN = 180.0 + (90.0-180.0*ZWORK(:)/XPI)
    !
