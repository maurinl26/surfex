...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!                                                                                           !!
!!   WARNING : if a diag is added (or delated), please update surfex_fields.xml in xmls dir  !!
!!                                                                                           !!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!
REAL, DIMENSION(U%NSIZE_NATURE,IO%NGROUND_LAYER) :: ZSOILCARBL, ZSOILLITTERL
!
...
REAL              :: ZWGHT
!
INTEGER           :: JI, JL, JK, JP, INI, INP, INL, INLIT, INCARB, IMASK
!
        ...
        DO JL=1,INL
           DO JI=1,PK%NSIZE_P
              ...
              IF(JL<=PK%NWG_LAYER(JI))THEN
                 ...
                 ZSOILLITTERL(IMASK,JL) = ZSOILLITTERL (IMASK,JL)+PEK%XSOILDIF_LITTER(JI,JL,JK)*XGTOKG*PK%XPATCH(JI)
...
!
! * Anthropogenic carbon pool (already account for patch fraction)
!
   YRECFM='cProduct'
   ...
   DO JP=1,INP
      DO JL=1,IO%NNDECADAL
         DO JI=1,INI
            ZLULUCARB  (JI   )=ZLULUCARB  (JI   )+S%XCSTOCK_DECADAL(JI,JL,JP)
            ZP_LULUCARB(JI,JP)=ZP_LULUCARB(JI,JP)+S%XCSTOCK_DECADAL(JI,JL,JP)
         ENDDO
      ENDDO
   ENDDO
   DO JP=1,INP
      DO JL=1,IO%NNCENTURY
         DO JI=1,INI
            ZLULUCARB  (JI   )=ZLULUCARB  (JI   )+S%XCSTOCK_CENTURY(JI,JL,JP)
            ZP_LULUCARB(JI,JP)=ZP_LULUCARB(JI,JP)+S%XCSTOCK_CENTURY(JI,JL,JP)
         ENDDO
      ENDDO
   ENDDO
...
!
! * Below-ground litter carbon stock by layer
!
IF(IO%CRESPSL=='DIF')THEN
  YRECFM='cLitSoilLev'
  YCOMMENT='Litter carbon mass in each soil level (kgC m-2)'      
  IF (LALLOW_ADD_DIM)  THEN
     CALL WRITE_SURF(DUO%CSELECT,HPROGRAM,YRECFM,ZSOILLITTERL(:,:),IRESP,YCOMMENT,HNAM_DIM=YGROUND_LAYER_DIM_NAME)
  ELSE
      DO JL=1,INL
          WRITE(YLVL,'(I4)') JL
          YWORK=YRECFM(:LEN_TRIM(YRECFM))//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
          CALL WRITE_SURF(DUO%CSELECT,HPROGRAM,YWORK,ZSOILLITTERL(:,JL),IRESP,YCOMMENT)    
      ENDDO
  ENDIF       
ENDIF
!
...
IF(IO%LCLEACH)THEN
  ...
  ZWORK(:)=ID%DE%XFDOC(:)
  IF(IO%LLULCC.AND.LCPL_RIVCARB)THEN
    ZWORK(:)=ZWORK(:)+S%XCCONSRV(:)
  ENDIF
...
IF (IO%LLULCC) THEN
!
! * land use carbon conservation flux
!
  YRECFM='fLucConserv'
  YCOMMENT='land use carbon conservation flux over land (kgC m-2 s-1)'
  CALL WRITE_SURF(DUO%CSELECT,HPROGRAM,YRECFM,S%XCCONSRV(:),IRESP,HCOMMENT=YCOMMENT)
!
...
!
  ...
  IF(IO%LLULCC_MANAGE)THEN
    ZP_WORK(:,:)=ZP_WORK(:,:) + S%XFANTATM(:,:)
  ENDIF
  CALL SUM_PATCH(ZP_WORK,ZFLUC)
  IF(.NOT.(LCPL_RIVCARB.AND.IO%LCLEACH))THEN
    ZFLUC(:)=ZFLUC(:)+S%XCCONSRV(:)
  ENDIF
...
!
  ...
  ZP_WORK(:,:)=S%XFLUATM(:,:) + S%XFLURES(:,:)
  IF(IO%LLULCC_MANAGE)THEN
    ZP_WORK(:,:)=ZP_WORK(:,:) + S%XFLUANT(:,:)
  ENDIF
  CALL SUM_PATCH(ZP_WORK,ZWORK)
  CALL WRITE_SURF(DUO%CSELECT,HPROGRAM,YRECFM,ZWORK(:),IRESP,HCOMMENT=YCOMMENT)
!
! * Carbon flux toward the atmosphere due to land-use clearance
!
  ...
  CALL SUM_PATCH(S%XFLUATM,ZFLUATM)
...
!
  IF(IO%LLULCC_MANAGE)THEN  
    ...
    CALL SUM_PATCH(S%XFLUANT,ZWORK)
    CALL WRITE_SURF(DUO%CSELECT,HPROGRAM,YRECFM,ZWORK(:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
! * Carbon flux toward the litter carbon pool due to land-use clearance
  ...
  CALL SUM_PATCH(S%XFLURES,ZWORK)
  CALL WRITE_SURF(DUO%CSELECT,HPROGRAM,YRECFM,ZWORK(:),IRESP,HCOMMENT=YCOMMENT)
!
...
!
  IF(IO%LLULCC_MANAGE)THEN  
    ...
    CALL SUM_PATCH(S%XFANTATM,ZWORK)
    CALL WRITE_SURF(DUO%CSELECT,HPROGRAM,YRECFM,ZWORK(:),IRESP,HCOMMENT=YCOMMENT)
  ENDIF
!
...
!-------------------------------------------------------------------------------
!
SUBROUTINE SUM_PATCH(PP_WORK,PWORK)
!
!
REAL, DIMENSION(:,:), INTENT(IN ) :: PP_WORK
!
REAL, DIMENSION(:),   INTENT(OUT) :: PWORK
!
REAL(KIND=JPRB) :: ZHOOK_HANDLE
!
IF (LHOOK) CALL DR_HOOK('WRITE_DIAG_CMIP_CARBON_ISBA_N:SUM_PATCH',0,ZHOOK_HANDLE)
!
PWORK(:) = 0.0
DO JP=1,INP
   DO JI=1,INI
      PWORK(JI)=PWORK(JI)+PP_WORK(JI,JP)
   ENDDO
ENDDO
!
IF (LHOOK) CALL DR_HOOK('WRITE_DIAG_CMIP_CARBON_ISBA_N:SUM_PATCH',1,ZHOOK_HANDLE)
!
END SUBROUTINE SUM_PATCH
!
!-------------------------------------------------------------------------------
!
