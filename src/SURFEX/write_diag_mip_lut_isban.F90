...
!!    -------------
!!      Original      10/2016
!!      R. Séférian   05/2017 correct units for carbon flux
!!      B. Decharmle     2023 add hurs diag  
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
!              ------------
...
INTEGER           :: JI, JL, JK, JP, INI, INP, INL, INLIT, IMASK
!
...
!
YRECFM='hussLut'
YCOMMENT='near-surface air specific humidty (2m above displacement height, i.e. t_ref) on land-use tile (kg kg-1)'
DO JP=1,INP
   PK => NP%AL(JP)
   DK => ID%ND%AL(JP)
...
!
YRECFM='hursLut'
YCOMMENT='near-surface air relative humidty (2m above displacement height, i.e. t_ref) on land-use tile (-)'
DO JP=1,INP
   PK => NP%AL(JP)
   DK => ID%ND%AL(JP)
   CALL UNPACK_SAME_RANK(PK%NR_P,DK%XHU2M(:),ZWORK(:,JP),XUNDEF)
ENDDO
ZWORK_LUT(:,:)=PATCH_TO_LUT_FUNC(S%XPATCH(:,:),ZWORK(:,:))
CALL WRITE_LUT_FIELD(ZWORK_LUT)
!
...
IF(IO%LLULCC)THEN
  ...
  DO JP=1,INP
     ...
     DO JI=1,PK%NSIZE_P
        IMASK = PK%NR_P(JI)
        ZNBP (IMASK,JP) = ZNBP (IMASK,JP) - DEK%XFHARVEST(JI)
     ENDDO
  ENDDO
ENDIF
!
...
!
   ...
   ZWORK(:,:)=S%XFLUATM(:,:) + S%XFLURES(:,:)
   IF(IO%LLULCC_MANAGE)THEN
     ZWORK(:,:)=ZWORK(:,:) + S%XFLUANT(:,:)
   ENDIF
...
!
   ...
   ZWORK_LUT(:,:)=PATCH_TO_LUT_FUNC(ZNOPATCH(:,:),S%XFLUATM(:,:),LFRAC=.TRUE.)
   CALL WRITE_LUT_FIELD(ZWORK_LUT) 
!
...
!
   IF(IO%LLULCC_MANAGE)THEN
     ...
     ZWORK_LUT(:,:)=PATCH_TO_LUT_FUNC(ZNOPATCH(:,:),S%XFLUANT(:,:),LFRAC=.TRUE.)
     CALL WRITE_LUT_FIELD(ZWORK_LUT)
   ENDIF
!
...
!
   ...
   ZWORK_LUT(:,:)=PATCH_TO_LUT_FUNC(ZNOPATCH(:,:),S%XFLURES(:,:),LFRAC=.TRUE.)
   CALL WRITE_LUT_FIELD(ZWORK_LUT) 
!
...
!
   IF(IO%LLULCC_MANAGE)THEN
     ...
     ZWORK_LUT(:,:)=PATCH_TO_LUT_FUNC(ZNOPATCH(:,:),S%XFANTATM(:,:),LFRAC=.TRUE.)
     CALL WRITE_LUT_FIELD(ZWORK_LUT)
   ENDIF
!
   ...
   IF(IO%LLULCC_MANAGE)THEN
     ...
     DO JP=1,INP
        DO JL=1,IO%NNDECADAL
           DO JI=1,INI
              ZWORK(JI,JP)=ZWORK(JI,JP)+S%XCSTOCK_DECADAL(JI,JL,JP)
           ENDDO
        ENDDO
     ENDDO
     DO JP=1,INP
        DO JL=1,IO%NNCENTURY
           DO JI=1,INI
              ZWORK(JI,JP)=ZWORK(JI,JP)+S%XCSTOCK_CENTURY(JI,JL,JP)
           ENDDO
        ENDDO
     ENDDO
     ZWORK_LUT(:,:)=PATCH_TO_LUT_FUNC(ZNOPATCH(:,:),ZWORK(:,:),LFRAC=.TRUE.)
