...
ENDIF
!
! * land use water conservation flux
!
IF(IO%LLULCC.AND.IO%CISBA=='DIF')THEN
   YRECFM='mrrolu'
   YCOMMENT='land use water conservation flux over land (kg m-2 s-1)'
   CALL WRITE_SURF(DUO%CSELECT,HPROGRAM,YRECFM,S%XWCONSRV(:),IRESP,HCOMMENT=YCOMMENT)
ENDIF
!
!-------------------------------------------------------------------------------
! * floodplain flood
