...
!!      Modified    11/2023 by B. Decharme : XPATCH come back in write_diag_pgd_isban.F90
!!
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
...
IF(IO%NPATCH>1)THEN
  ...
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                    NP%AL(JP)%NR_P,NP%AL(JP)%XPATCH(:),ILU,S%XWORK_WR)    
  ENDDO
ENDIF
!
!* Vegetation fraction
...
IF(IO%CISBA=='DIF')THEN
  ...
  DO JP = 1,IO%NPATCH
    PK => NP%AL(JP)
    CALL  WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
              PK%NR_P,PK%XDG2(:),ILU,S%XWORK_WR)
  ENDDO    
  !  
  IF(IO%NPATCH>1)THEN
    YRECFM='DG2_DIF_ISBA'
  ...
  DO JP = 1,IO%NPATCH
    PK => NP%AL(JP)
    CALL  WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                PK%NR_P,PK%XRUNOFFD(:),ILU,S%XWORK_WR)
  ENDDO    
  !
  ...
  DO JP = 1,IO%NPATCH
    PK => NP%AL(JP)
    CALL  WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                PK%NR_P,ZWORK2(1:PK%NSIZE_P,JP),ILU,S%XWORK_WR)
  ENDDO    
  DEALLOCATE(ZWORK2)
  !
  IF(IO%NPATCH>1)THEN
  ...
  DO JL=1,SIZE(PK%XROOTFRAC,2)
    DO JP = 1,IO%NPATCH
      ...
      ENDDO
      CALL  WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
              PK%NR_P,ZWORK1(1:PK%NSIZE_P),ILU,S%XWORK_WR)
    ENDDO
  END DO
  DEALLOCATE(ZWORK1)
  !
...
!
IF(IO%CHORT/='DEF'.AND.IO%CISBA/='DIF')THEN
  YRECFM='DICE'
  YCOMMENT='soil ice depth for runoff (m)'
  DO JP = 1,IO%NPATCH
    PK => NP%AL(JP)
    CALL  WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                PK%NR_P,PK%XD_ICE(:),ILU,S%XWORK_WR)
  ENDDO    
ENDIF
!
!-------------------------------------------------------------------------------
...
!
  DO JL=1,SIZE(S%XVEGTYPE_PATCH,2)
    ...
    DO JP = 1,IO%NPATCH
      PK => NP%AL(JP)
      CALL  WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                  PK%NR_P,PK%XVEGTYPE_PATCH(:,JL),ILU,S%XWORK_WR)
    ENDDO      
  END DO
!
ENDIF
...
DO JP = 1,IO%NPATCH
  ...
  PEK => NPE%AL(JP)
  CALL  WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
              PK%NR_P,PEK%XRSMIN(:),ILU,S%XWORK_WR)
ENDDO
!
YRECFM='GAMMA'
...
DO JP = 1,IO%NPATCH
  ...
  PEK => NPE%AL(JP)
  CALL  WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
              PK%NR_P,PEK%XGAMMA(:),ILU,S%XWORK_WR)
ENDDO
!
YRECFM='CV'
...
DO JP = 1,IO%NPATCH
  ...
  PEK => NPE%AL(JP)
  CALL  WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
              PK%NR_P,PEK%XCV(:),ILU,S%XWORK_WR)
ENDDO
!
YRECFM='RGL'
...
DO JP = 1,IO%NPATCH
  ...
  PEK => NPE%AL(JP)
  CALL  WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
              PK%NR_P,PEK%XRGL(:),ILU,S%XWORK_WR)
ENDDO
!
YRECFM='EMIS_ISBA'
...
DO JP = 1,IO%NPATCH
  ...
  PEK => NPE%AL(JP)
  CALL  WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
              PK%NR_P,PEK%XEMIS(:),ILU,S%XWORK_WR)
ENDDO
!
YRECFM='WRMAX_CF'
...
DO JP = 1,IO%NPATCH
  ...
  PEK => NPE%AL(JP)
  CALL  WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
              PK%NR_P,PEK%XWRMAX_CF(:),ILU,S%XWORK_WR)
ENDDO
!
IF (ISIZE_LMEB_PATCH>0) THEN
  !
  ...
  DO JP = 1,IO%NPATCH
    ...
    PEK => NPE%AL(JP)
    CALL  WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                PK%NR_P,PEK%XH_VEG(:),ILU,S%XWORK_WR)
  ENDDO
  !
ENDIF
!
