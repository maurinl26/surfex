...
!!      C. Delire   02/2023  Good names and unities for soil gas scheme
!!
!-------------------------------------------------------------------------------
!
!*       0.    DECLARATIONS
...
IF (IO%LLULCC) THEN
   ...
   YCOMMENT='carbon flux due to crop harvesting'//YUNIT
...
!
    DO JL=1,IO%NGROUND_LAYER
        ...
        CALL WRITE_SURF(DUO%CSELECT,HPROGRAM,YRECFM,ZSOILDIF_SOC_TOT(:,JL),IRESP,HCOMMENT=YCOMMENT)
        !
...
!
IF (IO%LLULCC.AND.IO%CPHOTO=='NCB') THEN
  !
  ZFLUATM(:)    = 0.
  ZFLURES(:)    = 0.
  !
  ...
  DO JP=1,IO%NPATCH
     DO JI=1,INI 
        ZFLUATM (JI) = ZFLUATM (JI) + S%XFLUATM (JI,JP)
        ZFLURES (JI) = ZFLURES (JI) + S%XFLURES (JI,JP)
     ENDDO
  ENDDO
  !
  YRECFM='FLUATM'
...
IF (IO%LLULCC_MANAGE) THEN
  ...
  DO JP=1,IO%NPATCH
     DO JI=1,INI 
        ZCANT   (JI) = ZCANT   (JI) + (SUM(S%XCSTOCK_DECADAL(JI,:,JP)) + SUM(S%XCSTOCK_CENTURY(JI,:,JP)))
        ZFLUANT (JI) = ZFLUANT (JI) + S%XFLUANT (JI,JP)
        ZFANTATM(JI) = ZFANTATM(JI) + S%XFANTATM(JI,JP)
     ENDDO
  ENDDO
  !
  YRECFM='FLUANT'
