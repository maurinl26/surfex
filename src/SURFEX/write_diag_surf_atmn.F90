...
!
USE MODI_WRITE_DIAG_MIP_SURF_ATM_n
USE MODI_WRITE_DIAG_MIP_FX_SURF_ATM_n
!
...
IF (YSC%U%NDIM_SEA    >0) THEN
   CALL WRITE_DIAG_SEA_n(YSC%DTCO, YSC%DUO, YSC%U, YSC%SM, HPROGRAM,HWRITE)
ENDIF
!
IF (YSC%U%NDIM_WATER  >0) THEN
   CALL WRITE_DIAG_INLAND_WATER_n(YSC%DTCO, YSC%DUO, YSC%U, YSC%WM, YSC%FM, HPROGRAM,HWRITE)
ENDIF
!
IF (YSC%U%NDIM_NATURE >0) THEN
   CALL WRITE_DIAG_NATURE_n(YSC%DTCO, YSC%DUO, YSC%U, YSC%IM, YSC%NDST,YSC%BLOWSNW, HPROGRAM,HWRITE)
ENDIF
!
IF (YSC%U%NDIM_TOWN   >0) THEN
   CALL WRITE_DIAG_TOWN_n(YSC%DTCO, YSC%DUO, YSC%U, YSC%TM, YSC%GDM, YSC%GRM, YSC%HM, HPROGRAM,HWRITE)
ENDIF
!
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
! Writing
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
!
ITIME_RATIO=NINT(YSC%U%TTIME%TIME/YSC%DUO%XDIAG_TSTEP)
GWRITE     =(YSC%DUO%XDIAG_TSTEP==XUNDEF.OR.ABS(ITIME_RATIO*YSC%DUO%XDIAG_TSTEP-YSC%U%TTIME%TIME)<1.E-3)
!
IF (YSC%DUO%LDIAG_MIP.AND.GWRITE) THEN
!
   CALL WRITE_DIAG_MIP_SURF_ATM_n(YSC%DTCO, YSC%DUO, YSC%DU, YSC%U, HPROGRAM)
!
   CALL WRITE_DIAG_MIP_FX_SURF_ATM_n(YSC%DTCO, YSC%DUO, YSC%DU, YSC%U, YSC%UG, HPROGRAM)
!
ELSEIF (GWRITE) THEN
!
