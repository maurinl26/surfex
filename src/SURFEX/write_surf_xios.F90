...
!$OMP SINGLE
   ...
   IF (PRESENT(PFIELD1)) THEN
      ...
      ELSE
         CALL SFX_XIOS_SEND_BLOCK(HREC,HDOMAIN=YXIOS_DOMAIN, PFIELD=PFIELD1)
      ENDIF
      !
   ELSE IF (PRESENT(PFIELD2)) THEN 
      ...
      IF (TRIM(YAXIS) /= TRIM(YPATCH_DIM_NAME)) THEN 
         ...
         ELSE
            CALL SFX_XIOS_SEND_BLOCK(HREC,HDOMAIN=YXIOS_DOMAIN, PFIELD2=PFIELD2)
         ENDIF
      ELSE ! Per-patch fields are written through a loop, and with patch number suffix
         DO JI=1,SIZE(PFIELD2,2)
            ...
            ELSE
               WRITE(YLVL,'(I2)') JI
            ENDIF
            YNAME=HREC//'_P'//TRIM(YLVL)
            IF (.NOT. LXIOS_DEF_CLOSED ) THEN 
               CALL SFX_XIOS_DECLARE_FIELD(YNAME, YXIOS_DOMAIN, HCOMMENT=HCOMMENT)
            ELSE
               CALL SFX_XIOS_SEND_BLOCK(YNAME,HDOMAIN=YXIOS_DOMAIN, PFIELD=PFIELD2(:,JI))
            ENDIF
         ENDDO
      ENDIF
      !
   ELSE IF (PRESENT(PFIELD3)) THEN 
      ...
      IF (TRIM(YAXIS) /= TRIM(YPATCH_DIM_NAME)) THEN 
         ...
         ELSE
            CALL SFX_XIOS_SEND_BLOCK(HREC,HDOMAIN=YXIOS_DOMAIN, PFIELD3=PFIELD3)
         ENDIF
         !CALL ABOR1_SFX('XIAS CANNOT YET HANDLE 2 AXES IN ADDITION TO HORIZONTAL SPACE AXIS/AXES')
      ELSE
         ...
         DO JI=1,SIZE(PFIELD3,2)
            ...
            IF (.NOT. LXIOS_DEF_CLOSED ) THEN 
               CALL SFX_XIOS_DECLARE_FIELD(YNAME, YXIOS_DOMAIN, HCOMMENT=HCOMMENT)
            ELSE
               CALL SFX_XIOS_SEND_BLOCK(YNAME,HDOMAIN=YXIOS_DOMAIN, PFIELD2=PFIELD3(:,JI,:))
            ENDIF
         ENDDO
      ENDIF
   ENDIF
