...
!!    -------------
!!      Original    01/2003 
!!      P. Le Moigne      12/2004 : correct dimensionning if more than 10 layers in the soil 
!!      B. Decharme          2008 : Floodplains
!!      B. Decharme       01/2009 : Optional Arpege deep soil temperature write
!!      A.L. Gibelin      03/2009 : modifications for CENTURY model 
...
!!      Séférian/Decharme 08/2016 : fire scheme ; change landuse implementation
!!      B. Decharme       02/2017 : exact computation of saturation deficit near the leaf surface
!!      B. Decharme       02/2021 : explicit soil carbon and gas scheme:browse confirm wa
!!      B. Decharme       11/2023 : XPATCH come back in write_diag_pgd_isban.F90
!!      B. Decharme       12/2023 : spliting of previous writesurf_isban.F90 in several routines
...
USE MODD_DST_SURF
!
USE MODI_WRITESURF_ISBA_CC_n
USE MODI_WRITESURF_ISBA_LANDUSE_n
!
...
DO JL=1,IWORK
  ...
  YFORM='(A26,I1.1,A4)'
  IF (JL >= 10)  YFORM='(A26,I2.2,A4)'
  WRITE(YCOMMENT,FMT=YFORM) 'Temperature of soil layer ',JL,' (K)'
  DO JP = 1,IO%NPATCH
...
DO JL=1,IO%NGROUND_LAYER
  ...
  YFORM='(A35,I1.1,A8)'
  IF (JL >= 10)  YFORM='(A35,I2.2,A8)'
  WRITE(YCOMMENT,FMT=YFORM) 'Liquid water content of soil layer ',JL,' (m3/m3)'
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
...
DO JL=1,IWORK
  ...
  YFORM='(A34,I1.1,A8)'
  IF (JL >= 10)  YFORM='(A34,I2.2,A8)'
  WRITE(YCOMMENT,YFORM) 'Solid water content of soil layer ',JL,' (m3/m3)'
  DO JP = 1,IO%NPATCH
    CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
...
DO JP = 1,IO%NPATCH
  CALL WRITESURF_GR_SNOW(OSNOWDIMNC, HSELECT, HPROGRAM, 'VEG', '     ', KI, &
           ...
           S%XHEA_WR, S%XAGE_WR, S%XSG1_WR, S%XSG2_WR, S%XHIS_WR, S%XALB_WR, S%XMMP_WR,&
           S%XIMP_WR,S%XMEPRA_WR)
ENDDO
!
!-------------------------------------------------------------------------------
!
...
IF (IO%CPHOTO=='NIT' .OR. IO%CPHOTO=='NCB') THEN
  ...
  DO JNB=2,IO%NNBIOMASS
    ...
    YFORM='(A16,I1.1,A10)'
    WRITE(YCOMMENT,FMT=YFORM) 'X_Y_RESP_BIOMASS',JNB,' (kgDM m-2 day-1)'
    DO JP = 1,IO%NPATCH
      CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
...
!           -------
!
CALL WRITESURF_ISBA_CC_n(HSELECT, IO, S, NP, NPE, KI, HPROGRAM)
!
!-------------------------------------------------------------------------------
!
!
...
!           ---------------------------
!
CALL WRITESURF_ISBA_LANDUSE_n(HSELECT, IO, S, NP, NPE, KI, HPROGRAM)
!
!-------------------------------------------------------------------------------
!
!*       6.  Other
...
!* friction velocity
!
IF (IO%CZ0HEAT=='Z95') THEN
   YRECFM='USTAR'
   YCOMMENT='Friction velocity (m/s)'
   DO JP = 1,IO%NPATCH
      CALL WRITE_FIELD_1D_PATCH(HSELECT,HPROGRAM,YRECFM,YCOMMENT,JP,&
                      NP%AL(JP)%NR_P,NPE%AL(JP)%XUSTAR(:),KI,S%XWORK_WR)
   ENDDO
ENDIF
!
! * Dust
