...
!
!* type of ground resistance for baresoil evaporation
!
YRECFM='GNDRES'
YCOMMENT=YRECFM
 CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,IO%CGNDRES,IRESP,HCOMMENT=YCOMMENT)
!
! 
...
!
   YRECFM='MEB_GNDRES'
   YCOMMENT=YRECFM
   CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,IO%CMEB_GNDRES,IRESP,HCOMMENT=YCOMMENT)
!
!* flag indicating if vertical scaling option for tall vegetation is used or not
!
   YRECFM='MEB_TALL_VEG'
   YCOMMENT=YRECFM
   CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,IO%LMEB_TALL_VEG,IRESP,HCOMMENT=YCOMMENT)
!
!* flag indicating if Raleigh & Lundquist (2012) intercepted snow melt option is used or not
!
   YRECFM='MEB_INT_PLUN'
   YCOMMENT=YRECFM
   CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,IO%LMEB_INT_PLUN,IRESP,HCOMMENT=YCOMMENT)
!
!* flag indicating if Lundquist (2021) intercepted snow unloading option is used or not
!
   YRECFM='MEB_INT_ULUN'
   YCOMMENT=YRECFM
   CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,IO%LMEB_INT_ULUN,IRESP,HCOMMENT=YCOMMENT)
!
!* flag indicating if aged snow properties for unloading snow as opposed to using falling 
!  snow properties is used or not
!
   YRECFM='MEB_INT_USFC'
   YCOMMENT=YRECFM
   CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,IO%LMEB_INT_USFC,IRESP,HCOMMENT=YCOMMENT)
!
ENDIF
!
...
YRECFM='CLAY_LAYER'
YCOMMENT=YRECFM
 CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,IO%NCLAY_LAYER,IRESP,HCOMMENT=YCOMMENT)
!
!* clay fraction
!
IF (IO%NCLAY_LAYER==1) THEN
  YRECFM='CLAY'
  YCOMMENT='X_Y_CLAY, uniform clay fraction'
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,K%XCLAY(:,1),IRESP,HCOMMENT=YCOMMENT)
ELSE
  DO JLAYER=1,IO%NGROUND_LAYER
    WRITE(YLVL,'(I4)') JLAYER     
    YRECFM='CLAY'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    YCOMMENT='clay fraction for the soil layer '//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,K%XCLAY(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
ENDIF
!
...
YRECFM='SAND_LAYER'
YCOMMENT=YRECFM
 CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,IO%NSAND_LAYER,IRESP,HCOMMENT=YCOMMENT)
!
!* sand fraction
!
IF (IO%NSAND_LAYER==1) THEN
  YRECFM='SAND'
  YCOMMENT='X_Y_SAND, uniform sand fraction'
  CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,K%XSAND(:,1),IRESP,HCOMMENT=YCOMMENT)
ELSE
  DO JLAYER=1,IO%NGROUND_LAYER
    WRITE(YLVL,'(I4)') JLAYER     
    YRECFM='SAND'//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    YCOMMENT='sand fraction for the soil layer '//ADJUSTL(YLVL(:LEN_TRIM(YLVL)))
    CALL WRITE_SURF(HSELECT, HPROGRAM,YRECFM,K%XSAND(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
  END DO
ENDIF
!
!* soil organic carbon
