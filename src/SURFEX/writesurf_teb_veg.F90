...
END IF
!
!* equivalent vegetation canopy air specific humidity
!
YRECFM = HPATCH//HSURF//'_QC'
YRECFM=ADJUSTL(YRECFM)
YCOMMENT='X_Y_'//HSURF//'_QC ((kg/kg)'
CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,PEK%XQC(:),IRESP,HCOMMENT=YCOMMENT)
!
...
IF (HSURF/='GH') THEN
   !
   YRECFM=HSURF
   ALLOCATE(IMASK_P(SIZE(PEK%XTG,1)))
   ...
   CALL WRITESURF_GR_SNOW(OSNOWDIMNC, HSELECT, HPROGRAM, YRECFM, HPATCH,         &
                       ...
                       S%XHIS_WR, S%XALB_WR, S%XMMP_WR, S%XIMP_WR, S%XMEPRA_WR)
   !
   DEALLOCATE(IMASK_P)
   !
END IF
