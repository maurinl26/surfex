...
!     #########
      SUBROUTINE WRITESURF_TEB_n (HSELECT, OSNOWDIMNC, DTCO, U, TOP, BOP, T, B, ODATA_ROAD_DIR, TPN, &
                                  GDO, GDS, GDPEK, GRO, GRS, GRPEK, TH, HPROGRAM,KPATCH,HWRITE)
!     ####################################
!
...
REAL, DIMENSION(0,1) :: ZMMP_WR
REAL, DIMENSION(0,0,1,1) :: ZIMP_WR
REAL, DIMENSION(0,0,1) :: ZMEPRA_WR
!
...
!
YPATCH='   '
IF (TOP%NTEB_PATCH>1) WRITE(YPATCH,FMT='(A1,I1,A)') 'T',KPATCH,'_'
!
!
!*       3.     Prognostic fields:
!               -----------------
...
!
!* soil column under building deep temperatures (same number of layers as for road)
!
IF (SIZE(T%XT_BLD,2).NE.SIZE(T%XT_ROAD,2)) THEN
   CALL ABOR1_SFX("Road column and soil column under building layers number must be equal")
ENDIF
!
DO JLAYER=1,TOP%NTEB_SOIL
      !
      ...
      YRECFM=ADJUSTL(YRECFM)
      !
      YSTRING='Soil temperature under buildings for layer '
      !
      IF (JLAYER.LT.10) THEN
         WRITE(YCOMMENT,'(A,I1.1,A4)') TRIM(YSTRING), JLAYER,' (K)'
      ELSE
      ...
      ENDIF
      !
      CALL WRITE_SURF(HSELECT, HPROGRAM, YRECFM, T%XT_BLD(:,JLAYER), IRESP,HCOMMENT=YCOMMENT)
      !
ENDDO
!
!
...
! Valery: the above need to be parallelized. 
! (e.g. by storing in a character variable this information for all procs)
  ELSE
...
IF (LWRITE_EXTERN) THEN
  ...
  DO JLAYER=1,TOP%NTEB_SOIL
    ...
    ELSE
      WRITE(YRECFM,FMT='(A,I2.1)') 'D_ROAD',JLAYER
    ENDIF
  ...
  IF (TOP%CBEM=='BEM') THEN
    ...
    DO JLAYER=1,BOP%NMASS_LAYER
      WRITE(YRECFM,FMT='(A,I1.1)') 'D_MASS',JLAYER
      YCOMMENT='MASS layer thickness'
      CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,B%XD_MASS(:,JLAYER),IRESP,HCOMMENT=YCOMMENT)
    END DO
  ENDIF
  !
ENDIF
...
!
DO JCOMP=1,SIZE(B%XTI_BLD,2)
   WRITE(YRECFM,'(A3,A5,I1.1)') YPATCH,'TIBLD',JCOMP
   WRITE(YCOMMENT,'(A9,I1.1,A4)') 'X_Y_TIBLD',JCOMP,' (K)'
   YRECFM=ADJUSTL(YRECFM)
   CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,B%XTI_BLD(:,JCOMP),IRESP,HCOMMENT=YCOMMENT)
ENDDO
!
!
!* outdoor window temperature
!
WRITE(YRECFM,'(A3,A6)') YPATCH,'T_WIN1'
YRECFM=ADJUSTL(YRECFM)
WRITE(YCOMMENT,'(A14)') 'X_Y_T_WIN1 (K)'
CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,B%XT_WIN1(:),IRESP,HCOMMENT=YCOMMENT)
!
IF (TOP%CBEM=='BEM') THEN
  !* internal building specific humidity
  !
  DO JCOMP=1,SIZE(B%XQI_BLD,2)
     WRITE(YRECFM,'(A3,A5,I1.1)') YPATCH,'QIBLD',JCOMP
     WRITE(YCOMMENT,'(A9,I1.1,A4)') 'X_Y_QIBLD',JCOMP,' (kg/kg)'
     YRECFM=ADJUSTL(YRECFM)
     CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,B%XQI_BLD(:,JCOMP),IRESP,HCOMMENT=YCOMMENT)
  ENDDO
  !
  !* indoor window temperature
  !
  WRITE(YRECFM,'(A3,A6)') YPATCH,'T_WIN2'
  YRECFM=ADJUSTL(YRECFM)
  WRITE(YCOMMENT,'(A14)') 'X_Y_T_WIN2 (K)'
  ...
  DO JLAYER=1,BOP%NFLOOR_LAYER
     DO JCOMP=1,SIZE(B%XT_FLOOR,3)
        ...
        YRECFM=ADJUSTL(YRECFM)
        CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,B%XT_FLOOR(:,JLAYER,JCOMP),IRESP,HCOMMENT=YCOMMENT)
     END DO
  END DO
  !
  ...
  DO JLAYER=1,BOP%NMASS_LAYER
     DO JCOMP=1,SIZE(B%XT_MASS,3)
        ...
        YRECFM=ADJUSTL(YRECFM)
        CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,B%XT_MASS(:,JLAYER,JCOMP),IRESP,HCOMMENT=YCOMMENT)
     END DO
  END DO        
  !
  ...
  YRECFM=ADJUSTL(YRECFM)
  YCOMMENT='PSOLD (Pa)'
  CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,B%XPSOLD(:),IRESP,HCOMMENT=YCOMMENT)
  !
  !* Status of nocturnal ventilation
  !
  DO JCOMP=1,SIZE(B%XVENTNIGSW,2)
     WRITE(YRECFM,'(A3,A7,I1.1)') YPATCH,'VENTNIG',JCOMP
     YRECFM=ADJUSTL(YRECFM)
     YCOMMENT='Fraction ()'
     CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,B%XVENTNIGSW(:,JCOMP),IRESP,HCOMMENT=YCOMMENT)
  ENDDO
  !
  !* Status of shading when vacant
  !
  DO JCOMP=1,SIZE(B%XSHADVACSW,2)
     WRITE(YRECFM,'(A3,A7,I1.1)') YPATCH,'SHADVAC',JCOMP
     YRECFM=ADJUSTL(YRECFM)
     YCOMMENT='Fraction ()'
     CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,B%XSHADVACSW(:,JCOMP),IRESP,HCOMMENT=YCOMMENT)
  ENDDO
  !
ENDIF
!
...
YRECFM=ADJUSTL(YRECFM)
YCOMMENT='TDEEP_TEB (K)'
 CALL WRITE_SURF(HSELECT,HPROGRAM,YRECFM,T%XTDEEP_TEB(:),IRESP,HCOMMENT=YCOMMENT)
!
!* snow mantel
!*  
...
YRECFM='RF'
 CALL WRITESURF_GR_SNOW(OSNOWDIMNC,HSELECT,HPROGRAM,YRECFM,YPATCH,&
                        ...
                        ZSG2_WR,ZHIS_WR,ZALB_WR, ZMMP_WR, ZIMP_WR, ZMEPRA_WR)
!
YRECFM='RD'
 CALL WRITESURF_GR_SNOW(OSNOWDIMNC,HSELECT,HPROGRAM,YRECFM,YPATCH,&
...
IF (TOP%LSOLAR_PANEL) THEN
  ...
  IF (.NOT. ASSOCIATED(TPN%XTHER_PRODC_DAY)) THEN
    ! for PREP cases
    ALLOCATE(TPN%XTHER_PRODC_DAY(SIZE(B%XTI_BLD,1)))
...
!            ---------------
!
IF (TOP%LGARDEN.AND.TOP%LURBHYDRO) THEN
  CALL WRITESURF_TEB_HYDRO_n(HSELECT, TH, HPROGRAM,YPATCH, TOP%NTEB_SOIL)
END IF
!
!-------------------------------------------------------------------------------
!
