...
!     #########
    SUBROUTINE Z0EFF (IO, HSNOW_SCHEME, &
                      ...
                      PZ0_MEBN,PZ0H_MEBN,PZ0EFF_MEBN,PUSTAR                     )
...
REAL, PARAMETER :: ZNU=0.156E-4    ! air kinematic viscosity
REAL :: ZCOEF
!-------------------------------------------------------------------------------
!
...
IF (IO%CZ0HEAT=='Z95') THEN
   Z0HREF(:)=PZ0H_WITH_SNOW(:)
   PZ0H_WITH_SNOW(:)=PZ0_WITH_SNOW(:)*EXP(-XKARMAN*IO%XZ0HEAT*SQRT(PZ0_WITH_SNOW(:)*PUSTAR(:)/ZNU))
   PZ0H_WITH_SNOW(:)=MAX(PZ0H_WITH_SNOW(:),Z0HREF(:))
ENDIF
!
