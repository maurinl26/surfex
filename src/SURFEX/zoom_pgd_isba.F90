...
IF (IVERSION>=8) THEN
  ...
  IF (ISIZE_LMEB_PATCH>0)THEN
    ...
    CALL READ_SURF(HINIFILETYPE,'MEB_GNDRES',  IO%CMEB_GNDRES,  IRESP)
    CALL READ_SURF(HINIFILETYPE,'MEB_TALL_VEG',IO%LMEB_TALL_VEG,IRESP)
    CALL READ_SURF(HINIFILETYPE,'MEB_INT_PLUN',IO%LMEB_INT_PLUN,IRESP)
    CALL READ_SURF(HINIFILETYPE,'MEB_INT_ULUN',IO%LMEB_INT_ULUN,IRESP)
    CALL READ_SURF(HINIFILETYPE,'MEB_INT_USFC',IO%LMEB_INT_USFC,IRESP)
  ELSE      
...
IF (IVERSION > 8) THEN
   CALL READ_SURF(HINIFILETYPE,'GNDRES',IO%CGNDRES,IRESP)
ELSE
   IO%CGNDRES = 'NON'
ENDIF
...
!
 ...
 CALL MPPDB_CHECK_SURFEX2D(ZSSO_SLOPE,"ZOOM_PGD_ISBA:ZSSO_DIR",PRECISION,ILUOUT)
#endif
!
DEALLOCATE(ZAOSIP)
DEALLOCATE(ZAOSIM)
